...
`code.repo.feature_flags` 的过滤字段直接位于操作 payload：`domain` 为显式领域字符串，`source` 为 `java`、`properties`、`ini`、`ctmpl`、`shell` 或 `dotenv`，`hot_reload` 为布尔值，`consistency` 为布尔值（默认 `false`）。`domain`、`source` 接受字符串或 null（视为未提供），其他类型返回 bad request，不得静默忽略过滤。未知元数据不匹配显式过滤条件。`query` 用于选择配置分组；响应保留已选范围内该分组的全部定义、声明、读取和守卫证据，再执行一致性检查。

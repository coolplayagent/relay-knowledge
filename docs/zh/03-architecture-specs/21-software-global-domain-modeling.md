...
- `software_entities`、`software_statements` 和 `software_ontology_diagnostics` 与旧表并行物化。API trait/interface/protocol、测试符号、配置 flag、构建定义、release artifact、deployment unit/resource、service definition 和文档单元进入类型化实体；所有 accepted active statement 都必须具有同 scope evidence、source kind 和 extractor version。Projection schema version 6 会把旧 scope 标为 stale，并通过既有 durable task、lease、checkpoint 和单仓单写者路径重建，不做破坏性原地转换。

...
回归覆盖两模块以上的多跳传播、循环与预算、坐标歧义、外部依赖、profile 变体、增量删除依赖、稳定身份，原 dependencies 命令在没有 Java import 时的声明依赖输出、游标校验，以及包含 153 个节点和 455 条关系的 152 子模块跨页读取。Maven 语义参考 [reactor 文档](https://maven.apache.org/guides/mini/guide-multiple-modules.html) 和 [依赖机制](https://maven.apache.org/guides/introduction/introduction-to-dependency-mechanism.html)。

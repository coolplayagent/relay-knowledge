...
代码仓库查询采用 AST 优先、精确 grep 兜底的内部协作。Definition、reference 和 hybrid 查询先访问版本化 tree-sitter 图和 SQLite FTS 读模型；当结构化路径存在明确召回缺口时，允许在同一已索引 revision scope 内执行有界 `rg` 精确文本检索。当 dependency library 未作为代码图 target 建立索引时，import 查询也可以用 unresolved external dependency target hint 作为 grep 词项。Grep 兜底只属于词法层：它可以恢复精确源码行并提高召回，但不能返回 AST 图没有证明的 resolved edge 或 confidence。

...
仓库级 retention 会在 SQLite schema 初始化时幂等增加 `code_repository_retention_jobs`、其 `cutoff_publication_generation`（截止发布代次）水位列、单例 `code_repository_retention_scans` 调度游标表，以及 `code_repository_retention_catalog`（仓库保留目录）修订号；现有父任务和扫描记录使用兼容默认值。合格已索引仓库上限默认值为 10，可通过 `RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES` 在 `1..=i64::MAX` 范围内修改。候选发现在每个事务中最多推进 64 条目录记录，可通过游标在重启后续跑，并在影响候选排序或资格的目录变更发生后重启。升级不需要手工迁移数据：现有已索引仓库会在后续成功发布或 maintenance pass 中被评估，选中的仓库通过既有分阶段 scope GC 排空；repository row 和 alias 保持兼容且不会删除。回滚时不得让旧 binary 与新 binary 并发运行；旧 binary 可以忽略新增父任务表、扫描表和目录表，但无法安全协调仓库级 retention，也无法恢复已被子 GC job 退役的 scope。
Retention-trigger 兼容迁移会在每次打开文件数据库时先于通用 schema-marker 快速路径执行。通过受界的 `sqlite_master` 存在性检查识别 11 个仍使用 `INSERT OR IGNORE` 的旧版 `code_repository_retention_activity_*` trigger；只有命中时才在一个 transaction 内删除并重建完整 trigger 集合。失败会回滚该 trigger transaction 并显式返回 `storage_unavailable`，不会重写已索引事实，通用 schema marker 保持不变，也不会把派生检索状态标 stale 或触发重建。停止所有 writer 后，operator 可确认 `sqlite_master` 中匹配 trigger 的 SQL 不再包含 `INSERT OR IGNORE`，且 `relay_storage_schema_state` 的既有版本不变。Binary-only rollback 可以保留升级后的 trigger 定义，因为其 SQL 兼容且事实数据未改变。精确数据库状态回滚必须恢复升级前 runtime-database backup，新旧 binary 始终不得并发写入。
...
- SQLite graph-store 打开时会运行独立于 marker 的 retention-trigger 兼容检查，在不失效派生检索状态、不重写已索引事实的前提下原子替换旧版 dirty-activity trigger，暴露 trigger 诊断，并把升级前 database backup 保留为精确回滚边界。

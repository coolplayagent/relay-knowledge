...
- Skill 产物包含 workflow reference，以及 Draft 2020-12 的 `knowledge-map.schema.json`、`codespec-map.schema.json`、`business-glossary.schema.json`。Metadata gate 解析全部三个 schema，保护 Knowledge Map v4 的 map/shard/redirect branch 与 recent-history bounds、CodeSpec 强类型目录根、Business Glossary v1 bounds、代表性正反例和未知字段兼容策略。GitHub Release archive 必须显式包含全部 schema；基于目录的 ClawHub 发布自动包含相同 reference。Schema 接受不能替代 `map validate`，也不授权直接编辑生成 artifact。

Knowledge Map schema rollout 受 release gate 约束。PR CI 同时使用当前源码 binary 与 crates.io 最新 stable binary 校验仓库 map，并把完整结果发布为 artifact。低于 `1.1.17` 的 stable reader 无法读取可见的 v4 CodeSpec 与 Knowledge map 且源码版本更高时记录为 `staged_pending_reader_release`；源码与 stable 同版本不兼容时以 `incompatible_same_version` 硬失败。stable 达到 `1.1.17` 后，任何不兼容都必须使门禁失败。后续 writer schema 只有在上一 stable reader 已能接受时才可成为默认格式。该门禁只诊断，不会重写仓库 map，也不会静默升级已安装 binary。

Repository Map v4 使用可见的 `codespec/` 与 `knowledge/` 根目录，每张 map 的根文件只保存 `omitted_through` checkpoint 与最近 16 条历史。reader release 必须先于 writer migration release。迁移最后发布 `knowledge/knowledge-map.yaml`，随后在 `.knowledge/knowledge-map.yaml` 写入 v4 redirect，使旧 reader 明确失败。不再保留 repository map history 目录或数据级 rollback 命令；更早审计与恢复依赖 Git 或仓库备份。仓库 `docs/` 内容不会自动移动，卸载也不得删除任一仓库拥有的 map 目录。

forward migration 在复制任一 legacy artifact 前，会以 no-follow 方式读取 live root，并按 legacy policy 完整验证其 graph；raw redirect、任一引用 artifact 腐坏、tree directory/entry symlink、不安全 destination 或 immutable target 冲突都会在 visible root 发布前 fail closed。引用的 archive chain 必须在 v4 publication 前完成校验。writer 随后按需再次发布相同 v4 root，保证 current 与 `.previous` reader root 都不再引用 archive；只有此后才能退役 history 文件。cleanup 限定在 owned contract directory，只接受名称严格匹配的普通 archive/index 文件以及自有 reader-grace marker，并在删除任何 archive 前启动 60 秒 grace，让已经观察到上一版 root 的 reader 完成读取；此后每次最多删除 1,024 项。若仍有剩余，已提交的 map mutation 仍返回成功，同时发出可观测的 cleanup-pending warning，由下一次 `map init` 续跑，`map validate` 在清理完成前保持 invalid。未知 entry、directory、link 或路径逃逸一律 fail closed 且不得删除；如果它们是在 root 持久发布后发现，则报告为 post-commit maintenance state，不得反向把已提交 mutation 报为失败。只有 legacy live root 不存在或已成为受支持 redirect 时，才能清理 legacy namespace history；仍然存活的 legacy manifest 所可能引用的全部 archive 必须保留。grace 期间或批次间崩溃仍保留有效 v4 root，重试可以幂等续跑；topic-shard GC 继续保留 current 与 reader-fallback root 引用。

v4 迁移不提供数据级 rollback surface。仓库自有 map 内容通过 Git 或 operator backup 恢复；binary 回滚前必须先恢复迁移前 map snapshot，再启动旧 writer。redirect recovery 继续保持 crash-safe，并在替换 legacy root 前从保留 map route 解析 glossary URI。legacy root 的 business route 可使用历史 `.knowledge/business-glossary.yaml` URI 或 canonical `knowledge/glossary/business-glossary.yaml` URI。每个当前 Knowledge Map mutation 都先取得 legacy lock、再取得 current lock，并把 legacy lock 持有到最终 legacy-state probe 与 publication 完成，避免 legacy writer 创建第二个可写 root。目录 mutation 对 legacy snapshot 做 preflight，并在迁移出的 v4 snapshot 中修复 reserved repository route。

Repository Map 是仓库拥有的版本化 contract，不是平台 runtime state。writer 使用被忽略且带 marker 的 OS advisory-lock inode 与唯一 prepared 名称；no-follow reopen 会拒绝 symlink、reparse point 与非普通文件。crash residue 扫描有界，保留活跃或未知 entry，也不会抢占旧 writer 可能持有的无 marker lock。升级前先停止旧 writer，并提交或备份完整 map contract。v4 迁移只删除经过校验的生成式 history archive/index 文件，不删除 authored document、仍被 reader root 引用的 topic shard 或其他仓库内容。卸载 binary 或 service 不会删除仓库拥有的 map 内容。

带 archive chain 的 legacy v2/v3 root 在完成校验与 forward migration 前仍可读取。`map validate` 严格只读，会报告损坏 chain 或缺失 legacy index，不改变磁盘状态。`map init` 或 `map migrate --type knowledge --to-v4` 在 repository writer lock 下校验引用的 legacy chain、发布 v4，并执行上述有界 cleanup。v4 不再创建 archive 或 index node。省略 `map history --from` 时从最早保留条目开始；显式请求 `omitted_through` 及以前版本会返回历史不可用错误。长期审计保留是明确的 Git/backup 责任，不再由产品无限增长存储承担。

升级后可运行 `map init` 续跑过期 topic-shard 清理，无需制造内容更新或新增 history。仍被恢复根引用的分片保留；退役后的 60 秒内不删除，每次最多处理 1,024 个未引用普通分片，后续 init 继续。相同字段的 source update 不再消耗 source/map version 或写入新分片，必要的 legacy 迁移仍执行。本优化不改变 schema 或 SQLite 布局，不要求数据库重建；卸载与回滚的 Git/backup 边界不变。
...
Projection schema version 8 停止重复写入兼容图关系。旧数据库打开后 software projection 标为 stale；既有 fenced 九阶段 repair 重建投影，并清除该 scope 的遗留 `software_relationships` 行。空的兼容表与索引继续支持旧 import/retention cursor，常规受界 scope retention 也会回收旧 payload。打开数据库不会全库删除或执行 VACUUM；SQLite 复用空闲页，不承诺文件立即缩小。计数与 freshness 仍通过同一 lease/checkpoint barrier 发布。回滚到 version 7 reader 前须停止服务，恢复升级前事务一致的主库及全部 shard，或使用独立 runtime home 由旧 binary 重新索引；旧 reader 不能把新版空兼容表当成完整投影。Repository Map YAML 仍为 schema v4，不修改 authored source 或 map history 格式。
...
Query-index plan v5 在完整保留 v4 前缀的基础上追加 ordinal 20：`code_repository_feature_flags_source_key(source_scope, source_kind, source_key)`，共 21 个单元。数据库 warm open 只验证已有索引形状，不对 populated facts 同步创建该索引。旧 v1–v4 checkpoint 保留原 ordinal 和 resume payload 语义，超过旧版本末尾后输出 v5 token；新增索引作为一个 durable leased writer 单元构建。`feature-flag-key-index-v1` scope component 使普通 `repo index <alias> --ref <ref>` 即使在相同 HEAD 也重新构建旧 completed scope。缺少索引时 feature-flags 查询给出明确索引操作提示，不静默退化为 scope 扫描；升级后窄 key 查询继续使用 SQL candidate plan。回滚 binary 前须排空或取消未完成 v5 task，精确回滚需一起恢复 database/WAL/shard checkpoint。

- 配置注册表元数据升级会幂等增加 `code_repository_feature_flags.metadata_json`，旧行默认为 `{}`；warm-open capability 校验元数据列和 migration marker；缺少 deferred source-key 索引不会触发启动 DDL。Index fact-version refresh 经普通 durable task 流程补齐新抽取元数据，snapshot copy、删除和历史读取保持 source-scope 所有权。升级前独占 writer 并备份 runtime database 及全部 shard；binary rollback 可忽略新增列，但不能重现新版注册表分析，精确回滚须同时恢复升级前 database/shard checkpoint 和旧 binary。
...
Warm-open schema 能力检查包含 Java namespace 列、摘要表与维护触发器；仅有旧配置 schema 的数据库因此会在索引或复制文件前执行既有增量初始化。结构检查不要求延后构建的查询索引，也不在启动时回填旧 namespace 事实。

查询索引计划 v7 在 ordinal 23 追加 `code_repository_java_source_type_lookup(source_scope, package, type_name, source_set_kind, module_root, path)`，共 24 个单元。v1–v6 的 ordinal 身份及 resume payload 完整保留；v6 checkpoint 限于 0–22，只有越过该前缀才输出 v7。namespace/type 投影新增源集列，warm 初始化只更新结构化 schema 和 trigger，不回填事实，也不构建延迟索引。配置事实版本 v6 使普通同 HEAD 索引在持久租约下重建旧 completed scope；缺失的旧源集证据在重建前保持 unknown。回滚前完成或取消未完成 v7 任务，并一致恢复匹配的 database、WAL、shard 和 checkpoint。
...
`python-overload-declarations-v17` 事实组件修正挂起、match 捕获与延迟直接调用的证据。对同一 HEAD 执行普通索引会重建 v15 或 v16 已完成作用域，无需修改源码或 reset。既有任务租约与检查点仍是恢复依据：切换版本前，使用兼容二进制完成或取消未完成任务。启动阶段不回填调用事实；回滚时须一并保留数据库、WAL 与检查点，重建后重新取得快照选择器。

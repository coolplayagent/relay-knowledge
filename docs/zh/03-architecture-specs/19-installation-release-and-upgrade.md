...
仓库级 retention 会在 SQLite schema 初始化时幂等增加 `code_repository_retention_jobs` 及其 `cutoff_publication_generation`（截止发布代次）水位列；现有父任务使用兼容默认值 `0`。合格已索引仓库上限默认值为 10，可通过 `RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES` 修改。升级不需要手工迁移数据：现有已索引仓库会在下一次成功发布或 maintenance pass 被评估，选中的仓库通过既有分阶段 scope GC 排空；repository row 和 alias 保持兼容且不会删除。回滚时不得让旧 binary 与新 binary 并发运行；旧 binary 可以忽略新增父任务表，但无法安全协调父任务，也无法恢复已被子 GC job 退役的 scope。

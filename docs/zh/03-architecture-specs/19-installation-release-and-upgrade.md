...
rollback 必须先恢复普通 manifest backup，之后才能把根判断为未完成的 forward staging；其 prepared rollback marker 通过已同步的临时文件和同目录 rename 写入或替换，且只能在 rollback publication commit 后清理陈旧 redirect transition marker。rollback validation 与 redirect recovery 必须从保留 map route 解析 glossary URI，并在替换根之前解析该精确 artifact；即使 map root 位于 `.knowledge/`，canonical `knowledge/glossary/business-glossary.yaml` route 仍必须读取 canonical 文件。redirect recovery 在替换 legacy root 前还会比较已路由的 legacy glossary 与 canonical glossary，因此仅修改 legacy glossary 的旧 writer 内容会保留给修复，不会被 redirect 隐藏。legacy root 的 business route 可使用历史 `.knowledge/business-glossary.yaml` URI 或 canonical `knowledge/glossary/business-glossary.yaml` URI。每个当前 Knowledge Map mutation 都先取得 legacy lock、再取得 current lock，并把 legacy lock 持有到最终 legacy-state probe 与 publication 完成，避免 v2 writer 创建第二个可写 root。目录 mutation 会在创建 forward staging 前对 legacy snapshot 做 preflight，并在迁移出的 v3 snapshot 中修复 reserved repository route。

...
`python-overload-declarations-v18` 事实组件修正挂起、match 捕获与延迟直接调用的证据。对同一 HEAD 执行普通索引会重建 v15 或 v16 已完成作用域，无需修改源码或 reset。既有任务租约与检查点仍是恢复依据：切换版本前，使用兼容二进制完成或取消未完成任务。启动阶段不回填调用事实；回滚时须一并保留数据库、WAL 与检查点，重建后重新取得快照选择器。

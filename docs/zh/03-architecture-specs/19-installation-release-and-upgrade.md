...
- Skill 产物包含 `references/knowledge-map-workflows.md`、Draft 2020-12 的 `references/knowledge-map.schema.json`、Draft 2020-12 的 `references/business-glossary.schema.json`，并通过 policy gate 固化 knowledge-map/code-map 联合 bootstrap 与固定 ref 的 spec 开发默认提示词。Metadata gate 会解析两个 schema，保护四类 Knowledge Map v2 artifact branch、Business Glossary v1 的 definition/enum/bound 和未知字段兼容策略，并运行有代表性的有效与无效样例。Release job 必须显式确认两个 schema 都存在于生成的 GitHub Release skill archive；基于目录的 ClawHub 发布会自动包含相同 reference。升级 skill 只更新 agent 指令和结构化工具；只有获得授权的 agent 显式执行文档中的 CLI 工作流后，才会修改仓库 YAML 或 runtime index state。两个 schema 都不能替代 `map validate` 的 runtime、跨文件和语义完整性校验。Knowledge Map schema 不授权直接编辑生成的 shard、archive 或 index node；glossary schema 描述的是 intentionally authored、受版本控制的 source。
...
Schema marker v7 为 `entities` 增加 scoped ontology identity 列，并幂等创建 `business_domains`、`business_terms`、`business_term_aliases`、`business_mappings` 与 `business_knowledge_status`。这些表是从版本控制下 glossary 派生的 projection；升级不改写旧 label-only entity id，它们继续标记为 `untyped`。旧 scope 没有可证明 fresh 的业务投影，必须通过正常 durable `repo index`/`repo update`、同一 lease 和 publication fence 重建后才能重新发布或同树复用。备份必须包含主库、WAL/SHM 与所有 repository shard；binary-only rollback 可忽略新增表，但无法恢复被新版 supersede 或 retention 淘汰的旧投影，精确回滚必须恢复升级前事务一致的整个 runtime state。仓库拥有的 `.knowledge/business-glossary.yaml` 与 map topic shard 不在 runtime database backup 内，应通过 Git 或单独仓库备份恢复。

Software projection schema version 5 是独立的 derived-state compatibility migration，不是 graph-store v4 marker。Schema 初始化会为遗留 `software_global_status` 增加 `projection_schema_version`，把所有低于 5 的状态标为 stale、把记录版本推进到 5，并在没有既有错误时记录 `software global projection schema changed; refresh required`。Version 5 会把同一 repository/source-scope 级 package 坐标的重复 lockfile 证据合并为一个派生 locked component，同时保留 declared component 与全部权威 dependency/SBOM 证据 row。它不会改写权威 code fact，也不能宣称 projection refresh 已完成；受影响 scope 必须经过正常 fenced software-projection workflow 重建后才能恢复 fresh。Upgrade 与 doctor 输出必须区分“schema migration 完成”和“software projection 已 fresh”。

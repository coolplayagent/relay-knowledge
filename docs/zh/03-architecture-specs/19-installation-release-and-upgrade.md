...
Schema marker v7 会在 marker v6 或更早的数据库下次独占打开时，替换 11 个仍使用 `INSERT OR IGNORE` 的旧版 `code_repository_retention_activity_*` trigger。替换与 marker 更新位于同一个 schema-initialization transaction：失败时两者一起回滚，且不会重写已索引事实。停止所有 writer 后，operator 可检查 `relay_storage_schema_state` 已记录版本 7，并确认 `sqlite_master` 中匹配 trigger 的 SQL 不再包含 `INSERT OR IGNORE`；打开失败仍以 `storage_unavailable` 显式暴露。Binary-only rollback 可以保留 v7 trigger 定义，因为其 SQL 兼容且事实数据未改变；但旧 binary 可能把 marker 改回自身版本，因此后续再次前向升级会重跑初始化。精确数据库状态回滚必须恢复升级前 runtime-database backup，新旧 binary 始终不得并发写入。
...
- SQLite graph-store upgrade 能识别 schema marker v7，在不重写已索引事实的前提下原子替换旧版 retention dirty-activity trigger，暴露 marker/trigger 诊断，并把升级前 database backup 保留为精确回滚边界。

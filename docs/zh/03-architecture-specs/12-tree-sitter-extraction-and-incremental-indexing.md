...
显式 `Full` Git 初始化只有在启用历史复用且目标 scope 不 fresh 时，才沿不可变目标 commit 的第一父链回溯最多 10 个祖先。应用层先推导目标计划的有效 filter（包括发现出的 source root），再按近到远查询 commit-to-scope alias，只接受未 stale、未 retiring、与这些可克隆的 canonical path/language filter 精确相等且符合当前 code-fact version 的已发布 scope；命中后把请求固定为真实 `Incremental` task，并在 task mode 与 summary 中暴露 base/head，同时把调用方的 symbolic `requested_ref` 与不可变 execution SHA 分开保留。相同 target 与请求 scope 的 unfinished full 或 incremental task 优先复用。候选 base 到 target 的 diff 若超过初始化历史复用专用的 100 impacted-path 上限（rename 和 copy 的 old/new path 分别计数，并在最终计数前展开命中 filter 的 gitlink 子文件），则不尝试更老祖先，直接回退 checkpointed full index。若 retention 在 preflight 或原子 task admission 固定基线前将所选 scope 标为 retiring，自动复用同样回退 full task；显式 incremental request 仍报告 base 不可用。未显式启用、filesystem source 和未命中历史基线都保持 full 路径。显式 incremental update 继续使用独立的 512 changed-path 上限。祖先枚举、目标 filter 发现、diff 预检和后续解析都必须位于 blocking worker 边界，Git 探测错误不得静默当作“无基线”。

增量 finalization 从有界 changed/deleted path 推导候选 symbol name 与文件集合变化，再按 name 通过索引比较 cardinality、location 和 callable kind/signature 分布。精确 reference 使用 scope/name index；克隆 target ID 指向被替换 base 文件中 symbol 的 reference 使用 scope/target-ID index；带 alias 的 call 与符号依赖 import（包括 TypeScript `export ... from` re-export）先执行有界 FTS 候选查询，再复用最终解析所用的跨语言 call-name 以及 TypeScript/Python/Java import parser。因此无需物化完整 symbol、reference 或 import catalog，也能纳入本地 alias 与变化的导出 symbol 名不同的未变 import，以及 symbol 形状未变但 snapshot identity 已重新生成的未变 reference。Unique affected-path 达到 cap-plus-one 上限或原始 SQL candidate row 达到查询上限时，必须把 finalization 提升到整个 scope，不能发布截断或被去重结果掩盖截断的 candidate set。文件或模块集合变化时同样提升到整个 scope，因为未变化的 side-effect import 可能没有 reference row，而新解析的 import 还会改变下游 reference、call 与 search projection。若过滤后的 delta 没有任何命中的 changed/deleted path，则直接发布克隆 scope，不重新运行 edge finalization。Partial checkpoint 仅在持久 staging identity 匹配同一个 durable task（或同一个 unfenced full/base-to-target session）时续跑；incremental base 不同就先重置 target 并重新 clone，再接受 batch。

常驻 FileWatcher 把 `.git/HEAD`、ref、packed-ref 和 HEAD log event 作为低延迟 hint。启动时及受界 periodic interval（默认 5000 ms）会独立解析 checked-out HEAD/tree，因此 linked worktree 与漏报/合并 event 仍可恢复。HEAD 推进后，将最近发布 clean base 与新 head 固定进 durable task。每个 repository、checked-out ref 与 filter set 使用稳定 fingerprint，在任务槽未完成时合并重复 hint；queue admission 每仓库限制 32 个、全局限制 256 个未完成 task。Attempt-scoped lease 与单调递增的 publication generation 共同构成 single-writer authority；snapshot、batch、workspace 与 software-projection transaction 都在 commit 前校验 live generation，因此 commit event 或 lease 过期后的游离 attempt 不能绕过有界 retry/backoff、recovery 或 dead-letter。full 与预解析 incremental plan 共用 durable begin/batch/finalize checkpoint 生命周期：已完成 batch 可幂等续跑，finalization 会暴露当前 phase，publication 仍受 fence 保护并保持原子性。

...
增量 finalization 从有界 changed/deleted path 推导候选 symbol name 与文件集合变化，再按 name 通过索引比较 cardinality、location 和 callable kind/signature 分布。精确 reference 使用 scope/name index；带 alias 的 call 与符号依赖 import 先执行有界 FTS 候选查询，再复用最终解析所用的跨语言 call-name 以及 TypeScript/Python/Java import parser；因此本地 alias 与变化的导出 symbol 名不同的未变 import 也会纳入，而无需物化完整 symbol、reference 或 import catalog。文件或模块集合变化时必须把所有 edge finalization 提升到整个 scope，因为未变化的 side-effect import 可能没有 reference row，而新解析的 import 还会改变下游 reference、call 与 search projection。若过滤后的 delta 没有任何命中的 changed/deleted path，则直接发布克隆 scope，不重新运行 edge finalization。

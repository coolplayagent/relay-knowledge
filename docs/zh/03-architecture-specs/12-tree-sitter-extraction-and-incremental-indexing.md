...
显式 `Full` Git 初始化在目标 scope 不 fresh 时，先沿不可变目标 commit 的第一父链回溯最多 10 个祖先。应用层按近到远查询 commit-to-scope alias，只接受未 stale、未 retiring、filter 兼容且符合当前 code-fact version 的已发布 scope；命中后把请求固定为真实 `Incremental` task，并在 task mode 与 summary 中暴露 base/head。相同 target 与请求 scope 的 unfinished full 或 incremental task 优先复用。候选 base 到 target 的 diff 若超过 512 changed path，则不尝试更老祖先，直接回退 checkpointed full index；filesystem source 和未命中历史基线也保持 full 路径。祖先枚举、diff 预检和后续解析都必须位于 blocking worker 边界，Git 探测错误不得静默当作“无基线”。

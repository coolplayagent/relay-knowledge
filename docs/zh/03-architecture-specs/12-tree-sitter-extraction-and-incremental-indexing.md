...
增量 finalization 在 symbol cardinality 或 callable kind/signature metadata 变化时，使用与 call resolution 相同的跨语言 call-name candidates，以及与 import finalization 相同的 TypeScript、Python、Java 符号依赖 import parser 扩散 affected path；即使未变 import 的本地 alias 与发生变化的导出 symbol 名不同，也必须纳入。文件或模块集合变化时必须把所有 edge finalization 提升到整个 scope，因为未变化的 side-effect import 可能没有 reference row，而新解析的 import 还会改变下游 reference、call 与 search projection。

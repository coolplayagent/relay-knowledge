...
每个语言注册项包含：language id、file extensions、tree-sitter grammar、capture queries、comment rules、identifier segmentation 和 fallback chunker。缺失 grammar 时，文件仍进入 text chunk 和 BM25 路径。查询时的 `ripgrep` 兜底不是 grammar 替代品；它只能在已索引源码候选上补充精确文本证据。
...
Query-time `ripgrep` 兜底与 Git blob 读取一样必须进入 blocking-worker 边界。它只能物化有界候选 blob，应用 path/language/scope 过滤，并在 timeout、候选文件预算或物化字节预算触发时返回 degraded reason，而不是把查询热路径退化成全仓扫描。

## 7. 降级策略

Parse error、grammar panic、capture mismatch 或 unsupported language 生成 parse status 诊断，并回退到 text chunk。降级结果必须出现在 repo status、health 和 context pack metadata 中。`rg` 缺失或失败属于查询时 exact-text fallback 降级，应出现在 code query 响应 metadata 中，而不是写入索引状态。

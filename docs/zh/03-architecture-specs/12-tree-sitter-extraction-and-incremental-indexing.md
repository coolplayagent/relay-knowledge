...
增量 finalization 在 symbol cardinality 变化时使用与 call resolution 相同的跨语言 call-name candidates 扩散 affected path。文件或模块集合变化时必须对整个 scope 重新执行 import finalization，因为未变化的 side-effect import 可能没有 reference row；其余 edge phase 仍保持 path-scoped，除非 affected-path 阈值要求全量回退。

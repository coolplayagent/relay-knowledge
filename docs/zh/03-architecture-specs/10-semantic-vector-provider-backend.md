...
[中文](../../zh/03-architecture-specs/10-semantic-vector-provider-backend.md) | [英文](../../en/03-architecture-specs/10-semantic-vector-provider-backend.md)
...
- 更新 profile 时，回传的脱敏 secret header 必须保留本地已保存值；`clear_api_key=true` 必须显式清除已保存的 API key，不能依赖 omitted/null 字段的歧义语义。

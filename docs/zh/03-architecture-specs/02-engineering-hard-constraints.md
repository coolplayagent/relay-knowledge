...
CLI adapter 统一收敛在 `interfaces::cli`：`mod.rs` 负责全局 option 解析、dispatch 和稳定公开 CLI surface，`command::diagnostics` 独占 `CliError`、结构化 grammar diagnostic、退出码及 stderr 编码，并直接挂载同级 `diagnostics_tests`；根 facade 只能重导出该错误合同，不得重新实现错误渲染。`spec` 负责 machine-readable command contract，`render` 负责输出序列化，`repo`、`repo_set`、`setup` 负责各自命令族，解析、命名、remote、service、map、version 定向测试放在 `tests`。需要 white-box 访问或兼容性时，命令模块可保留既有逻辑名称，但禁止恢复根目录级 `*_cli` 前缀桶。

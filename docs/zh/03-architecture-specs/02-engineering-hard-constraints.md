...
CLI adapter 统一收敛在 `interfaces::cli`：`mod.rs` 负责 dispatch、执行编排和稳定公开 CLI surface；`command::parse` 独占 global option/token 处理、help/version 短路及命令族解析分发，`command::values` 独占 flag 后值提取与 freshness policy 校验，两者分别直接挂载 `parse_tests` 与 `values_tests`，依赖必须保持 `command::parse -> 各命令族 -> command::values`。`command::diagnostics` 独占 `CliError`、结构化 grammar diagnostic、退出码及 stderr 编码，并直接挂载同级 `diagnostics_tests`；根 facade 只能重导出这些命令合同，不得重新实现解析或错误渲染。`spec` 负责 machine-readable command contract，`render` 负责输出序列化，`repo`、`repo_set`、`setup` 负责各自命令族，兼容性级命名、remote、service、map、version 定向测试放在 `tests`。需要 white-box 访问或兼容性时，命令模块可保留既有逻辑名称，但禁止恢复根目录级 `*_cli` 前缀桶。

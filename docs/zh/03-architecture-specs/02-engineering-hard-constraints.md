...
Web adapter 统一收敛在 `interfaces::web`：`mod.rs` 负责 router composition 和共享 response/error 边界，`operation_request` 独占 Web operation payload 的字段/类型/范围校验、枚举解析与 domain/API request 构造，`code_api`、`code_index_request`、`code_view_request`、`files`、`model_config` 负责各自命名的 HTTP contract。Payload validation 与 request mapping 断言必须由 `operation_request_tests` 直接维护，不得回流到 facade `mod_tests`；`code_api_integration_tests` 与 `files_integration_tests` 会运行装配后的 router 和共享 application service，因此由 facade 显式挂载。不得恢复根目录级 `web_*` 兄弟文件，也不得从该 adapter 打开 socket。

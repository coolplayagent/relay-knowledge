...
仓库级 retention 是复用上述物理删除机制的第二层策略。`RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES` 默认值为 10，统计具有当前已发布索引、且不属于用户管理 repository set 的仓库；系统特意忽略自动生成的 automatic-workspace set，不把它作为豁免。成功发布后和每次有界 retention maintenance 前都会执行调度。超过上限时，成功发布时间最旧的合格仓库会获得一个持久化父任务，其中记录 initial scope 和 publication cutoff。父任务可跨重启恢复，并在 retention status 暴露 phase 与 last error，同时协调全部子 scope-GC job。它不删除仓库注册或 alias，不阻止新索引，也不取消 queued/running work；未完成 task 的 target/base 和 cutoff 时刻及之后的成功发布均受保护。仓库后来加入用户管理 set 时，不再退役更多 scope，但已经标为 `retiring` 的 scope 会完成子 GC。Single-SQLite 在没有子任务或可退役 scope 后完成父任务；partitioned SQLite 则等待 control 与 shard 状态都收敛后完成。

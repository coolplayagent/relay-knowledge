...
Software-global projection 遵守同一 writer-quantum 原则。旧 `finalizing:software_projection` checkpoint 兼容解码为 reset，随后持久推进 `finalizing:software_projection:v1:{reset|dependencies|sdk_usages|lifecycle|files|topics|relationships|publish}`。每个 phase 最多提交一次确定性派生边界，phase rows 和 next token 位于同一事务；事务开始与提交前都验证 source scope、repository 和 live publication fence。阶段之间释放 SQLite writer，并让外层 lease heartbeat 获得确定续租窗口；恢复从最后提交 token 继续，不重启 parser 或已提交 phase。中间投影保持 `stale=1` 且 target scope 不可查询，最终 publish phase 才原子更新 business mapping、software status、scope publication、terminal checkpoint 和本地 receipt。Single SQLite 与 partitioned shard 共用该协议，后者仍在 shard 终态后通过 control fence 完成 catalog handoff。

...
Software-global projection 遵守同一 writer-quantum 原则。旧 `finalizing:software_projection` checkpoint 兼容解码为 reset，随后持久推进 `finalizing:software_projection:v3:{reset|dependencies|sdk_usages|lifecycle|files|topics|relationships|ontology|publish}`。每个 phase 最多提交一次确定性派生边界，phase rows 和 next token 位于同一事务；事务开始与提交前都验证 source scope、repository 和 live publication fence。阶段之间释放 SQLite writer，并让外层 lease heartbeat 获得确定续租窗口；当前 v3 恢复从最后提交 token 继续，不重启 parser 或已提交 phase。旧 v1/v2 的非终态 token 则解码为 reset，避免升级前的 checkpoint 跳过新增 reactor 派生；现有 fenced 事务原子地把旧 token 推进到下一个 v3 token，源码事实、task lease、fact proof 和 receipt 仍由原有边界管理。中间投影保持 `stale=1` 且 target scope 不可查询，最终 publish phase 才原子更新 business mapping、software status、scope publication、terminal checkpoint 和本地 receipt。Single SQLite 与 partitioned shard 共用该协议，后者仍在 shard 终态后通过 control fence 完成 catalog handoff。

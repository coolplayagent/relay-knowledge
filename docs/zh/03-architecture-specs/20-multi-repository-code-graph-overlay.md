...
- `repo-set refresh` 构建 import/module 层面的跨仓 overlay edges，支持 resolved 和 ambiguous 状态，并暴露 evidence JSON。没有 member export candidate 的外部 import 继续作为来源仓库内的权威 unresolved metadata，不会重复物化进 set overlay。本地、相对或已在成员仓库内解析的 import 同样只留在成员仓库内，不会通过跨仓 symbol-name 或 basename fallback 解析。
...
当前实现只把 import/module 层面的明确证据解析为跨仓边。没有 package metadata 或唯一 module match 时，边保持 `ambiguous` 或 `unresolved`。调用和 reference 的跨仓解析属于未来可选的语言级增强，不是当前多仓基础查询的隐含能力。

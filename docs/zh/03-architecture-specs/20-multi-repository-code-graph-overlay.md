...
当跨仓库边为 `ambiguous`、`unresolved` 或 `inferred` 时，结果必须暴露 resolution state 和 confidence，不得把它提升为单仓 `resolved` 调用或引用。
...
导航: 上一章: [19. 安装、发布与升级](19-installation-release-and-upgrade.md)

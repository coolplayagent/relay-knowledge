...
## 20260518T202013Z
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches/20260518T202013Z.patch`
- score: 0.919417 (foundational=0.969136, competitive=0.844246, accuracy=0.906691, semantic_vector=1.0, research_judge=0.87, performance=0.86495, stability=1.0)
- cases: 86/87 passed
- changed paths: `docs/zh/05-benchmarks/04-self-iteration-accepted-optimizations.md`, `src/relay_knowledge/storage/sqlite/code_query.rs`, `src/relay_knowledge/storage/sqlite/code_query_import_ranking_tests.rs`, `src/relay_knowledge/storage/sqlite/code_query_import_scoring.rs`, `src/relay_knowledge/storage/sqlite/code_query_import_targets.rs`, `src/relay_knowledge/storage/sqlite/code_query_rows.rs`
- key improvements: score_component:score 0.910797->0.919417; score_component:competitive_capability 0.832341->0.844246; score_component:accuracy 0.900739->0.906691; score_component:research_judge 0.84->0.87; metric:cargo_build_release_ms 35180.0->32792; metric:cargo_fmt_check_ms 802.0->623; metric:cargo_clippy_ms 183.0->142; metric:cargo_test_ms 8690.0->5514
- known degradations: metric:relay_teams_index_ms 32184.0->33663; metric:relay_teams_register_index_ms 32273.0->33754; metric:relay_teams_query_p50_ms 213.5->292.0; metric:leveldb_cpp_index_ms 1568.0->1631; metric:leveldb_cpp_register_index_ms 1766.0->1830; metric:semantic_vector_refresh_ms 377.0->426; case:leveldb_hybrid_recovery_manifest_full_scope {'passed': True, 'rank': 1, 'false_positive_count': 0, 'score': 1.0}->{'passed': True, 'rank': 2, 'false_positive_count': 0, 'score': 0.5} rank_worsened; case:leveldb_cpp_dependency_filter_policy_header_challenge {'passed': False, 'rank': 3, 'false_positive_count': 0, 'score': 0.416667}->{'passed': False, 'rank': 3, 'false_positive_count': 0, 'score': 0.0} score_worsened
- latency metrics: cargo_build_release_ms=32792ms; cargo_fmt_check_ms=623ms; cargo_clippy_ms=142ms; cargo_test_ms=5514ms; relay_teams_index_ms=33663ms; relay_teams_register_index_ms=33754ms; relay_teams_query_p50_ms=292ms; relay_teams_query_p95_ms=646ms
Adopted optimization notes:
prepare(&sql)?; +    let rows = statement.query_map(params_from_iter(values), |row| { +        Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)) +    })?; +    rows.collect::<Result<Vec<_>, _>>() +        .map_err(StorageError::from) +} + pub(super) fn search_imports_by_target_symbols( connection: &Connection, status: &CodeRepositoryStatus, @@ -104,6 +175,7 @@ language_id: row.get(2)?, module: row.get(3)?, matched_symbol_name, +            target_symbol_names: None, line_range: RepositoryCodeRange { start: row.get(4)?, end: row.get(5)?, diff --git a/src/relay_knowledge/storage/sqlite/code_query_rows.rs b/src/relay_knowledge/storage/sqlite/code_query_rows.rs index 293d5c0f7414f8519de29b5e88c1f03975e5e481..6d3c5d6a9aefaa601ef2f24b0ad87981577582dd --- a/src/relay_knowledge/storage/sqlite/code_query_rows.rs +++ b/src/relay_knowledge/storage/sqlite/code_query_rows.rs @@ -57,6 +57,7 @@ pub(super) language_id: String, pub(super) module: String, pub(super) matched_symbol_name: Option<String>, +    pub(super) target_symbol_names: Option<String>, pub(super) line_range: RepositoryCodeRange, pub(super) target_hint: Option<String>, pub(super) resolution_state: String, tokens used 221,498
## 20260518T213007Z
...
Adopted optimization notes:
.cecb247b5764d136a6815418bc9d1bcfe32d583c --- a/src/relay_knowledge/code/parser_c_tests.rs +++ b/src/relay_knowledge/code/parser_c_tests.rs @@ -141,6 +141,39 @@ } #[test] +fn c_header_decorated_cpp_class_uses_real_type_name() { +    let snapshot = parse_source_snapshot( +        "include/leveldb/filter_policy.h", +        br#" +namespace leveldb { + +class LEVELDB_EXPORT FilterPolicy { + public: +  virtual bool KeyMayMatch() const = 0; +}; + +} +"#, +    ); +    let file = snapshot +        .files +        .iter() +        .find(|file| file.path == "include/leveldb/filter_policy.h") +        .expect("header file should be indexed"); + +    assert_eq!(file.language_id, "c"); +    assert!(snapshot.symbols.iter().any(|symbol| { +        symbol.name == "FilterPolicy" && symbol.kind == "class" +    })); +    assert!( +        !snapshot.symbols.iter().any(|symbol| { +            symbol.name == "LEVELDB_EXPORT" && symbol.kind == "function" +        }), +        "export macros should not replace the real C++ class name" +    ); +} + +#[test] fn c_function_pointer_declarations_are_not_function_symbols() { let snapshot = parse_source_snapshot( "include/linux/callbacks.h", tokens used 352,840
## 20260518T214222Z
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches/20260518T214222Z.patch`
- score: 0.923286 (foundational=0.969136, competitive=0.856151, accuracy=0.912643, semantic_vector=1.0, research_judge=0.88, performance=0.86258, stability=1.0)
- cases: 87/87 passed
...
- key improvements: score_component:score 0.916663->0.923286; score_component:research_judge 0.85->0.88; metric:relay_teams_index_ms 33836.0->30289; metric:relay_teams_register_index_ms 33933.0->30378; metric:leveldb_cpp_index_ms 1610.0->1554; metric:semantic_vector_provider_probe_ms 1254.0->1206; case:research_judge {'passed': True, 'rank': 1, 'false_positive_count': 0, 'score': 0.85}->{'passed': True, 'rank': 1, 'false_positive_count': 0, 'score': 0.88} score_improved
- known degradations: metric:cargo_build_release_ms 37047.0->39005; metric:cargo_fmt_check_ms 784.0->812; metric:opencode_typescript_index_ms 22285.0->24729; metric:opencode_typescript_register_index_ms 22506.0->24945; metric:local_documents_file_index_ms 419.0->462; metric:local_background_auto_index_file_index_ms 413.0->443; metric:local_background_auto_index_files_background_service_auto_indexes_new_document_file_auto_index_first_seen_ms 1222.0->1301.0
...
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779242800.patch`; score 0.800278; cases 13/13 passed; changed paths: repository-set query/service docs.
- summary: accepted indexed overlay evidence lookup and repo-set service split, improving performance while recording relay-teams and Temporal index/query degradations; full raw metrics remain in the report artifact.

## run-1779244212

- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779244212.patch`; score 0.801946; cases 13/13 passed; changed paths: repository-set query docs.
- summary: accepted origin-file overlay evidence for non-import hits, improving Temporal repo-set client.Dial rank and competitive capability while recording relay-teams and LevelDB latency degradations; full raw metrics remain in the report artifact.

## run-1779245204
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779245204.patch`; score 0.819451; cases 13/13 passed.
- summary: accepted repository-set bridge support and caller named-slot ranking restored LevelDB caller rank and competitive capability, with full metric details retained in the report artifact and the strategy summary above.
...
## run-1779254116
- compacted: accepted hybrid chunk anchor FTS candidate, score 0.830563, full patch/report retained at `.git/relay-knowledge-self-iteration/patches-v2/run-1779254116.patch` and report artifacts.

## run-1779256273
- compacted: accepted symbol identity fast path candidate, score 0.813180, full patch/report retained at `.git/relay-knowledge-self-iteration/patches-v2/run-1779256273.patch` and report artifacts.

## run-1779257829

- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779257829.patch`
- score: 0.846219 (foundational=1.000000, competitive=1.000000, accuracy=1.000000, semantic_vector=0.000000, research_judge=n/a, performance=0.867882, stability=1.000000)
- cases: 13/13 passed
- changed paths: `docs/zh/05-benchmarks/04-self-iteration-accepted-optimizations.md`, `src/relay_knowledge/storage/sqlite/code_schema.rs`
- key improvements: score_component:score 0.81318->0.8462188144971885; score_component:performance 0.684332->0.8678823027621585; metric:temporal_sdk_go_index_ms 2764.0->1873.0; metric:temporal_sdk_go_register_index_ms 4600.0->4392.0; metric:temporal_samples_go_index_ms 1958.0->967.0; metric:temporal_samples_go_register_index_ms 8707.0->1793.0; metric:leveldb_cpp_index_ms 1897.0->866.0; metric:leveldb_cpp_register_index_ms 5462.0->1710.0
- known degradations: metric:cargo_fmt_check_ms 2021.0->2196.0; metric:cargo_build_debug_ms 363.0->403.0
- latency metrics: cargo_fmt_check_ms=2196ms; self_iteration_cargo_fmt_check_ms=323ms; cargo_build_debug_ms=403ms; self_iteration_cargo_check_ms=141ms; temporal_sdk_go_index_ms=1873ms; temporal_sdk_go_register_index_ms=4392ms; temporal_samples_go_index_ms=967ms; temporal_samples_go_register_index_ms=1793ms
...
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779262414.patch`
- score: 0.839604 (foundational=1.000000, competitive=1.000000, accuracy=1.000000, semantic_vector=0.000000, research_judge=n/a, performance=0.831131, stability=1.000000)
- cases: 13/13 passed
- changed paths: `docs/zh/05-benchmarks/04-self-iteration-accepted-optimizations.md`, `src/relay_knowledge/application/code_repository_set_query.rs`, `src/relay_knowledge/application/code_repository_set_service.rs`, `src/relay_knowledge/storage/sqlite/code_query.rs`, `src/relay_knowledge/storage/sqlite/code_query_import_targets.rs`, `src/relay_knowledge/storage/sqlite/code_query_prepare.rs`, `src/relay_knowledge/storage/sqlite/code_query_symbols.rs`
...
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779266107.patch`
- score: 0.843861 (foundational=1.000000, competitive=1.000000, accuracy=1.000000, semantic_vector=0.000000, research_judge=n/a, performance=0.854785, stability=1.000000)
- cases: 13/13 passed
- changed paths: `docs/zh/05-benchmarks/04-self-iteration-accepted-optimizations.md`, `src/relay_knowledge/storage/sqlite/code_query.rs`, `src/relay_knowledge/storage/sqlite/code_query_call_ranking_tests.rs`, `src/relay_knowledge/storage/sqlite/code_query_calls.rs`, `src/relay_knowledge/storage/sqlite/code_query_prepare.rs`
- key improvements: score_component:performance 0.832182->0.8547854489307346; metric:cargo_build_debug_ms 505.0->303.0; metric:relay_teams_index_ms 2858.0->2096.0; metric:relay_teams_register_index_ms 3665.0->2923.0; metric:relay_teams_query_p50_ms 1106.0->1051.0; metric:temporal_samples_go_index_ms 947.0->868.0; metric:temporal_samples_go_register_index_ms 3747.0->1656.0; metric:leveldb_cpp_index_ms 867.0->826.0
- known degradations: metric:temporal_sdk_go_index_ms 1853.0->1912.0
- latency metrics: cargo_fmt_check_ms=2117ms; self_iteration_cargo_fmt_check_ms=343ms; cargo_build_debug_ms=303ms; self_iteration_cargo_check_ms=121ms; relay_teams_index_ms=2096ms; relay_teams_register_index_ms=2923ms; relay_teams_query_p50_ms=1051ms; relay_teams_query_p95_ms=1614ms
...
- algorithm: 聚焦运行按 `foundational`、`competitive`、`semantic_vector`、`file_fixtures`、`repository_sets`、`research_judge`、`performance` 或 `all` 选择主攻 workload；`performance` 聚焦保留会产生性能指标的 repository、repo-set、semantic/vector 和 file-fixture workload；无论聚焦哪个类别，底线 `guardrail=true` case 都会转成 `guardrail_case_*` gate 拒绝候选。
- architecture: 类别解析集中在 self-iteration config，workload 选择集中在 evaluator selection helper；run history 额外记录 `category_focus`/`selected_categories` 并按 profile + category focus 查找对比基线；`self-iterate.sh` 继续透传参数，case 文件只用声明式 `guardrail` 字段，不需要脚本侧维护硬编码 case id。
- invariants: 未传 `--categories` 时仍使用 profile 默认语义；fast 默认扩展到每仓 8 条普通 case、2 条 repo-set case，并增加 1 条 semantic/vector guardrail；聚焦模式会跳过没有选中 query case 且不是所选 repo-set 成员的仓库；full/exhaustive 默认仍运行完整慢 suite；smoke 仍只做 launcher/format 类轻量验证。
...
- risks: 同一 profile 下不同 category focus 的历史分数不再共享采纳比较基线；已有旧历史缺少 `category_focus` 字段时只会继续作为默认 workload 基线，不会污染新的聚焦运行。

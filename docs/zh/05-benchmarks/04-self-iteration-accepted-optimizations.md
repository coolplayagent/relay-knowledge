...
- summary: accepted hybrid API identity symbol coverage; score 0.926903 with protected floors foundational=0.947917, competitive=0.875000, semantic_vector=1.0, stability=1.0. Full metrics remain in `.git/relay-knowledge-self-iteration/patches-v2/run-1779373568.patch` and the matching report.

## candidate run-1779375550-query-proximity-hybrid-ranking
- 算法/架构：Hybrid chunk ranking adds a bounded query-proximity signal after the existing FTS candidate window and base scorer. For long multi-term code queries, it scans six-line windows inside each recalled chunk and gives a capped bonus only when one local window covers most normalized query terms, so tightly related declarations or implementation context outrank broad class/file chunks whose matches are scattered.
- 不变量：不改变 parser facts、SQLite schema、FTS MATCH planning、candidate limits、symbol/reference/call/import retrieval、repo-set overlay, semantic/vector read model, env/paths/net, CLI/API, installation behavior, or the self-iteration harness. The signal is Hybrid-only, requires a positive high base score, at least five meaningful query terms, at most 80 nonblank chunk lines, path/language filters, non-test guards unless the query asks for tests, and it does not stack onto queries already shaped as three-or-more API identity sequences; final dedupe/top-k still applies.
- 预期影响：Expected to improve multi-repository and large-codebase retrieval for queries that combine API names, data-flow nouns, and configuration terms, especially when the best evidence sits in a short declaration or body window while larger neighbors match the same words only by scattering them through a long chunk. This directly builds on accepted hybrid chunk ranking from `run-1779371550` and API-identity coverage from `run-1779373568`.
- 风险：Localized windows can lift a nearby declaration above a broader architectural surface for some exploratory Hybrid queries. The risk is bounded by the 75%/five-term coverage gate, six-line window, 80-line chunk cap, 1.9 score cap, existing base-score requirement, test-path guard, unchanged recall window, and no fixture/query/path enumeration; it avoids the rejected `run-1779372563` pattern of narrow path demotion that improved local metrics while regressing overall query latency.

## run-1779375550 compacted
- summary: accepted query-proximity hybrid ranking; score 0.933757 with protected floors foundational=0.947917, competitive=0.894231, semantic_vector=1.0, stability=1.0. Full metrics remain in `.git/relay-knowledge-self-iteration/patches-v2/run-1779375550.patch` and the matching report.
...
## candidate run-1779380191-api-identity-facet-weighting
- 算法/架构：Hybrid multi-identity symbol retrieval keeps the accepted direct API identity lookup from `run-1779373568`, but changes its bonus from a small coverage nudge into a bounded facet-weighting signal. Each explicit API-shaped facet, such as scoped `worker.New` or CamelCase `InterruptCh`, receives a base identity weight plus a capped sequence-facet bonus; simple facets also receive a same-owner bonus only when another scoped identity in the query establishes that API owner through qualified/canonical symbol identity fields, so later public facets can survive the per-repository top-k merge instead of being hidden by repeated broad usage chunks.
- 不变量：不改变 parser facts、SQLite schema、FTS document content、candidate limits、SQL fanout、chunk/call/reference/import retrieval、repo-set overlay、semantic/vector read model、env/paths/net、CLI/API、安装发布或 self-iteration harness；仍为 Hybrid-only、至少两个 API-shaped identities、最多 6 个 identities、每 identity 仍走既有 direct indexed lookup，并继续受 path/language filters、positive score、dedupe/top-k 和 exact symbol-name/scoped-owner checks 约束。
- 预期影响：预期改善多仓 workspace 查询的 API facet 覆盖，尤其是 Temporal worker/client 这类同时要求 usage flow 与 SDK/core definitions 的查询，让 `New`、`RegisterWorkflow`、`RegisterActivity`、`InterruptCh` 等显式命名 API 都有机会进入合并候选；不增加索引或查询候选窗口，因而避免最近拒绝中 LevelDB/C 语法 query p95 和 register/index 耗时回退的路径。
- 风险：显式 API symbol/definition 可能在部分 exploratory Hybrid 查询中上移并替换一条泛化上下文 chunk；风险由 multi-identity gate、bounded facet/owner caps、owner-term match、unchanged recall window、unchanged single-identity Definition/Symbol behavior、test coverage 和 fast profile protected floors 控制。

## run-1779380191 compacted
- summary: accepted API identity facet weighting; score 0.938610 with protected floors foundational=0.947917, competitive=0.932692, semantic_vector=1.0, stability=1.0. Full metrics remain in `.git/relay-knowledge-self-iteration/patches-v2/run-1779380191.patch`, the matching report, and progressive memory.
...
- patch: `/opt/workspace/relay-knowledge-refactor/.git/relay-knowledge-self-iteration/patches-v2/run-1779387282.patch`
- score: 0.960773 (foundational=1.000000, competitive=0.975962, accuracy=0.987981, semantic_vector=1.000000, research_judge=n/a, performance=0.811452, stability=1.000000)
- cases: 43/43 passed
- changed paths: `docs/zh/05-benchmarks/04-self-iteration-accepted-optimizations.md`, `src/relay_knowledge/storage/sqlite/code_query_call_ranking_tests.rs`, `src/relay_knowledge/storage/sqlite/code_query_calls.rs`, `src/relay_knowledge/storage/sqlite/code_query_excerpt_tests.rs`, `src/relay_knowledge/storage/sqlite/code_query_excerpts.rs`
- key improvements: score_component:competitive_capability 0.951923->0.9759615384615384; case_score:c_syntax_callees_dispatch_sequence 0.625->1.0; case_rank:c_syntax_callees_dispatch_sequence 4->1; case_score:cpp_syntax_callees_run_pipeline_lambda_sequence 0.75->1.0; case_rank:cpp_syntax_callees_run_pipeline_lambda_sequence 2->1; metric:leveldb_cpp_index_ms 1228.0->606.0; metric:leveldb_cpp_register_index_ms 1793.0->1152.0; metric:leveldb_cpp_query_p50_ms 1169.0->1128.0
- known degradations: metric:cargo_fmt_check_ms 1493.0->1572.0; metric:cargo_build_debug_ms 304.0->7282.0; metric:temporal_sdk_go_index_ms 930.0->1151.0; metric:temporal_sdk_go_register_index_ms 1580.0->1817.0; metric:temporal_samples_go_index_ms 587.0->626.0; metric:c_syntax_fixture_register_index_ms 1732.0->1851.0; metric:relay_teams_register_index_ms 1716.0->1778.0; metric:cpp_syntax_fixture_register_index_ms 1808.0->2216.0
- latency metrics: cargo_fmt_check_ms=1572ms; self_iteration_cargo_fmt_check_ms=283ms; cargo_build_debug_ms=7282ms; self_iteration_cargo_check_ms=122ms; leveldb_cpp_index_ms=606ms; leveldb_cpp_register_index_ms=1152ms; leveldb_cpp_query_p50_ms=1128ms; leveldb_cpp_query_p95_ms=1290ms

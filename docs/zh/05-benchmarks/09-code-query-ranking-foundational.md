...
- Imports: 无测试意图的 import 查询会更明确地下调测试路径命中；extensionless relative module specifier（例如 `./redaction`）会更重视靠前的直接 import site；bare import syntax（例如 `import "./protocol"`）会优先匹配 dynamic/side-effect import expression，而不是静态 named/type import 声明；imports 查询会基于导入绑定在同文件内的实际使用情况补充排序信号。

...
- Hybrid chunks and repo-set: test/benchmark 与 generated chunk 降权保留正分，避免唯一候选被过滤；type-surface companion phrase 不参与 compound identifier 输入；repository-set domain-affinity bonus 只在 fresh 且有 overlay evidence 支持 priority 时生效。

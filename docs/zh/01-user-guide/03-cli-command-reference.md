...
需要从本地 CLI 访问已部署常驻服务时，使用全局 `--remote <base-url>` 或 `RELAY_KNOWLEDGE_REMOTE_BASE_URL`。远端模式当前只覆盖 `repo index`、`repo scope preview`、`repo status` 和 `repo query`，用于访问服务端已经注册的仓库；其他命令不会隐式写本机 runtime storage。

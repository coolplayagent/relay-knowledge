...
MCP 请求通过 `admit_queued_request` 进入，Web/HTTP 路由通过 `admit_connection` + `admit_request` 进入。已校验 session 的 MCP `notifications/cancelled` 使用协议层优先路径，避免普通请求预算满载时无法取消活跃工具调用。

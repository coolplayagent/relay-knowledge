...
Web 工作区使用左侧导航和右侧详情区。桌面端左侧导航固定在视口内，右侧详情区独立滚动；窄屏端导航固定为顶部横向菜单，详情区占满剩余空间。点击 Status、Readiness、Graph、Providers、Operations、Indexes 或 Runtime 时，详情区只显示对应页面，不把所有面板堆在同一个长页面内。
...
测试覆盖 diagnostics、Status 首页查询入口、Status graph overview 点线与字体比例、单详情页导航、主题切换、GraphRAG readiness、graph canvas tabs/selection/zoom controls、operation composer、index table、runtime panel 和移动端布局。

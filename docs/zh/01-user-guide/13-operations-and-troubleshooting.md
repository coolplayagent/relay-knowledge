...
`repo status` 显示 `active_task.state=running`，但 `checkpoint.parsed_file_count` 一直是 0: 先查看 `repo status --format json` 中的 `active_task.lease_expires_at_ms` 和 `checkpoint.updated_at_ms`。非交互式 agent session 中不要反复启动 `service run`；它是前台常驻进程。如果任务处于 queued 或 retrying，可执行 `repo index-worker --task-id <active_task.task_id> --format json` 做一次有界 worker attempt。如果之前的 service 进程已经 claim lease 后退出，应等待 lease recovery 后再重试；不要杀 `relay-knowledge` 进程，也不要绕过任务 lease。

...
`source_scope '<scope>' is not authorized for this MCP policy`: 请求 scope 不在 `RELAY_KNOWLEDGE_MCP_ALLOWED_SCOPES` 中，也不是当前运行时已注册的 code repository alias。运行中可先执行 `relay-knowledge repo register <path> --alias <scope>` 并完成需要的索引；否则按错误提示追加 `RELAY_KNOWLEDGE_MCP_ALLOWED_SCOPES=<scope>` 后重启服务。
...
`repo query` 返回 `ripgrep unavailable`、`ripgrep timeout` 或 `ripgrep ... budget exhausted`: 这表示精确文本兜底层降级，不表示 tree-sitter 代码图或 SQLite FTS 结果不可用。先确认本机可执行 `rg --version`；如果是 timeout 或预算耗尽，优先收窄 `--path`、`--language`、`--ref` 或注册 scope，再用 `repo status --format json` 确认目标快照 fresh。

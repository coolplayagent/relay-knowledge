...
`expected_degraded_files` 返回按快照遍历顺序发现的前 50 个不同降级文件，每项包含 `path` 和 `reason`；原因直接来自完整索引使用的解析器诊断，包括语法错误、无效 UTF-8、二进制内容、不支持的 grammar 和超大文件。仅缺失外部依赖源码不算降级。发现第 51 个降级文件后，`expected_degraded_files_truncated` 为 `true`，预览停止请求后续解析批次；当前有界批次可能已经解析更多文件。恰好发现 50 个时仍继续检查，只有实际发现更多文件才标记截断；未截断的成功结果覆盖全部选中文件。截断列表不提供总数，也不能证明剩余文件已经检查。

`excluded_paths` 同样最多展示前 50 项及排除原因；实际发现第 51 条时，`excluded_paths_truncated` 为 `true`。这只截断展示，不停止范围枚举，选中文件数、字节数、语言分布等统计仍完整。两个数组及其布尔标记始终返回；无结果时为 `[]` 和 `false`，两个标记互相独立。text/Markdown 输出也展示路径、原因及截断提示；JSON 和 streaming JSON 使用相同结构。

预览读取并解析选中源码，但不写入索引事实、任务或 checkpoint。应用最多允许两个预览 worker 并发，准入等待最多五秒，响应期限为 120 秒。取消、超时或已观察到的源码读取失败仍返回错误，不返回部分成功；当前 blocking 批次结束后才释放许可并停止后续工作。范围枚举成本仍然存在，只有发现超过 50 个降级文件时才能节省后续解析成本。正式索引始终执行完整流程，不受预览列表限制影响。比较诊断时必须使用相同 resolved ref 和 scope；增量摘要和 worktree overlay 可能覆盖不同文件集合。

接口迁移：`expected_degraded_file_count` 已移除且没有兼容别名。调用方改读 `expected_degraded_files` 和截断标记，不能把截断数组长度当作降级总数。远端 CLI 与服务端需同步升级或回滚；没有持久化数据迁移。

...
Python 来源变化重解析在增量、文件系统和 worktree overlay 索引中使用共享语言分类器，包含 `.pyw`。Overlay 来源重解析与已变更文件共享既有总字节预算。类体模块导入可能执行任意模块代码，仅已证明标准 typing 来源的导入保留 overload 证明。控制流中可能建立的别名及前序装饰器表达式副作用纳入有界证明，装饰器表达式按源码顺序求值。v12 通过普通索引重建旧 scope，保持预算失败及降级诊断可见。

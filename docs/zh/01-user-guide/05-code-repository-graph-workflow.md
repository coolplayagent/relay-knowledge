...
对 `--ref worktree`，只要父仓 gitlink 已 staged，已提交的 submodule 更新就会进入 overlay；submodule 内未暂存的脏内容仍会被忽略。
...
overlay 绑定当前 checked-out `HEAD`，使用合成 snapshot 标识，包含已修改文件、未跟踪文件和 staged submodule gitlink 更新。overlay 活跃时，clean commit ref 查询会被拒绝，避免把未提交内容误标成 clean Git snapshot。

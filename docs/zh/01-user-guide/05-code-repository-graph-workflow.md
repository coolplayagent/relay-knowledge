...
Python 重载证明在共享目标预算内检查链式赋值的每个写入目标，不把普通右侧或独立布尔读取当成写入。装饰器及其模块接收者的透明括号（包括注释）保留原绑定身份；自定义或已重绑定的装饰器仍视为运行时定义。

静态导入的继承证明不会假定未解析的 `Object` 父类属于 `java.lang`：缺少包级类型证据时，需要全限定名称或显式平台类型导入。父类型的私有方法和接口的静态方法不会遮蔽导入调用，本类型声明的方法及真正继承的方法仍保留正常遮蔽行为。Java 默认字符串遵循 [Java 词法转换](https://docs.oracle.com/javase/specs/jls/se21/html/jls-3.html#jls-3.3)：先转换符合条件的 Unicode 转义，再解码标准和八进制字符串转义，区分字面量反斜杠与 Unicode 转义。文本块、无效字符串、孤立 UTF-16 代理项及超过 64 KiB 的源码字符串保留未知默认值，不持久化不准确的值。

Java 配置键字面量与已引用的字符串常量也使用相同的有界运行时字符串解码：Unicode 或八进制转义的键与配置文件中的实际键关联，双反斜杠保留为字面反斜杠，不进行二次 Unicode 解码；仅字符串值相等的未引用常量不构成配置声明。

`.ctmpl` 的 `key`、`keyOrDefault` 与 `env` 读取按完整模板 action 识别，允许函数和字面量参数跨行；保留原始 UTF-8 字节范围、起止行及静态默认值。扫描单向前进，模板注释及引号/原始字符串内的 `}}` 不终止 action；每个 action 的字面量解码上限为 65,536 字节，超限显式报告提取错误，不返回截断事实。字符串字面值遵循 Go 转义及原始字符串去除回车的规则；字节转义若产生无效 UTF-8，默认值保持未知，不进行有损替换。
...
配置 `--query` 在 SQL 候选选择与最终分组匹配中共用 Unicode 字母、数字及下划线的词边界，中文路径或片段查询词在小结果上限和一致性分析中也会保留。ASCII 匹配仍不区分大小写；非 ASCII 字符保留原有字形与大小写。显式查询若没有可搜索词，会返回 `error_kind=invalid_argument`（HTTP 400），不再列出无关配置或误报存储故障。此处使用专用查询校验错误；仓库分片缺失等既有后端故障保留存储故障分类（HTTP 503），查询预算耗尽仍为超时（HTTP 408）。不传 `--query` 仍列出选定授权 scope。别名分组、元数据条件、候选上限和共享 SQL 预算保持不变；此查询修正本身无需 schema 迁移或事实重建。
...
Go 模板恢复支持语言规定的 `range` 双变量声明头，并保留既有单变量形式。其他控制管道的双变量声明、三变量 range 及未证明的变量表达式仍保留部分解析诊断；恢复不求值变量，也不扩大工作预算。

配置显示名规范化保留混合文字键边界的 Unicode 字母和数字。例如 `功能.flag` 显示为 `功能_flag`；源键和身份不变，保留既有分隔符替换及 ASCII 大小写规范化。常量和 getter 绑定身份与 namespace 证据共享有界的结构化 Java 包名，因此合法的间隔块注释或行注释不改变跨文件绑定身份，原始源码范围保持完整。

配置查询候选筛选与最终匹配使用相同的文本字段：名称、来源类别和键、边类别、路径、摘要、解码后的绑定名称及引用符号。domain、默认值等其他元数据不会占用排序候选窗口，除非相同文本也出现在可检索字段中。显式元数据过滤仍要求同一个 usage 满足，多词别名组装、候选数量上限及共享 SQLite 执行预算保持有效。

Java 隐式平台证据使用已支持的静态源码根模型。标准 Java main/test 源码根记录所属模块前缀：main 只使用本模块 main 源集的提供者，test 使用本模块 main 与 test 源集；独立模块不会隐式共享提供者。不属于已识别源集布局的文件保留已文档化的仓库根模型。歧义布局及缺少旧源集证据时保持 unknown；这不是 Maven/Gradle 依赖或 classpath 解析。请求路径过滤只缩小返回 usage，不能隐藏对消费者可见的提供者。

源集身份根据授权源码路径确定，与类型解析是否完整分开记录。已知无关模块或仅 test 源集中的 Java 文件即使解析不完整，也不会破坏 main 源码的提供者缺失证明；可见源集中的不完整证据及未知源集身份仍采取保守判断。源集类别和模块文本计入既有投影行数、重复字节及 durable copy 预算。
...
读取独立导入的标准 typing 模块属性时，若接收者具备自身有界且未变的导入证明，可保留另一个独立导入的 overload 函数证明。本地提供方 origin、中间未知执行、接收者重新绑定和协议钩子写入都会终止该正证明。

单个同步上下文管理器也可具备无副作用证明，但必须在同一词法块中证明其局部类没有装饰器和基类，并以零参数构造。支持的类只包含返回 None 的进入方法和返回 False 的退出方法，参数形式必须兼容且没有其他执行副作用。属性目标的接收者仍须具备自身的局部实例证明；未知调用、写入、继承、装饰器、async 或多个管理器继续保留未知协议行为。

中间若导入未经证明的外部模块，也会终止独立接收者的正证明；不能仅凭模块名称信任其执行行为。

`repo feature-flags --query` 最多接收 4,096 UTF-8 字节及 64 个可搜索词，词边界与 Unicode 匹配规则一致。超限在 freshness 选择和 SQL 构造之前返回 `invalid_argument` / HTTP 400。CLI 与 Web 配置查询共用该上限，约束 SQL 参数数量和表达式深度，不截断用户查询。
...
文本与元数据过滤组合使用时，候选准入先在有界绑定组中检查二者的交集，再应用排序候选数量上限。文本可以来自 Java usage，元数据可以来自与之相连的配置定义；全部元数据条件仍须由同一个 usage 满足。每条关系都限制在授权 snapshot、路径和语言范围内，并保留 Java 平台准入规则。关联邻域超过身份预算，或四跳后仍有未访问的邻居时，即使尚未找到元数据匹配，也显式返回分析不完整，不能当作完整的空结果。最终组匹配及共享 SQLite 执行预算保持有效。此查询修正不要求重建事实或迁移查询索引。
...
Java 克隆字节预算集成测试必须通过公开的检查点会话 API 发布基础作用域，并在尝试带围栏的覆盖写入前验证已完成且事实行数非零的持久化证明。基础和目标作用域保持相同的文件及行数限制，目标字节配额仍为 61,440；只有基础字节配额更大。断言必须识别不可变基础作用域的字节配额拒绝，并确认没有写入目标事实。缺少证明时的回退属于另一种行为，不得满足这项预算专用断言。

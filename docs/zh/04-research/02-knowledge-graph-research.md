...
[中文](../../zh/04-research/02-knowledge-graph-research.md) | [英文](../../en/04-research/02-knowledge-graph-research.md)
...
| 维度 | 结论 |
| --- | --- |
| 研究来源 | 以知识图谱构建、KG refinement、RAG、GraphRAG、BM25/HNSW/RRF 和属性图标准化论文为主，并对照图数据库工程生态。 |
| 研究目标 | 明确 relay-knowledge 的知识图谱底座不是一次性三元组存储，而是持续演化、可检索、可解释、可回滚的核心服务。 |
| 关键竞争力 | 属性图、证据来源、图版本、索引新鲜度、三层检索和事件驱动刷新共同构成长期基础能力。 |
| 场景与未来 | 支撑文档图谱、代码图谱、GraphRAG context pack、Agent 查询和后台索引恢复，预留动态知识和多模态证据演进空间。 |

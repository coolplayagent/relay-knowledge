...
[中文](../../zh/04-research/03-arxiv-knowledge-graph-paper-insights.md) | [英文](../../en/04-research/03-arxiv-knowledge-graph-paper-insights.md)
...
| 维度 | 结论 |
| --- | --- |
| 研究来源 | 以 arXiv 论文和综述为主，覆盖 GraphRAG、LLM+KG、动态图谱、KGC、可解释检索和评估基准。 |
| 研究目标 | 建立前沿算法雷达，区分应该进入近期架构的通用原则和仍需观察的实验性方向。 |
| 关键竞争力 | 把论文中的成本、延迟、时间性、证据组织和鲁棒性要求转化为可测试的图版本、预算、路径和评估设计。 |
| 场景与未来 | 面向复杂问题分解、多跳证据、时间敏感知识、增量 GraphRAG 和可信评估，不为单篇论文定制特殊实现。 |

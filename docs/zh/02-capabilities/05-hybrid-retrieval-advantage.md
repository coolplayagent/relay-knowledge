...
混合检索是第二卷最核心的竞争力能力。它同时使用 BM25、local semantic token read model、local hashed-vector ANN、可配置 external semantic/vector backend、graph evidence fallback、code graph documents、local file path/content read model、schema path、temporal event、community summary 和 RRF。

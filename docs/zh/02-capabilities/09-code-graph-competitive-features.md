...
代码图谱在索引期间检测 Web 框架路由处理器绑定。支持的框架包括 Express（JavaScript/TypeScript）、Flask/FastAPI（Python）和 Spring（Java）。检测器覆盖真实 Express app/router 调用、带 middleware 链的最终 endpoint handler、堆叠 Flask decorator、FastAPI `APIRouter(prefix = ...)` 前缀、Spring 类级 `@RequestMapping` 前缀、顺序无关的 `value`/`path` 属性、path 数组、`RequestMethod` 数组，以及以 `http_method = "any"` 记录的无 method Spring mapping。每条检测到的路由生成一条 `CodeRouteRecord`，包含 HTTP 方法、URL 路径、处理器名称、框架标识符、源码位置，以及解析器能够匹配到结构化符号时的处理器符号链接。

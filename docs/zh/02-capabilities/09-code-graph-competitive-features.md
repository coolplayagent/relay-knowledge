...
宽泛 regex、未解析或歧义边、parser degraded、stale code index、ripgrep 缺失、ripgrep timeout 和 ripgrep budget exhaustion 都必须在响应中可见；unresolved external dependency edge 是 coverage metadata，本身不是降级。`text_fallback` 命中只能补齐召回窗口，不能压过已有 exact symbol 或 resolved edge。benchmark 提升不能依赖已知 path、query 或 symbol 特例。

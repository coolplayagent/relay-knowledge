...
[中文](./03-code-repository-tree-sitter-retrieval.md) | [英文](../../en/02-capabilities/03-code-repository-tree-sitter-retrieval.md)
...
- C 抽取在 tree-sitter-c tags 基础上增加语言感知补强：函数定义使用完整
  `function_definition` 范围而不是 `function_declarator` 单行范围，因此
  caller/callee 可以归属到真实函数；顶层函数声明标记为
  `function_declaration`，宏定义从 `preproc_def` / `preproc_function_def`
  索引为 `macro`，`#include` 会解析到已索引的本地头文件并给出 resolved
  import edge。语法树遍历使用显式栈，避免深层 C 语法树触发 Rust 调用栈溢出。
...
- `code_repository_search` FTS 候选表覆盖 symbol、reference、call、import 和 chunk。查询会在 FTS 子查询的 `ORDER BY bm25(...) LIMIT` 前下推已索引 scope 与本次请求的 path filters，减少大仓窄路径查询被路径外候选挤出窗口的风险。
- Symbol fuzzy 召回对查询词使用 OR 语义，避免自然语言描述词阻断 `_CHECKPOINT_VERSION` 或 `archive_output_dir` 这类多段 identifier；reference、call、import 仍保持更窄的 all-term 召回，并在有界候选截断前按 BM25 排序。
- Rust scoring 会识别 snake_case 与 CamelCase identifier 片段，给多段 symbol name 的部分覆盖、调用方向上下文、相关 callee 名称、声明形态 chunk 和纯虚 interface chunk 增加受限 bonus。所有 bonus 只作用于已召回的有界候选，不改变 source scope、path/language filter、edge metadata 或 API shape。
- Call 查询为了生成 excerpt，会通过 `code_repository_chunks(source_scope, symbol_snapshot_id)` lookup index 找 caller chunk，并要求 chunk line range 覆盖 call line，避免长函数或大类方法把同一 call edge 放大成多个候选。
- Finalize 阶段仍在完整 scope fact 落库后解析 reference、include 和 call edge；call FTS document 通过集合式 `INSERT ... SELECT` 从 `code_repository_calls` 重建，减少大仓逐条写入 FTS 的成本。

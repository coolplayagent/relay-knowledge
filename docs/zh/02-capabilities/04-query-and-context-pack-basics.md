...
`degraded_reason` 可能来自 stale index、graph-only、backend unavailable、parser degraded 或 budget exceeded。调用方应读取 metadata，而不是只看 item 数量。每条检索结果的 `staleness_hint` 字段提供了比 `stale` 布尔更结构化的逐条新鲜度标记，可区分 `fresh`、`pending_index` 和 `stale`，便于 agent 判断何时必须读取直接源文件。

...
| 区域 | 刷新内容 |
| --- | --- |
| 用户手册 | 在使用指南总览、CLI 命令参考、代码仓库工作流和排障章节说明 `repo query` 的 `definition`、`references`、`hybrid` 查询会先走 tree-sitter/FTS，再按需使用有界 `ripgrep` 兜底。 |
...
- `rg` 缺失、超时、候选文件预算或物化字节预算耗尽时，只降级 exact-text fallback，并通过 `degraded_reason` 暴露。

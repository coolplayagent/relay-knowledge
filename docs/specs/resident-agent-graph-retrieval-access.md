...
`prompts/list`、`prompts/get` 和 `DELETE /mcp` session termination。MCP tools 已覆盖通用图检索、诊断、
索引状态、授权 code graph query 和授权 code impact。MCP resources 暴露 graph metadata、
authorized scopes、index status 和 current diagnostics；prompts 暴露 context planning、
grounded answer drafting 和 graph debugging helper templates。旧 HTTP+SSE resumability
兼容端点仍返回稳定未实现错误。ACP 已提供本地会话 adapter，用于 agent client
...
- Transport lifecycle: `DELETE /mcp` 终止 session；GET/SSE resumability 返回稳定未实现错误。
...
当前 Rust 实现通过 `observability` runtime 记录上述 agent protocol metrics，并在
`RELAY_OTEL_METRICS=true` 时通过 OTLP HTTP/protobuf 导出到 Collector。`RELAY_OTEL_TRACES=true`
时，同一 runtime 安装 tracing/OpenTelemetry bridge，把 service mode 的结构化 spans 导出到
OTLP traces endpoint。OTLP 初始化或导出失败只降级 observability，不阻塞 MCP/ACP 请求。
...
- MCP resources/prompts shape、authorized scope filtering、diagnostics path redaction 和 `DELETE /mcp` session termination。
- OTLP exporter 初始化、失败降级和 agent protocol metrics snapshot。
...
7. Done: add persistent agent audit events and OTLP metrics/traces exporter.
8. Done: add MCP resources/prompts for graph metadata、authorized scopes、index status、diagnostics and helper templates.

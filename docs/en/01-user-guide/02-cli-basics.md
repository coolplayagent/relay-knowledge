...
`--remote` or `RELAY_KNOWLEDGE_REMOTE_BASE_URL` sends supported code repository index, incremental update, scope preview, status, query, feature-flag, impact, report, and software projection commands to the resident service HTTP API instead of opening local runtime storage. Remote mode does not run `repo index-worker`; index and update tasks are drained by the bounded worker pool in the remote `service run --web` process. Remote-selected `repo index --reset` and `repo index-worker` commands are rejected so maintenance cannot accidentally clear or drain local state.

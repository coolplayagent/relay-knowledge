...
Text queries can start from matching getter paths or excerpts before resolution and group metadata filtering. Getter bodies ignore comments when proving a sole return. A single wildcard type import is supported; competing wildcard imports remain unresolved. Referenced Java key declarations join the property/environment namespaces established by their reads, including both when a constant is reused. Properties defaults retain trailing whitespace. Properties, INI, template and Shell extraction share the 10,000-fact per-file limit, including template read actions.
...
`repo software` reads the software global model for the selected repository scope. Existing kinds keep their compatibility projections: `dependencies` returns manifest/lockfile package components and dependency usage; `sdks` returns unresolved, ambiguous, or external targets; and `files`, `topics`, `relationships`, `build`, `iac`, and `design` return their established slices. Dockerfile/Containerfile evidence is now a build definition and a CI workflow job is a pipeline/build job. Only explicit deployment evidence such as Compose, Kubernetes, Helm, Terraform, systemd, and launchd enters IaC. An ordinary README heading produces only a documentation topic; promotion to a system, component, API, or resource requires explicit frontmatter, a controlled manifest/schema, or structured code evidence.

Typed kinds `systems`, `apis`, `resources`, `tests`, `deployments`, and `releases` return ontology entities with stable `entity_key` and snapshot `occurrence_id`. `statements` returns subject/predicate/object, source/evidence, assertion mode, resolution, validity, extractor, confidence, and fact state. `conflicts` returns conflicting, unresolved, superseded/rejected statements and shape diagnostics. Every response status includes `ontology_version`, `projection_schema_version`, `source_coverage`, `completeness_basis_points`, `freshness`, and `conflict_count`.

For `repo software --kind all`, `--limit` is a strict combined row bound rather than a separate allowance for each array. Rows are assigned round-robin across non-empty arrays in the fixed response order `components`, `dependency_usages`, `sdk_usages`, `files`, `topics`, `relationships`, `build_targets`, `iac_resources`, `design_elements`, `entities`, `statements`, and `diagnostics`. Empty or exhausted arrays give their capacity to later rounds. A limit smaller than the number of non-empty arrays therefore uses that order as its deterministic priority; the default 100-row bound prevents a dense dependency slice from hiding later lifecycle and ontology slices.

`repo software export` emits a raw standard JSON document from the same snapshot-bound application service: `spdx-3` is SPDX 3.0.1 JSON-LD, `cyclonedx-1.7` is CycloneDX 1.7 JSON, and `prov-o` is PROV-O JSON-LD. Export does not invent standard fields absent from the current ontology. Queries and exports do not execute build tools or scan package caches, SDK directories, cloud APIs, unindexed external source, or whole-repository docs at request time. Run `repo index` or `repo update` after source-scope changes.
...
Agent-facing MCP kind access reuses the same kind families rather than introducing parallel names. `relay_code_query` covers code graph kinds, `relay_business_query` covers authored business terms and technical mappings, and `relay_software_query` covers every software global-model kind and accepts `export_profile=spdx-3|cyclonedx-1.7|prov-o` for a standard export envelope. `relay_code_feature_flags` covers configuration-driven feature flags, and `relay_codebase_view` covers the `repo view` kind family. Common agent aliases are normalized to existing kinds: `dependency` to `dependencies`, `configuration` to `relationships`, and `model` or `models` to `design`.

`map` commands maintain `codespec/codespec-map.yaml` and `knowledge/knowledge-map.yaml`. `map init`, `show`, `history`, and `validate` default to `--type all`; targeted mutations require `--type knowledge` or `--type codespec`, while source and route commands accept only Knowledge. Schema v4 keeps typed `directories` and content-addressed topics under `knowledge/topics/`, but retains only the latest 16 history entries in each root and creates no history archive tree. `map history` defaults to the earliest retained version, accepts at most 16 entries, and rejects an explicit version at or before `omitted_through`; use Git or repository backups for older audit history. `map directory add|update|remove` is the supported update surface for directory governance; the five baseline directories in each map cannot be removed. `map migrate --type knowledge --to-v4` validates legacy assets, publishes the visible and fallback v4 roots, writes a v4 redirect at the legacy path, and performs bounded, resumable cleanup of recognized old history artifacts. There is no data-level map rollback command. `map validate` remains authoritative for files, digests, relationships, history, paths, built-in sources, and AGENTS references.

The CLI skill ships `references/knowledge-map.schema.json`, a JSON Schema Draft 2020-12 document covering the v4 root manifest, topic shard, recent-history window, and redirect. Editors and agents may use it for field discovery and structural checks after parsing YAML into a JSON-compatible value. The schema intentionally permits unknown fields to match the current Serde readers. Schema acceptance does not prove digest/content agreement, global source-id uniqueness, route completeness, recent-history continuity, omission-checkpoint agreement, or reserved-source invariants; `relay-knowledge map validate` remains authoritative. The schema also does not authorize direct edits to CLI-generated roots, shards, or redirects.
...
Repeating `map source update` with the same normalized fields preserves versions,
history, shards, and root files unless migration or reserved-route repair is needed.
`map init` resumes obsolete-shard cleanup even without a content change: it protects
recovery roots, waits for a 60-second retirement grace, and processes at most 1,024
unreferenced regular shards per attempt. Run init again when cleanup remains pending.
...
The same skill package includes the Knowledge Map v4 schema for structural
tooling. Its metadata gate checks the Draft identifier, all three artifact

...
`map` commands maintain the repository-local `.knowledge/knowledge-map.yaml` knowledge navigation contract. `map init` creates a Knowledge Map v2 root manifest or idempotently migrates a valid v1 single-file map while ensuring the reserved `software-model` route and `repository-software-model` repository source; an incompatible reserved source is rejected rather than overwritten. V2 stores each topic's sources and route in a content-addressed file under `.knowledge/topics/`; `map route <topic>` reads only the root and requested shard, while `map show` assembles the compatibility view. The root keeps at most 16 recent map changes and points to a SHA-256-verified immutable archive under `.knowledge/history/` for older entries. `map validate` verifies the complete archive and every shard.

The contract stores stable navigation and model-entry metadata only. It does not copy authoritative knowledge or snapshot-bound architecture/build/deployment projection rows out of documents, code, config, CI, runtime systems, or external sources. One topic can contain multiple sources, and `map source add` appends distinct source ids to that topic route order. All refs are repository-controlled relative paths; absolute paths, parent traversal, and symlink escape are rejected. Mutations share one writer lock, publish immutable artifacts first, and replace the root manifest last so an interrupted write leaves the previous root usable. LLM agents should use `map show` and `map route` to locate sources, maintain the contract through `map source add/update/remove`, and run `map validate --format json` after changes. AGENTS.md should keep only a stable reference such as `Knowledge map: .knowledge/knowledge-map.yaml`.

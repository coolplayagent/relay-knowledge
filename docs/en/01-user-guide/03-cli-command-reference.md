...
`repo context` is the one-call coding-agent context pack over the same committed code graph read model. It first runs bounded hybrid, definition, and symbol entry queries, then expands top seeds through references, callers, callees, and imports. JSON responses expose `entry_points`, `related_symbols`, `graph_paths`, `impact_hints`, `code_excerpts`, `retrieval_layers`, `budget`, `truncated`, `freshness`, and `diagnostics`. `--max-context-bytes` caps the packed evidence, `--no-code` removes excerpts while preserving provenance, and the command never starts repository indexing or refresh work.
...
`map` commands maintain the repository-local `.knowledge/knowledge-map.yaml` knowledge navigation contract. `map init` creates a Knowledge Map v2 root manifest or idempotently migrates a valid v1 single-file map while ensuring the reserved `software-model` route and `repository-software-model` repository source; an incompatible reserved source is rejected rather than overwritten. V2 stores each topic's sources and route in a content-addressed file under `.knowledge/topics/`; `map route <topic>` reads only the root and requested shard. `map show` reads all current shards but returns only the root's bounded recent-history window, with `archived_through` and `complete` making omitted history explicit. Text and Markdown output report `history_complete`, `history_archived_through`, and the recent-entry count; when the window is incomplete they also direct the operator to start paging archived entries with `map history`. The root keeps at most 16 recent changes and points to SHA-256-verified immutable archives under `.knowledge/history/`. A content-addressed, bounded-depth B+ tree index lets `map history --from <version> --limit <count>` locate old archives directly, with at most 11 index-node reads plus one archive read per archive and a page maximum of 256. When an early v2 archive chain has no `history.index`, paging tells the user to run `relay-knowledge map init`; that command publishes the index root atomically under the writer lock instead of letting reads fall back to a linear chain scan. `map validate` verifies the complete archive, index, and every shard.

The contract stores stable navigation and model-entry metadata only. It does not copy authoritative knowledge or snapshot-bound architecture/build/deployment projection rows out of documents, code, config, CI, runtime systems, or external sources. One topic can contain multiple sources, and `map source add` appends distinct source ids to that topic route order. All refs are repository-controlled relative paths; absolute paths, parent traversal, and symlink escape are rejected. Mutations share a cross-platform OS advisory writer lock, publish immutable artifacts first, and replace the root manifest last. A live writer remains exclusive, while an abnormal process exit releases ownership without requiring deletion of the persistent `.lock` inode. The first mutation also creates or extends the target repository's `.knowledge/.gitignore`; commit that nested contract with the map so canonical and prepared lock inodes remain excluded in ordinary Git repositories and linked worktrees without relying on this project's root `.gitignore`. LLM agents should use `map show` and `map route` to locate sources, maintain the contract through `map source add/update/remove`, and run `map validate --format json` after changes. AGENTS.md should keep only a stable reference such as `Knowledge map: .knowledge/knowledge-map.yaml`.

...
[English](../../en/01-user-guide/07-operations-and-troubleshooting.md) | [中文](../../zh/01-user-guide/07-operations-and-troubleshooting.md)

This is the English documentation page for `user-guide/07-operations-and-troubleshooting.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
`source_scope '<scope>' is not authorized for this MCP policy`: 请求 scope 不在 `RELAY_KNOWLEDGE_MCP_ALLOWED_SCOPES` 中，也不是当前运行时已注册的 code repository alias。运行中可先执行 `relay-knowledge repo register <path> --alias <scope>` 并完成需要的索引；否则按错误中的提示追加 `RELAY_KNOWLEDGE_MCP_ALLOWED_SCOPES=<scope>` 后重启服务。
...
当前版本会在启动时迁移已知的过期 SQLite schema，并只重建可派生的索引表或刷新队列。
如果仍复现，先运行 `relay-knowledge health --format json` 获取完整错误，再用隔离

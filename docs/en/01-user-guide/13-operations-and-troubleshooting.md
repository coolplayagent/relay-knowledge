...
`invalid --kind value`: `index refresh` accepts only `bm25`, `semantic`, or `vector`; `repo query` accepts only `hybrid`, `symbol`, `definition`, `references`, `callers`, `callees`, or `imports`.

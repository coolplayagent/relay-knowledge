...
`repo status` shows `active_task.state=running` but `checkpoint.parsed_file_count` stays at 0: inspect `active_task.lease_expires_at_ms` and `checkpoint.updated_at_ms` in `repo status --format json`. In a non-interactive agent session, do not repeatedly start `service run`; it is a foreground resident process. On startup, `service run` recovers `code-index-worker-<pid>` leases whose owner process has exited and records `lease_orphaned`; leases owned by a still-running worker are preserved. Git blob batch reads used by cold indexing are bounded and report a Git command error when `git cat-file` stops responding, so the task can retry or dead-letter instead of remaining leased indefinitely. If `git cat-file --batch` or `--batch-check` returns an unexpected header such as a missing object row where a blob size was expected, indexing falls back to bounded per-path Git reads instead of failing the whole batch. If the task is queued or retrying, run `repo index-worker --task-id <active_task.task_id> --format json` to make one bounded worker attempt. For an unfinished task that remains stuck, run `repo index <alias> --reset --format json` and then run `repo index-worker --task-id <reset_task_id> --format json`; if a historical task has already dead-lettered, rerun `repo index <alias> --ref <ref>` to intentionally queue the current scope instead of reviving terminal history. Do not kill relay-knowledge processes or bypass task leases.

`repo status` remains at `finalizing:software_projection:v3:*`: this is a recoverable software-projection phase, not a successful publication. Text output appends `error_kind` and a quoted `error` for retrying or failed tasks; JSON retains the complete `active_task.last_error_kind`, `active_task.last_error_message`, lease, and checkpoint timestamps. After upgrade, legacy v1/v2 phases safely replay projection derivation from reset. A normal worker resumes the current v3 phase and releases the SQLite writer between phases for lease renewal; do not edit the checkpoint to completed. Git reconciliation warnings from the watcher now include `operation=resolve_head|observe_worktree|resolve_snapshot`, which distinguishes Git-ref failures, worktree-file I/O, and snapshot resolution. A single temporarily unreadable changed worktree path contributes a stable unreadable fingerprint instead of being mislabeled as a HEAD-resolution failure; the later index task still reports authoritative-content read failures through normal task diagnostics.

SQLite WAL or planner maintenance looks wrong: inspect `graph.sqlite` in `health --format json` or graph inspection. `journal_mode` should be `wal`, `wal_size_bytes` reports the current `-wal` file size, `last_maintenance_at_ms` is the most recent post-bulk-index maintenance attempt, and `last_maintenance_error` records a failed `PRAGMA optimize` or `PRAGMA wal_checkpoint(PASSIVE)`. The timestamp and error are persisted in SQLite, so they should remain visible after a service restart or one-shot worker exit. Under `partitioned_sqlite`, these values aggregate the control database and active repository shard databases through read-only shard diagnostics; if any active shard cannot be inspected, `wal_size_bytes` is unknown and the shard error remains in `last_maintenance_error`. This error does not invalidate an already completed index result; if query latency also regresses, reproduce the large-repository performance gate with `tools/self_iteration --categories performance` and then check database disk space, file permissions, and read-only mounts.

...
`repo status` remains at `finalizing:software_projection:v1:*`: this is a recoverable software-projection phase, not a successful publication. Text output appends `error_kind` and a quoted `error` for retrying or failed tasks; JSON retains the complete `active_task.last_error_kind`, `active_task.last_error_message`, lease, and checkpoint timestamps. A normal worker resumes the current phase and releases the SQLite writer between phases for lease renewal; do not edit the checkpoint to completed. Git reconciliation warnings from the watcher now include `operation=resolve_head|observe_worktree|resolve_snapshot`, which distinguishes Git-ref failures, worktree-file I/O, and snapshot resolution. A single temporarily unreadable changed worktree path contributes a stable unreadable fingerprint instead of being mislabeled as a HEAD-resolution failure; the later index task still reports authoritative-content read failures through normal task diagnostics.

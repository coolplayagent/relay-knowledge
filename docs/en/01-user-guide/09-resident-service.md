...
With `RELAY_KNOWLEDGE_WATCHER_ENABLED=true` (the default), the managed watcher handles both uncommitted source changes and checked-out Git commits. Native `.git/HEAD`, ref, packed-ref, and HEAD-log events are low-latency hints; a bounded reconciliation resolves every watched repository's `HEAD` at startup and every `RELAY_KNOWLEDGE_WATCHER_COMMIT_RECONCILE_INTERVAL_MS` milliseconds (default `5000`). Periodic reconciliation is authoritative, so linked worktrees and missed/coalesced watcher events recover without a repository-specific hook. When HEAD differs from the last published clean commit, the watcher queues one pinned durable Incremental task. Stable per-ref fingerprints coalesce repeated hints, and normal task leases preserve one writer per repository. Disabling the watcher disables both source events and commit reconciliation.

Use `relay-knowledge service status --format json` to inspect `storage`, `code_index_workers`, and `watcher`. `storage` reports the current topology, primary database path, `partitioned_sqlite` shard directory, active/staged/missing shard counts, runtime state paths, and missing-shard degraded reasons. `code_index_workers` reports configured worker count, active worker slots, queue depth, queued/running/retrying/dead-letter task counts, running leases, and last error. `watcher.enabled` and `watcher.commit_reconcile_interval_ms` report the effective installed commit-reconciliation configuration. These diagnostics explain whether the master is idle, saturated, retrying work, waiting for another repository writer lease, or missing partitioned data-plane shard files.

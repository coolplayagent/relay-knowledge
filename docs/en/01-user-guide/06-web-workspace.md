...
The displayed command is a CLI-equivalent preview, not a simulated frontend result. The real result comes from the unified API response returned by `/api/web/operations/execute`. The `knowledge.map.history` operation accepts the same positive `from_version` and bounded `limit` (1-256) as CLI `map history`; it is available when `service run --web` starts inside a discoverable repository. On error, copy operation, command, error kind, and metadata from result JSON, then reproduce with the same CLI arguments.

...
Operation payload validation follows the shared API contract. For example, `code.repo_set.add` defaults a missing `priority` to `0`, but a present malformed or out-of-range `priority` is rejected as a bad request instead of silently changing member ranking.

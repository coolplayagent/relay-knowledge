...
For `--ref worktree`, staging the parent gitlink is enough to include the committed submodule update in the overlay; unstaged dirty content inside the submodule is still ignored.
...
The overlay is bound to the current checked-out `HEAD`, uses a synthetic snapshot identifier, and includes modified files, untracked files, and staged submodule gitlink updates. While an overlay is active, clean commit ref queries are rejected so uncommitted content is not mislabeled as a clean Git snapshot.

...
For `--ref worktree`, staging the parent gitlink is enough to include the committed submodule update in the overlay, even if the submodule worktree is later deinitialized and the committed objects remain available through `.git/modules`; unstaged dirty content inside the submodule is still ignored.

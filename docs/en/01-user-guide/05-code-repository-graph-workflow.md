...
The `config-registry-v5` extraction component invalidates completed scopes created with earlier configuration semantics, independently of Python or query-index versions. Ordinary `repo index <alias> --ref <ref>` on the same commit refreshes these facts through the durable leased pipeline; `--reset` is not required. Java receiver/constant proof, template action boundaries and static shell defaults are refreshed together. Configuration fact v5 also adds bounded per-file Java namespace summaries and deferred query indexes; the installation guide describes the additive schema and query-plan v6 upgrade. Preserve the runtime database, WAL and task checkpoints during upgrade; exact rollback restores the pre-upgrade database/shards with the matching binary.

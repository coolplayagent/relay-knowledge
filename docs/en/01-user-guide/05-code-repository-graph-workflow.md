...
The `cpp-callable-declarations-v2` and `python-overload-declarations-v17` fact components require ordinary same-HEAD indexing to rebuild completed scopes. Schema migration only adds the nullable signature column; legacy records/imports retain unknown evidence until reindexing, with no startup fact backfill. Snapshot and durable clone/delta budgets include the signature bytes; imports preserve the nullable key. Copy new snapshot selectors after rebuilding. Complete or cancel unfinished tasks using a binary compatible with their original fact versions before upgrading; preserve the database, WAL, and checkpoints together for rollback.

Callable equivalence currently proves primitive parameter types and ignores top-level scalar cv qualifiers. Pointee and member cv/ref qualifiers, arity and variadic parameters remain significant. Typedefs, qualified/struct/template types, array parameter adjustment, relevant macros, top-level pointer cv qualifiers and noncanonical type spellings remain unknown. A canonical caller query that needs to combine such declarations asks for an explicit snapshot instead of guessing equivalence or omitting declaration callers silently. C++ `(void)` is equivalent to an empty parameter list; an old-style empty C parameter list remains unknown.

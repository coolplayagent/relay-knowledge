...
Java getter receiver resolution covers method/constructor parameters, explicitly typed fields and locals, `this.field`, parentheses, explicit casts, and `new` expressions. `var` inference uses only statically resolvable initializers with bounded alias chains. Visible same-file inherited fields and object field access share type identities; inner locals, lambda parameters, loop variables, and resource variables take precedence over outer fields. Dynamic factories and missing external type definitions do not supply guessed types. Each lookup visits at most 4,096 nodes and follows at most 16 expression levels; unknown types or exhausted budgets do not establish a proven configuration binding. Field/local getter reads and guards use the same persisted facts, filters, and consistency analysis as parameter receivers.
...
The expression budget follows evaluation timing: lambda defaults and a generator’s outer iterable are eager, while lambda and unconsumed generator bodies are deferred. Annotation-only module/class statements do not replace values; function-local annotation bindings remain local. A proven unchanged zero-argument invocation fixes the decorator evidence only when it synchronously reaches that specific decorator along a straight path and its remaining statement sequence cannot trigger another invocation. Async functions, generators (including yields in nested defaults), conditional or exiting paths, later calls or imports, changed callable identities, and argument evaluation remain conservative boundaries. Uncalled nested function bodies stay deferred.

The `config-registry-v7` extraction component invalidates completed scopes created with earlier configuration semantics, independently of Python or query-index versions. Ordinary `repo index <alias> --ref <ref>` on the same commit refreshes these facts through the durable leased pipeline; `--reset` is not required. Java receiver/constant proof, template action boundaries and static shell defaults are refreshed together. Configuration fact v6 retains bounded per-file Java namespace summaries and deferred query indexes; the installation guide describes the additive schema and query-plan v7 upgrade. Preserve the runtime database, WAL and task checkpoints during upgrade; exact rollback restores the pre-upgrade database/shards with the matching binary.
...
Configuration query validation runs before storage access, index availability and freshness selection. A supplied query must contain a searchable Unicode letter, number or underscore under the same term boundaries used for SQL selection and final matching. Invalid queries return `invalid_argument` / HTTP 400 for `graph_only`, `allow_stale` and `wait_until_fresh`; an omitted query keeps its list behavior, and valid queries retain existing freshness and backend-error behavior.

Manual Go template recovery recognizes a single-variable declaration head, `$name := pipeline`, without evaluating the declaration name as a read, and distinguishes it from assignment with `=`. Legal digit-leading names and root-variable declarations remain supported. The pipeline retains the existing action-byte, parenthesis, control-block, and literal checks. Variable reads or assignments without lexical-scope proof remain conservative diagnostics; this does not authorize arbitrary `$name` expressions. Literal configuration calls after declaration keep their source ranges and defaults.

Python origin refresh uses the shared language classifier, including `.pyw`, in incremental, filesystem and worktree-overlay indexing. Overlay origin reparsing shares the existing total byte budget with changed files. A class-body module import may execute arbitrary module code: only imports with proven standard typing origins preserve overload proof. Possible aliases created in control flow and earlier decorator-expression side effects are included in the bounded proof; decorator expressions are evaluated in source order. Version v14 rebuilds previous scopes through ordinary indexing and preserves explicit budget failures and degraded diagnostics.

Implicit Java platform receivers require package evidence from the authorized indexed source scope. The original Java AST records the declared package and direct top-level types, including non-public types in differently named files. Qualified names are assembled from identifier nodes, allowing legal intervening comments without interpreting comment text as part of a name; all visited nodes share the namespace collection budget. A same-package type shadows the implicit platform type; an explicit platform import or fully qualified receiver keeps its own proof. Incomplete authorization, parse evidence or bounded namespace inventory does not prove absence of a shadow. This is a static authorized-source model: indexing does not inspect host classpaths or external compiled dependencies. Raw reads and guards remain persisted so adding or deleting a same-package provider can change the current scope interpretation without losing the original evidence. Retained historical scopes keep their own namespace facts. Query path filters narrow results, not the namespace evidence of an already authorized complete scope. Proven constant keys retain bounded decoded nonempty runtime strings, including slashes, colons and Unicode; an unread string constant does not become a configuration declaration.

Java namespace collection from the original AST is bounded to 1,024 node visits and 65,536 name bytes. Namespace projections count toward batch row and byte budgets, including repeated scope, path, package and per-row overhead. Ordinary parsing, direct snapshot admission and durable copying account for these costs; checkpoints retain actual source bytes and original logical-fact counts. Implicit platform-read metadata stores only the receiver type, resolving its package through the indexed owner file instead of duplicating package strings per read or guard. The existing planner finishes a bounded file before checking the flush threshold, so a single file may cross that threshold; this is not a new whole-batch hard-rejection rule. Each Java file adds one namespace row and at most 1,024 type rows, and unknown evidence adds no type rows. Incomplete source coverage remains unknown evidence without inventing external-dependency degradation.

When a Python provider change forces unchanged consumers to be parsed again, explicit Git incremental and filesystem refreshes enforce a checked aggregate file and byte budget across changed files and added reparses, matching the bounded overlay contract. Prefetch limits alone do not authorize unlimited fallback parsing. A refresh beyond the admitted budget fails explicitly and preserves the previously published scope; use the durable full-index workflow for larger refreshes. Ordinary updates without an origin change retain their existing behavior.

For Java implicit-platform absence proof, an unfiltered Git snapshot covers the repository root. Filesystem indexing with empty path filters instead uses discovered default source roots and cannot prove whole-root absence. Register an explicit root path filter, such as --path ., for filesystem-wide package evidence. Source scanning and evidence admission share the same filesystem path normalization; request-level query filters still only narrow returned usages.

Configuration display names retain the existing ASCII normalization where it produces a name. If normalization would erase a valid nonempty runtime key, including a pure Unicode key, the display name falls back to the original key. The persisted source key, configuration binding and source range remain unchanged, and software-graph materialization no longer receives an empty display-name attribute for these keys.

When a provider change and Gitlink update overlap, unchanged Python consumers remain scheduled for origin refresh. An exact-path set at the actual parse entry point ensures that an expanded changed child and its forced refresh are parsed, charged and recorded only once. The existing Gitlink diff-expansion limit, aggregate origin budgets and durable full-index fallback remain in force.

Go-template recovery accepts the language-defined two-variable declaration head in `range`, alongside the existing single-variable form. Two-variable declarations in other control pipelines, three-variable ranges and unproven variable expressions retain partial diagnostics. Recovery does not evaluate variables or expand its work budget.

Configuration display-name normalization preserves Unicode letters and numbers at mixed-script key boundaries. For example, `功能.flag` displays as `功能_flag`; source keys and identities remain unchanged, with existing separator replacement and ASCII case normalization retained. Constant and getter binding identities use the same bounded structured Java package spelling as namespace evidence, so legal intervening block or line comments do not alter cross-file binding identity. Original source ranges remain intact.

Configuration query seed selection searches the same text fields as final matching: name, source kind/key, edge kind, path, excerpt, decoded binding names and referenced symbols. Unrelated metadata such as domain or default values does not occupy the ranked seed window unless that text also appears in a searchable field. Explicit metadata filters still apply to a single usage, and multi-term alias assembly, seed limits and the shared SQLite execution budget remain in force.

Java implicit-platform evidence uses the supported static source-root model. Standard Java main and test source roots carry their enclosing module prefix: main sources see providers in that module's main source set, while tests see that module's main and test sets. Independently rooted modules do not implicitly share providers. Files outside recognized source-set layouts retain the documented repository-root model. Ambiguous layouts or missing legacy source-set evidence remain unknown; this is not Maven/Gradle dependency or classpath resolution. Request path filters only narrow returned usages and cannot hide providers visible to the consumer.

Source-set identity is derived from the authorized source path independently of type-parse completeness. An incomplete Java file in a known unrelated module or test-only source set does not invalidate a main source's provider-absence proof. Incomplete evidence in a visible source set, and unknown source-set identity, remain conservative. Source-set kind/module text participates in the existing repeated projection row/byte accounting, including durable copies.

Origin-triggered incremental indexing first expands the existing bounded Gitlink diff, rename and copy workflow into an exact authorized file work set. Each file is queued once; Gitlink containers and unaffected descendants do not consume parse-file or blob-byte budget. The queue is capped before blobs are read, exact planned bytes are admitted before prefetch, and the same files are parsed with runtime budget checks. Raw diff changes and forced Python candidates remain separately bounded. The existing deletion, tombstone, lease and checkpoint behavior is preserved. This admission correction keeps Python fact version v14 because successfully published facts do not change. Failed updates can be retried through the ordinary update command.

Python overload evidence accounts for eager class-body bindings redirected by `global` or `nonlocal` to the same lexical binding target, including names introduced by function and class definitions. A redirected custom decorator therefore remains executable evidence instead of being discarded as a typing declaration; two executable definitions with one canonical identity remain ambiguous and require a snapshot selector. Ordinary class-local names and deferred function bodies retain their separate scope and evaluation rules. Traversal keeps the existing shared work budget. The `python-overload-declarations-v18` component makes ordinary same-HEAD indexing rebuild completed scopes with the corrected classification; no reset or source change is required.

Python overload proofs distinguish an annotation-only class name from a runtime binding: only an assigned class annotation owns that name, while a function-local annotation still establishes a local binding. Eager implicit protocol execution (including truth tests, iteration, context entry, operators, descriptors, subscription, formatting, unpacking, and dictionary-key/set-member hashing) can invalidate a typing-provider proof. Bounded literal proofs preserve safe built-in operations; dictionary values and deferred function bodies retain their evaluation rules. The shared traversal budget remains enforced.

C/C++ canonical caller queries include calls targeting declarations that have the same structured callable signature as the unique executable definition. Parameter names and defaults do not distinguish signatures; parameter types, declarator structure, arity, and member qualifiers do. Ambiguous definitions or unproven declaration equivalence require an explicit snapshot selector. Explicit snapshot queries keep their exact target semantics. The nullable `callable_signature_key` is separate from display/search signatures and is limited to 2,048 UTF-8 bytes, 1,024 syntax nodes per key, and 65,536 shared syntax nodes per file. Canonical resolution admits at most 1,024 candidate symbols and retains the existing query work budget; overflow reports ambiguity rather than silently truncating callers.

The `cpp-callable-declarations-v3` and `python-overload-declarations-v18` fact components require ordinary same-HEAD indexing to rebuild completed scopes. Schema migration only adds the nullable signature column; legacy records/imports retain unknown evidence until reindexing, with no startup fact backfill. Snapshot and durable clone/delta budgets include the signature bytes; imports preserve the nullable key. Copy new snapshot selectors after rebuilding. Complete or cancel unfinished tasks using a binary compatible with their original fact versions before upgrading; preserve the database, WAL, and checkpoints together for rollback.

Callable equivalence proves standard builtin parameter types, normalizes equivalent specifier order and omitted `int` (such as `unsigned`/`unsigned int`), and ignores top-level scalar cv qualifiers. Distinct type ranks and plain/signed/unsigned `char` remain distinct. Pointee and member cv/ref qualifiers, arity and variadic parameters remain significant. Typedefs, qualified/struct/template types, array parameter adjustment, relevant macros, top-level pointer cv qualifiers and unsupported declarator spellings remain unknown. A canonical caller query that needs to combine such declarations asks for an explicit snapshot instead of guessing equivalence or omitting declaration callers silently. C++ `(void)` is equivalent to an empty parameter list; an old-style empty C parameter list remains unknown.

Implicit-protocol analysis preserves operations whose receivers are proven safe within the shared lexical work budget: direct/local module aliases, built-in container iteration, the truth value of a proven overload function, literal-key access to a proven module dictionary, and instances of a locally proven empty class. Intervening unknown execution or receiver mutations invalidate those proofs. An external constructor, including an imported constructor without source-origin evidence, is not trusted by its spelling; its receiver remains unknown even when a particular local execution happens to be harmless. Module `__getattr__` and `__class__` writes share the same conservative dispatch-hook policy across attribute, dictionary and mutator-call syntax. Previously proven decorators are not reinterpreted as unknown operations under an unrelated alias.

An attribute read on a separately imported standard typing module can preserve an independently imported overload function when the receiver has its own bounded, unchanged import proof. Local provider origins, intervening unknown execution, receiver rebinding and dispatch-hook writes terminate that proof.

A single synchronous context manager is also provably harmless when the same lexical block establishes its local, undecorated, base-free class and zero-argument construction. The supported class has only an enter method returning None and an exit method returning False, with plain compatible parameters and no other executable effects. Receiver targets require their own local-instance proof; unknown calls, writes, inheritance, decorators, async or multiple managers retain unknown protocol behavior.

An intervening import from an unproven external module also ends the independent receiver proof; the imported module is not trusted by its name.

`repo feature-flags --query` accepts at most 4,096 UTF-8 bytes and 64 searchable terms, using the same Unicode term boundaries as matching. Excess input returns `invalid_argument` / HTTP 400 before freshness selection or SQL construction. The term budget bounds SQL parameters and expression depth for both CLI and Web feature-flag requests; it does not truncate the requested query.

Java identifier evidence uses Unicode 16.0 general categories for Java identifier-start/part rules, including currency symbols, connector punctuation, combining marks and identifier-ignorable characters. The parser compatibility view preserves every source byte offset and reads facts from the original source; comments, strings and text blocks retain their contents. Invalid identifier starts and unrelated broken syntax keep diagnostics. Unicode category coverage is versioned and is not a claim that every JDK release accepts the same character set. The native grammar's bare-CR line-comment gap remains unsupported: such files have incomplete namespace evidence, so implicit platform reads are not inferred from a falsely complete type inventory. CRLF handling remains supported, and this change does not alter the repository's line-coordinate contract.

Go-template `template`, `define` and `block` names must be valid quoted or raw string literals, including valid escape sequences. Missing, unquoted and rune-literal names retain partial syntax diagnostics even when the proxy grammar accepts them without error nodes. Valid quoted/raw names, escaped names and template invocation without a pipeline remain supported; this validation retains the existing bounded action scanner.

Configuration fact version `config-registry-v7` refreshes these Java identifier and Go-template classification changes through ordinary same-HEAD indexing of completed scopes. It introduces no additional query-index plan version: the existing query-plan v7 remains unchanged. Keep unfinished-task compatibility and database/WAL/checkpoint rollback guidance above when changing binaries.

Direct worktree-overlay clone admission counts `source_set_kind` and `module_root` in both Java namespace and type projections, including every repeated type row and the existing read/write multiplier. An over-budget clone requests the existing durable staging path before target fact writes; the active base scope remains available. Frozen task budgets, leases and checkpoint ownership remain unchanged.

When text and metadata filters are combined, candidate admission checks their intersection across the bounded binding group before applying the ranked seed limit. Text may occur on a Java usage while a connected configuration definition supplies the metadata; all requested metadata filters must still match one usage. Every relationship stays inside the authorized snapshot, path and language scope, with Java platform admission preserved. A neighborhood that exceeds the identity budget or still has unseen neighbors after four hops returns explicit incomplete analysis, even if the metadata match has not yet been found; it is never reported as a complete empty result. Final group matching and the shared SQLite execution budget remain in force. This query-only correction needs no fact rebuild or query-index migration.

Properties parsing isolates the process-global EOF state in `tree-sitter-properties` 0.3.0 with a properties-only mutex inside the existing synchronous parser workers. The guard covers parser creation, language setup, reset and parsing; other languages remain parallel. Callback work limits, worker admission, bounded batch queues and task leases are unchanged. After cancellation or a caught parser panic, the next parse resets its cached parser under the guard and starts from empty scanner state; the original failure remains observable. The dependency's scanner destructor is currently a no-op, so thread-local cache destruction cannot change shared EOF state. Remove this compatibility guard only after verifying an upstream per-instance scanner. The performance suite retains the complete 36-file configuration fixture and runs two additional isolated cold indexes, each with its existing completion, freshness and query-result gates and separate cold-index latency metrics; queries do not rebuild their scope.

Binding-group admission computes one bounded aggregate of reachable integer rowids and the four-hop frontier, with at most 5,001 recursive states. The count, frontier and metadata checks reuse that aggregate instead of repeating the correlated traversal. Equal source/reference identities and self-links do not add redundant work; distinct referenced identities and unknown frontiers retain their existing meaning. Query ranking, final alias resolution and hydration still share the same SQLite execution budget. A metadata intersection proven empty is returned as empty, while a query whose remaining terms cannot be established within the seed budget still reports incomplete analysis.

Adjacent code-repository integration-test registrations are kept compact to meet the existing file-length limit; module paths, fixtures and assertions remain unchanged.

Suspension through `await` or yielding and names bound by match capture patterns can invalidate an earlier overload import. Empty built-in tuple/list/dictionary delegation does not suspend. Unknown or custom decorators preserve executable classification: canonical selectors reject multiple executable definitions, while a symbol snapshot selector chooses a specific definition and its existing call edges.

Storage unit tests that call the internal feature-flag matcher directly must establish the same deferred query indexes required by public query admission. Their fixtures first verify that the key-index prerequisite is missing, then use the existing test-only index builder and verify readiness before exercising the unchanged records, filters, result assertions and execution budget. Missing-index behavior remains covered by public rejection tests; this test setup does not add query-time index creation or change production budgets.

Canonical callers/callees admission counts callable symbols only: classes, constructors, functions, callable declarations, macros, and methods share the existing 1,024-candidate limit. Repeated variables or constants with the same canonical identity do not make a sole function ambiguous. Multiple real definitions still require an explicit snapshot, and scanning remains bounded by the existing query execution budget.

The Java clone byte-budget integration fixture must publish its base through the public checkpointed session API and verify a completed, nonzero durable fact-row proof before attempting the fenced overlay. It keeps the same file and row limits for both scopes and a 61,440-byte target quantum; only the base byte quantum is larger. The assertion must identify the immutable-base byte-quantum rejection and confirm that no target facts were written. A missing-proof fallback is a separate behavior and must not satisfy this budget-specific assertion.

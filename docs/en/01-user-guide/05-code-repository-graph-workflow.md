...
Symbol hits also include `canonical_symbol_id` for expressing logical symbol identity across snapshots. Reference, call, import, and SBOM hits return `edge_kind`, `edge_resolution_state`, `edge_target_hint`, `edge_confidence_basis_points`, and `edge_confidence_tier`. Uniquely unresolved targets are marked `unresolved` or `ambiguous` instead of being written as certain calls. `repo query --kind sbom` returns dependency declarations and locked packages extracted during indexing from `Cargo.toml`, `Cargo.lock`, `package.json`, `package-lock.json`, `go.mod`, `go.sum`, `pyproject.toml`, `requirements*.txt`, Maven `pom.xml` BOM imports, Gradle dependency blocks, and Conan `conanfile.txt` or common `conanfile.py` declarations. It handles common Python PEP 508 markers, editable Python direct references, Cargo rename syntax, and Gradle map-style notation; it de-duplicates `go.sum` module and `/go.mod` pairs, skips local Cargo path/workspace packages, local npm `file:`/`link:`/`workspace:` specs, npm workspace lock rows, and local Python/Poetry path dependencies, and treats Maven `dependencyManagement` entries as SBOM records only for imported BOMs. It does not execute package managers, resolve transitives, contact registries, or provide vulnerability/license analysis. If an import points at an unresolved external dependency that is not indexed as a code graph target, `repo query --kind imports` and repository-set import queries may run the bounded internal grep fallback over the current indexed repository source. The grep search is derived from the unresolved target hint and ranks after structured import-graph evidence, so agents can still inspect `edge_resolution_state` and `edge_target_hint` when limits are small. Fallback hits carry `text_fallback`; missing dependency source remains unresolved edge coverage metadata and does not set `degraded_reason` unless the fallback itself fails.

...
`expected_degraded_files` returns the first 50 distinct degraded files in snapshot traversal order, each with `path` and `reason`. Reasons come directly from the same parser diagnostics as full indexing, including syntax errors, invalid UTF-8, binary content, unsupported grammars, and oversized files. Missing external dependency source alone is not degradation. On finding a 51st degraded file, `expected_degraded_files_truncated` becomes `true` and preview stops requesting subsequent parser batches; the current bounded batch may already have parsed more files. Exactly 50 findings still require checking the remaining scope. A successful untruncated result covers all selected files. A truncated list supplies no total and does not establish that remaining files were checked.

`excluded_paths` likewise returns at most the first 50 paths and their exclusion reasons. Observing a 51st exclusion sets `excluded_paths_truncated` to `true`. Scope enumeration continues, so selected file counts, bytes, language distribution, and other scope statistics remain complete. Both arrays and their independent boolean flags are always returned; empty results use `[]` and `false`. Text/Markdown output also displays paths, reasons, and truncation notices; JSON and streaming JSON share the same structure.

Preview reads and parses selected source files without writing index facts, tasks, or checkpoints. The application admits at most two preview workers, waits up to five seconds for admission, and applies a 120-second response deadline. Cancellation, timeout, or an observed source read failure still returns an error, not partial success; the current blocking batch finishes before its permit is released and further work stops. Scope enumeration still costs work; only finding more than 50 degraded files saves subsequent parsing. Full indexing always completes its normal workflow regardless of preview list limits. Compare diagnostics against the same resolved ref and scope; incremental summaries and worktree overlays may cover different file sets.

Interface migration: `expected_degraded_file_count` is removed without a compatibility alias. Clients must read `expected_degraded_files` and its truncation flag rather than treating a truncated array length as a total. Upgrade or roll back remote CLI and server together. No persistent data migration is required.

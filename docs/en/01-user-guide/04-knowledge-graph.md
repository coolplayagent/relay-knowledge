...
The JSON response contains display-compatible `results` and an agent-oriented `context_pack`. For auditable references, prefer `context_pack.items[*].ranking`, `graph_facts`, `graph_paths`, `source_span`, `backend_statuses`, `budget_used`, `truncated`, and `degraded_reason`.

...
- Competitive code search combines exact symbols, trigram/regex, BM25, AST structure, reference/call/import edges, bounded internal exact-text fallback, language and path filters, revision scopes, and impact analysis instead of relying on code embeddings alone.
...
- **Candidate narrowing**: use inverted lists, trigrams, path tokens, symbol names, and scope/path/language filters to reduce candidates to bounded windows before expensive scoring. Code source fallback must reuse this bounded candidate window instead of starting an ad hoc full-repository scan.
...
| P1 | Include code source fallback in retrieval traces: trigger reason, candidate-file count, materialized bytes, candidate/budget degraded reason, and `text_fallback` rank contribution. | Definition/reference/hybrid fallback hits are explainable and do not outrank exact symbols or resolved edges. |

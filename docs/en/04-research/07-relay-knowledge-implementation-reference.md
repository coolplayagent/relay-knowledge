...
- `code` 和 `application::code_service` 已经实现 Git 仓库注册、clean snapshot 索引、增量 diff、worktree overlay、Tree-sitter 多语言解析、代码图检索、query-time `ripgrep` 精确文本兜底和 diff impact。
...
- grep 兜底边界: 只在 definition/reference/hybrid 结构化召回不足时搜索 indexed commit 候选内容，并把命中标记为 `text_fallback`，不生成 resolved edge。
...
| 代码图 | 已有 tree-sitter 多语言索引、scope metadata、path/language filter、报告、query/impact、MCP tools 和有界 `ripgrep` text fallback | 多仓库联邦调用解析和更大的真实性能报告仍待扩展 | P1 |

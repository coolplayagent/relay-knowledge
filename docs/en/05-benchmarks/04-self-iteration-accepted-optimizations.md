...
- Guardrails: the default fast `c_syntax_fixture` includes unresolved `ngx_*` headers, a `KONG_ACCESS_PHASE` handler, typedef-style module tables, symbol/definition/callee/import cases, and no repository/path/query special casing. Local macro lookup now accepts spaced `# define` directives and backslash continuations, evaluates bounded numeric `#if` conditions, and treats `#undef` plus inactive branches as unavailable so stale macro bodies cannot create caller ownership.

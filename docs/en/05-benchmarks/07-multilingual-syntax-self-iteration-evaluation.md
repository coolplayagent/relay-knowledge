...
- Except for the C/C++, cross-language, TypeScript, `nonstandard_layout_fixture`, and `grep_budget_fixture` cases already covered by the default fast profile, generated fixtures are not added to the normal fast repository list by default. `nonstandard_layout_fixture` now includes no-path-filter `external_deps/` and `modules/` definition guardrails so index creation and incremental paths cannot regress to a top-level-`src` assumption. `grep_budget_fixture` creates more than 256 C files and requires a no-path-filter `references` query to recover a late-sorted target through query-aware fallback candidates without source-fallback candidate budget exhaustion. Run targeted checks with `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPOS`, for example:

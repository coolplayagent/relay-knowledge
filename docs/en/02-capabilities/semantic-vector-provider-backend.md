...
[English](./semantic-vector-provider-backend.md) | [中文](../../zh/02-capabilities/semantic-vector-provider-backend.md)

This is the English documentation page for `semantic-vector-provider-backend.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.

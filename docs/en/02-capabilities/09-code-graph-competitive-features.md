...
The code graph detects web framework route handler bindings during indexing. Supported frameworks include Express (JavaScript/TypeScript/JSX), Flask/FastAPI (Python), and Spring (Java). The detectors cover real Express app/router calls, router alias assignments, router mount prefixes, `route()` chains including multiline chains, member-expression handler links, multiline route registrations, commented-out registration filtering, `get/post/put/delete/patch/head/options/all` Express methods, async inline callbacks recorded as anonymous handlers, Express middleware chains with a final endpoint handler, stacked and multiline Flask/FastAPI decorators, Flask Blueprint `url_prefix` values including multiline declarations, Flask tuple or list `methods` declarations, FastAPI `APIRouter(prefix = ...)` prefixes including multiline declarations, FastAPI/Flask `head` and `options` decorators, multiple Spring class-level `@RequestMapping` prefixes, multiline Spring mapping annotations, unannotated Java class prefix resets, order-independent `value`/`path` attributes, path arrays, `RequestMethod` arrays, and methodless Spring mappings recorded with `http_method = "any"`. Each detected route produces a `CodeRouteRecord` with HTTP method, URL path, handler name, framework identifier, source location, and a handler symbol link when the parser can match the route to a structured symbol.

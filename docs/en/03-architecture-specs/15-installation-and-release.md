# Installation and Release Specification

[English](../../en/03-architecture-specs/15-installation-and-release.md) | [中文](../../zh/03-architecture-specs/15-installation-and-release.md)

This is the English documentation page for `03-architecture-specs/15-installation-and-release.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
4. **后台服务安装是一等能力**: 当前 CLI 已落地 `service plan install|uninstall`、`service definition write`、`service status|doctor` 和 `setup profile service`；后续安装器必须用这些只读/写定义入口执行 systemd、Windows Service 或 launchd 托管，而不是另起未受管 loop。
...
relay-knowledge.exe setup doctor
```
...
relay-knowledge service doctor
relay-knowledge service plan uninstall
```

当前 CLI 已先落地 `relay-knowledge service status|doctor` 的统一 API 输出，
用于报告服务名、默认 disabled 模式、后台更新默认关闭和平台服务定义路径。
当前 CLI 还提供 `relay-knowledge setup doctor` 和
`relay-knowledge setup profile service`，用于安装前检查 runtime readiness 并输出
service profile 的推荐命令。
当前 CLI 也提供 `relay-knowledge service run --web --mcp streamable-http` 前台服务入口，
...
[`基础运行时层规格`](03-foundational-runtime.md) 记录 `RELAY_KNOWLEDGE_HOME`、
逐项路径覆盖、HTTP bind 和 QoS 预算等环境变量。当前
`relay-knowledge service plan|definition|doctor`、安装器和后续 privileged
`service install|uninstall` 必须复用同一套路径解析结果，
不能重新实现目录规则。

## 4. 后台服务部署

后台服务部署必须沿用 [后台服务、静默更新与自愈设计](13-background-service-and-self-healing.md) 的约束:
...
当前 CLI 提供 `service plan install|uninstall` 和 `service definition write`。
安装器或后续 privileged `relay-knowledge service install` 至少提供:
...
- 二进制绝对路径。
- 前台服务命令，例如 `relay-knowledge service run --web --mcp streamable-http`。
- 配置、数据、缓存和日志目录。
...
- SQLite 运行时升级如果发现旧的 BM25 或结构化事实读模型，必须在启动兼容性检查中从持久图事实和代码图表重建派生行，不能要求用户重新导入数据。

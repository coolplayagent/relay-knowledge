...
- Compatibility relationships (`depends_on`, `uses_sdk`, `configures`, `references`, `documents`) are bounded same-snapshot joins over component, SDK, configuration and topic facts. Schema 8 stops copying these redundant payloads into `software_relationships`. SQL applies scope and request filters before the result limit; stable IDs, source/target identities, evidence, confidence and unresolved hints remain available. Configuration usages sharing the same relationship identity are collapsed by highest confidence, then widest end line, then usage ID; different relationship kinds remain distinct. The durable relationships phase stores the exact join count without allocating all edges. Typed ontology statements remain persisted because they carry reconciled provenance and fact state.

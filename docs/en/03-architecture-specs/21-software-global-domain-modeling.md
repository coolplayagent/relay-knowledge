...
- Compatibility relationships (`depends_on`, `uses_sdk`, `configures`, `references`, `documents`) are bounded same-snapshot joins over component, SDK, configuration and topic facts. Schema 8 stops copying these redundant payloads into `software_relationships`. SQL applies scope and request filters before the result limit; stable IDs, source/target identities, evidence, confidence and unresolved hints remain available. Configuration usages sharing the same relationship identity are collapsed by highest confidence, then widest end line, then usage ID; different relationship kinds remain distinct. The durable relationships phase streams every joined fact through the same domain validation as reads, including losing configuration duplicates, and stores the selected edge count. It retains only one edge at a time and never writes compatibility rows; invalid fields abort before fresh publication. Typed ontology statements remain persisted because they carry reconciled provenance and fact state.

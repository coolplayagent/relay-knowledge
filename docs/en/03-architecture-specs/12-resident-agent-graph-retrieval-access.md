# Resident Agent Graph Retrieval Access

[English](../../en/03-architecture-specs/12-resident-agent-graph-retrieval-access.md) | [中文](../../zh/03-architecture-specs/12-resident-agent-graph-retrieval-access.md)

This is the English documentation page for `03-architecture-specs/12-resident-agent-graph-retrieval-access.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
协议基线以 MCP Streamable HTTP 2025-11-25 为准；A2A 作为后续 agent-to-agent gateway 方向记录在 [开放 Agent Runtime 与混合检索架构](06-open-agent-runtime-and-hybrid-retrieval-architecture.md)。
...
| --- | --- | --- |
| `relay_retrieve_context` | enabled | `HybridRetrievalRequest` -> `retrieve_context` |
| `relay_inspect_graph` | enabled | `GraphInspectionRequest` -> `inspect_graph` |
| `relay_health` | enabled | `health` |
| `relay_service_status` | enabled | `service_status` |
| `relay_index_status` | enabled | `health` response 中的 index status projection |
| `relay_code_query` | enabled | `CodeRetrievalRequest` -> `query_code_repository` |
| `relay_code_impact` | enabled | `CodeImpactRequest` -> `impact_code_repository` |

`relay_retrieve_context` input schema:
...
- MCP `tools/list` contains only ASCII alphanumeric plus `_` tool names and never advertises index refresh or repository indexing tools.
...
- index refresh and repository indexing require explicit CLI/Web workflows outside MCP.

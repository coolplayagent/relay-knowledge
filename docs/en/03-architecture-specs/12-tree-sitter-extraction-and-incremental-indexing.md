...
Incremental finalization expands affected paths when symbol cardinality changes, using the same cross-language call-name candidates as call resolution. A file/module-set change reruns import finalization for the complete scope because unchanged side-effect imports may have no reference row; other edge phases remain path-scoped unless their affected-path threshold requests a full fallback.

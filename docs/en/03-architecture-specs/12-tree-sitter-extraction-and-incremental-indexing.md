...
When an explicit `Full` Git initialization opts into historical reuse and targets a scope that is not fresh, it walks at most 10 ancestors on the immutable target commit's first-parent chain. The application derives the target plan's effective filters, including discovered source roots, then queries commit-to-scope aliases from nearest to farthest and accepts only a published scope that is not stale or retiring, exactly matches those cloneable canonical path/language filters, and matches the current code-fact version. A match pins the request as a real `Incremental` task and exposes the base/head through task mode and the completion summary while preserving the caller's symbolic `requested_ref` separately from the immutable execution SHA. An unfinished full or incremental task for the same target and requested scope is reused first. If the candidate-to-target diff exceeds the 100 impacted-path initialization-reuse limit, counting both old and new paths for renames and copies and expanding selected gitlink children before the final count, the planner does not try older ancestors and falls back directly to the checkpointed full index. If retention retires the selected base before preflight or atomic task admission pins it, the automatic reuse attempt likewise falls back to the full task; explicit incremental requests continue to report an unavailable base. Requests without the opt-in, filesystem sources, and histories without a matching base remain on the full path. Explicit incremental updates retain their separate 512 changed-path limit. Ancestor enumeration, target filter discovery, diff preflight, and later parsing stay behind the blocking-worker boundary, and Git probe failures must not be silently treated as “no base.”

Incremental finalization derives candidate symbol names and file-set changes from the bounded changed/deleted paths, then compares indexed per-name cardinality, location, and callable kind/signature distributions. Exact references use the scoped name index; references whose cloned target ID belongs to a symbol in a replaced base file use the scoped target-ID index; alias-bearing calls and symbol-dependent imports, including TypeScript `export ... from` re-exports, use bounded FTS candidate queries followed by the same cross-language call-name and TypeScript/Python/Java import parsers as final resolution. This includes unchanged imports whose local alias differs from the changed exported symbol name and unchanged references whose target symbol kept the same shape but received a new snapshot identity, without materializing complete symbol, reference, or import catalogs. Reaching either the unique affected-path cap-plus-one limit or a raw SQL candidate-row limit promotes finalization to the complete scope instead of publishing a partial or deceptively deduplicated candidate set. A file/module-set change likewise promotes all edge finalization to the complete scope because unchanged side-effect imports may have no reference row and newly resolved imports can change downstream reference, call, and search projections. A filtered delta with no selected changed or deleted paths publishes the cloned scope without rerunning edge finalization. A partial checkpoint resumes only when its persisted staging identity matches the same durable task (or the same unfenced full/base-to-target session); a different incremental base resets the target and re-clones before accepting batches.

The resident FileWatcher treats `.git/HEAD`, ref, packed-ref, and HEAD-log events as low-latency hints. At startup and on a bounded periodic interval (default 5000 ms), it independently resolves the checked-out HEAD/tree; this reconciliation covers linked worktrees and missed/coalesced events. An advanced HEAD is pinned with the last published clean base into a durable task. A stable fingerprint per repository, checked-out ref, and filter set coalesces repeat hints while the slot is unfinished. Queue admission is transactional and capped at 32 unfinished tasks per repository and 256 globally. The attempt-scoped lease and a monotonically advancing publication generation remain the single-writer authority; every snapshot, batch, workspace, and software-projection transaction validates the live generation before commit, so commit events and detached expired attempts cannot bypass bounded retry/backoff, recovery, or dead-letter state. Full and pre-parsed incremental plans share the durable begin/batch/finalize checkpoint lifecycle: completed batches resume idempotently, finalization exposes its current phase, and publication remains fenced and atomic.

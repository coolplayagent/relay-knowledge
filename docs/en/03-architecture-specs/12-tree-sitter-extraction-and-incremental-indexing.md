...
Incremental finalization expands affected paths when symbol cardinality or callable kind/signature metadata changes, using the same cross-language call-name candidates as call resolution and the same TypeScript, Python, and Java symbol-dependent import parsers as import finalization. This includes unchanged imports whose local alias differs from the changed exported symbol name. A file/module-set change promotes all edge finalization to the complete scope because unchanged side-effect imports may have no reference row and newly resolved imports can change downstream reference, call, and search projections.

...
Incremental finalization derives candidate symbol names and file-set changes from the bounded changed/deleted paths, then compares indexed per-name cardinality, location, and callable kind/signature distributions. Exact references use the scoped name index, while alias-bearing calls and symbol-dependent imports use bounded FTS candidate queries followed by the same cross-language call-name and TypeScript/Python/Java import parsers as final resolution. This includes unchanged imports whose local alias differs from the changed exported symbol name without materializing complete symbol, reference, or import catalogs. A file/module-set change promotes all edge finalization to the complete scope because unchanged side-effect imports may have no reference row and newly resolved imports can change downstream reference, call, and search projections. A filtered delta with no selected changed or deleted paths publishes the cloned scope without rerunning edge finalization.

...
[English](../../en/03-architecture-specs/11-unified-api-and-interface-architecture.md) | [中文](../../zh/03-architecture-specs/11-unified-api-and-interface-architecture.md)

This is the English documentation page for `03-architecture-specs/11-unified-api-and-interface-architecture.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
Web v1 采用轻量 TypeScript 静态前端。Web 是交互层，不是第二套业务后端。
当前 Web workspace 工程位于 `web/`，通过前端 typed
...
Web client 必须从同源服务 API 读取 `/api/project/status`、`/api/health` 和
`/api/web/graph/canvas`，不得在前端
伪造健康状态、图版本、运行时路径、图节点边或索引元数据。当前 Web 页面从这些 contract 派生
Status、Graph、GraphRAG readiness、Indexes、Runtime 和操作工作台状态；GraphRAG readiness 只
...
Graph canvas endpoint 必须是只读 bounded snapshot: `kind=knowledge|code|mixed`、
可选 `scope`、可选 `query` 和 `limit`。默认 limit 为 250，最大为 1000。Knowledge
视图展示事实图，Code 视图展示代码结构图，Mixed 作为高级视图合并两者并仅加入可由
source scope 或 source path 推导的跨图边。前端可以保存交互状态，但不得从画布写入图事实、
索引或布局。
...
4. Web 工程使用轻量 TypeScript 静态前端和手写 typed client。Graph canvas 调用同源 `/api/web/graph/canvas`；Web Operations 的 `Run` 调用同源 `/api/web/operations/execute`，由 Rust Web adapter 分发到同一 application service。
...
`/api/web/graph/canvas` 当前支持 knowledge、code 和 mixed 三种只读画布。`/api/web/operations/execute` 当前支持 retrieve context、ingest evidence、graph inspect、index refresh、provider probe、worker status/run-once、proposal list/show/accept/reject/supersede、audit query、code repository register/index/update/query/impact/status，以及 service status/run snapshot。`service.run.streamable_http` 在 Web 中返回服务状态快照，不从浏览器启动常驻 loop。

...
`map validate` requires both reserved sources with those exact fields and requires each source to remain a member of its reserved topic route. Additional ordinary sources under either topic remain valid and are checked by their own source contract; they are not interpreted as business glossary documents. The visible v3 glossary source must use its canonical URI. A writer-locked `map init` treats the previously accepted legacy glossary URI in either a legacy or visible-v3 contract as a migration input, publishes the canonical shard, and leaves read-only visible-v3 validation strict. Source removal cannot delete either reserved entry. When `map source add` creates a Knowledge Map for the first time, it domain-validates the complete in-memory mutation before creating writer-lock or contract files, then creates the governed baseline README files and a missing minimal glossary before publishing the root. A rejected first-add request leaves neither visible, legacy, nor CodeSpec map artifacts behind. Every controlled source add, update, or removal serializes legacy-state discovery with the current-path lock; when legacy state is active, it acquires locks legacy-then-current and first validates the requested in-memory mutation against the live legacy snapshot. Every forward cycle replaces the verified rollback backup with the exact active legacy root and refreshes the canonical glossary from a validated active legacy glossary when present; only then may it repair missing built-in routes in the new visible v3 publication and install the legacy redirect. If the live legacy root diverges from that migration backup after v3 publication, redirect convergence fails closed and preserves those edits. A rejected legacy mutation therefore cannot publish migration state. Rolling back may restore a structurally valid older contract that current `map validate` reports as missing required routes until an explicit `map init` upgrades it again.
...
All generated refs are restricted to designated real directories under the selected map root; absolute paths, `..`, symlink/reparse escape, and mismatched map type/path are rejected. Multi-file mutations publish immutable artifacts first and the root last. V2 migration copies retained assets before publishing `knowledge/knowledge-map.yaml`; it replaces `.knowledge/knowledge-map.yaml` with a v3 redirect that old readers reject explicitly only after the visible v3 root and its artifacts validate. Redirect recovery compares a remaining live or staged legacy root to the exact migration backup and fails closed rather than overwriting edits made after v3 publication. Legacy topic and history copying enforces per-file, aggregate-byte, and file-count budgets before loading content. While rollback has prepared but not published the legacy root, readers use the retained v3 root and its contract directory. Rollback promotes an ordinary fallback-only root before moving it to retained recovery state; a repeated rollback first reconciles a committed transition, and once a clean rollback is active it validates and retains the live legacy root instead of restaging an older backup. The CLI retains a verified v2 root for `map migrate --type knowledge --rollback`; uninstall never deletes repository-owned map content.

`map show`, `map route`, `map validate`, and `map history` are strictly read-only boundaries. Legacy v1 readers normalize only the historical glossary URI before validation, leaving visible-v3 validation canonical. When an early v2 root has archived history but no range index, `map validate` reports that explicit `map init` is required and must not create index nodes or rewrite the root; `map route` verifies that the root `map_type` agrees with its visible path before reading the requested shard. Only an explicit initialization or migration path holding the writer lock may add the history index.
...
| KDL-13 | Every source mutation preserves reserved routes and the legacy rollback boundary | Application tests cover reserved removal plus legacy-only add, update, remove, validation, and exact-byte rollback |

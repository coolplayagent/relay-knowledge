...
Knowledge Map v2 keeps only topic summaries, each topic's ordered source-id summary, content-addressed shard refs, map version, and at most 16 recent history entries in the root manifest. The root summary rejects source ids duplicated across topics before any shard is loaded. Topic sources/routes live under `.knowledge/topics/`; complete history beyond that window lives in a content-addressed archive under `.knowledge/history/`. `map route <topic>` loads and verifies only the root and requested shard, while full `map show`/`map validate` and every mutation load all shards and verify digests, history continuity, and the archive checkpoint; a mutation never trusts an archive head it has not read. V1 single-file maps remain readable and migrate losslessly during `map init` or the next controlled mutation.

All shard/archive refs are restricted to designated real directories under `.knowledge/`; absolute paths, `..`, directory symlinks to another repository location, and symlink escape are rejected. Multi-file mutations share one repository writer lock, append only newly completed history chunks, publish immutable content-addressed artifacts first, and publish the root manifest last; failure restores the previous valid root. A successful publication retains the preceding recovery manifest and its refs, then applies a minimum 60-second grace period before best-effort cleanup of older superseded topic shards so a concurrent reader that already loaded an older root can finish loading its shards. The map remains stable navigation only and must never contain snapshot-bound code, build, IaC, framework-scan, or design projection facts.

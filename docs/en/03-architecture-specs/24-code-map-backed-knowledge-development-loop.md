...
| Repository maps | Typed CodeSpec/Knowledge directories plus topics, sources, routes, bounded history, and software-model entry point | `codespec/codespec-map.yaml`, `knowledge/knowledge-map.yaml`, `knowledge/topics/` | `schema_version`, `map_type`, `map_version`, SHA-256 digest |
...
`map validate` requires both reserved sources with those exact fields and requires each source to remain a member of its reserved topic route. Additional ordinary sources under either topic remain valid and are checked by their own source contract; they are not interpreted as business glossary documents. The visible v4 glossary source must use its canonical URI. A writer-locked `map init` treats the previously accepted legacy glossary URI as migration input, publishes the canonical shard, records the durable change as `source.migrate` rather than history compaction, and leaves read-only visible-v4 validation strict. Source removal cannot delete either reserved entry. When `map source add` creates a Knowledge Map for the first time, it domain-validates the complete in-memory mutation before creating writer-lock or contract files, then creates the governed baseline README files and a missing minimal glossary before publishing the root. A rejected first-add request leaves neither visible, legacy, nor CodeSpec map artifacts behind. Every controlled source mutation serializes legacy-state discovery with the current-path lock and validates the requested in-memory mutation against any live legacy snapshot before publishing v4 and installing the legacy redirect. A rejected legacy mutation cannot publish migration state.

Repository Map v4 keeps the strongly typed `directories` contract in both visible roots. `codespec` requires `requirements`, `design`, `api`, `test`, and `decisions`; `knowledge` requires `domain`, `guides`, `ops`, `glossary`, and `best-practices`, while custom confined directories remain extensible. Entries carry purpose, content scope, key files, load policy, typed qualified relations, and update policy. Knowledge Map keeps topic summaries, ordered source ids, content-addressed shard refs, map version, an `omitted_through` checkpoint, and at most 16 recent history entries in its root. Topic sources/routes live under `knowledge/topics/`; neither map creates a history archive directory. `map route <topic> --type knowledge` loads one shard; aggregate reads default to both maps, while targeted writes require a concrete type. A v4 history rejects the known legacy `archived_through`, `archive`, and `index` keys by presence in the CLI, bundled schemas, and code-index fact extraction, while unrelated future fields remain allowed. Root and referenced topic-shard schema versions must match. Cross-file digests, directory existence, relationship targets and cycles, recent-history continuity, path confinement, and reserved routes are enforced by `map validate` rather than JSON Schema alone.

All generated refs are restricted to designated real directories under the selected map root; absolute paths, `..`, symlink/reparse escape, and mismatched map type/path are rejected. Multi-file mutations publish immutable artifacts first and the root last. Legacy migration validates referenced topic and history assets before publishing `knowledge/knowledge-map.yaml`; it replaces `.knowledge/knowledge-map.yaml` with a v4 redirect only after the visible v4 root and its artifacts validate. Before retiring history artifacts, migration republishes until both current and `.previous` reader roots are v4, then waits a 60-second reader grace before bounded deletion. Cleanup accepts only strictly named regular archive/index files and its owned grace marker inside the contract directory, processes at most 1,024 archives per deletion attempt, rejects links and unknown entries, and resumes through `map init`. Long-term audit and rollback use Git or repository backups. Uninstall never deletes repository-owned map content.

`map show`, `map route`, `map validate`, and `map history` are strictly read-only boundaries. Legacy v1 readers normalize only the historical glossary URI before validation, leaving visible-v4 validation canonical. `map validate` verifies a legacy archive chain without rewriting it. Only an explicit initialization or migration path holding the writer lock may publish v4 and remove the verified archive tree. In v4, omitting `map history --from` starts at `omitted_through + 1`; an explicit older version fails rather than synthesizing or searching unavailable history, and each response is capped at 16 entries.

Storage maintenance does not require a new map version. After field validation and text normalization, `map source update` preserves the source version, recent history, shards, and root files when the source, reserved routes, and schema need no change. Required legacy migration or reserved-route repair still publishes. `map init` also runs shard reclamation without another content write: unreferenced shards become eligible after their 60-second retirement grace. Each attempt processes at most 1,024 unreferenced regular shards, protects references from current, previous, and other recovery manifests, and preserves unknown files and links. CodeSpec maintenance does not create an empty topics directory. Long-term recovery uses Git or backups.

The self-iteration fast `map_storage_regression_cases` gate requires 32 equal updates to add zero files and bytes while preserving mtimes. It also covers idle init reclamation, young retirement markers, recovery-root retention, bounded resumable batches, and required migration despite equal source content. Byte measurements use a deterministic fixture and do not claim whole-repository SQLite compression ratios.

The staging-lock cleanup test explicitly unlocks before asserting reclamation, matching the production guard's release protocol. Parallel tests spawning children cannot assume that dropping a descriptor immediately releases a lock.
...
| KDL-11 | Validation requires both exact reserved routes while allowing ordinary sibling sources | Application tests remove each reserved shard independently and add a non-glossary business source |

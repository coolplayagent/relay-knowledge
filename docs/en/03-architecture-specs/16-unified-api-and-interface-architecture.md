...
New control-plane capabilities first define shared `api` request/response types and application service methods, then map to CLI, Web, MCP, or HTTP routes. Existing same-origin Web operations continue to use `/api/web/operations/execute`; future stable external control-plane HTTP APIs must be versioned, for example under `/api/v1/control/*`, while keeping CLI JSON, Web, and MCP tool semantics compatible.

...
Route-detection syntax helpers are owned by the framework or lexical layer they interpret. `detect::express_arguments` owns quoted Express paths, top-level argument splitting, middleware-tail selection, callback arrays, and named-handler validation; `detect::python_strings` owns static Python string prefixes and escape handling; and `detect::javascript` owns JavaScript comment/string/regex lexical state. Each owner attaches a focused sibling test file. A generic `detect::shared` module is forbidden because JavaScript callbacks and Python strings do not share one semantic contract.

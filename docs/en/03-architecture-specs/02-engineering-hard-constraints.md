...
The MCP adapter is physically grouped under `interfaces::agent::mcp`: `mod.rs` owns server composition, streamable-HTTP/JSON-RPC dispatch, QoS admission, cancellation, and shared tool-result mapping, while named child modules own audit, protocol, resources, prompts, state, authorization, registry, and code-tool behavior. Root adapter tests stay beside that facade as `mod_tests`, `protocol_tests`, `tool_tests`, `software_tool_tests`, `feature_flag_tool_tests`, and `runtime_guardrail_tests`; reusable test storage and HTTP transport fixtures are explicitly named `test_support` and `transport_harness`. Do not restore root-level `mcp_*` files or issue-number test module names outside this ownership boundary.

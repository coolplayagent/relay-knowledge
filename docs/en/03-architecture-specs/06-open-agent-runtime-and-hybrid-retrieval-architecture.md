# Open Agent Runtime and Hybrid Retrieval Architecture

[English](../../en/03-architecture-specs/06-open-agent-runtime-and-hybrid-retrieval-architecture.md) | [中文](../../zh/03-architecture-specs/06-open-agent-runtime-and-hybrid-retrieval-architecture.md)

This is the English documentation page for `03-architecture-specs/06-open-agent-runtime-and-hybrid-retrieval-architecture.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
行业协议和检索能力刷新见 [2026 行业能力快照与差距分析](../04-research/01-industry-capability-snapshot-2026.md)。本文件保留 `relay-knowledge` 作为 knowledge substrate 的定位，不因 A2A 或 agent framework 生态成熟而把 core 改成 agent runtime。
...
[常驻进程 Agent 图检索访问规格](12-resident-agent-graph-retrieval-access.md)。
...
- Worker 或 agent 生成的 proposal 必须持久化 provenance，包括 producer、provider/model、prompt id/version、schema version、input source hash、input fact ids、stale 条件和预算说明。
- 外部 extractor 返回的 relation、claim 和 event 默认归一为 `proposed` fact status，即使 worker payload 声明为 `accepted`，也不能绕过 proposal/approval 流程直接成为 accepted graph fact。
...
- Proposal 审核页面和 agent artifact 必须显示 provenance 和 stale 条件，避免 reviewer 只看到模型生成的自然语言摘要。

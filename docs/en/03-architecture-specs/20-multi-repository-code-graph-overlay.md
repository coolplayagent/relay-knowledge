...
- `repo-set refresh` builds import/module-level cross-repository overlay edges with resolved or ambiguous states plus evidence JSON. An external import with no member export candidate remains authoritative unresolved metadata in its source repository instead of being duplicated into the set overlay. Local, relative, or already resolved member imports likewise stay inside their member repository and are not resolved through cross-repository symbol-name or basename fallback.
...
SQLite projects the import-origin path from evidence JSON into a safe virtual column and indexes both `(set, origin scope, origin kind, origin path)` and `(set, target scope, target kind, target id)`. Candidate projection must use these identity indexes instead of parsing every overlay edge on the query hot path. Invalid evidence JSON projects a null origin path and remains in storage diagnostics; it must not abort retrieval or gain origin evidence.
...
The current implementation resolves only clear import/module evidence into cross-repository edges. Without package metadata or a unique module match, edges stay `ambiguous` or `unresolved`. Cross-repository call and reference resolution is a future optional language-specific enhancement, not an implicit capability of the current basic multi-repository query path.

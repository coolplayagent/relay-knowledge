...
Schema marker v7 upgrades databases at marker v6 or earlier on their next exclusive open by replacing the 11 legacy `code_repository_retention_activity_*` triggers that use `INSERT OR IGNORE`. The replacement and marker update share the schema-initialization transaction: failure rolls both back, and no indexed facts are rewritten. After stopping every writer, operators can diagnose the upgrade by checking that `relay_storage_schema_state` records version 7 and that no matching trigger SQL in `sqlite_master` contains `INSERT OR IGNORE`; an open failure remains visible as `storage_unavailable`. Binary-only rollback may retain the v7 trigger definitions because their SQL is compatible and fact data is unchanged, but an older binary can rewrite the marker to its own version, so a later forward upgrade reruns initialization. Exact database-state rollback restores the pre-upgrade runtime-database backup, and old and new binaries must never write concurrently.
...
- SQLite graph-store upgrades recognize schema marker v7, atomically replace legacy retention dirty-activity triggers without rewriting indexed facts, expose marker/trigger diagnostics, and preserve a pre-upgrade database backup as the exact rollback boundary.

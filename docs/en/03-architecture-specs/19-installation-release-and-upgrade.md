...
Knowledge Map v2 is a repository-owned, versioned contract rather than platform runtime state. The first `map init` or controlled mutation performed by a new binary migrates the single-file `.knowledge/knowledge-map.yaml` v1 contract under the repository writer lock into a v2 root manifest with content-addressed `.knowledge/topics/` and `.knowledge/history/`; `.knowledge/knowledge-map.yaml.previous` retains the preceding valid root as the recovery boundary. Current writers put a protocol marker in the ignored `.knowledge/knowledge-map.yaml.lock` inode and use that inode only as an OS advisory lock target, so crashes release ownership automatically. An unmarked lock is treated as a possibly live create-new lock from an older binary and is never stolen. Before the first upgrade writer starts, stop managed and ad-hoc old writers; only after exclusive access is established may an operator remove an unmarked legacy lock left by a crashed old process. Before rolling back, stop every current writer and remove the marked advisory-lock inode so the old create-new protocol can acquire its pathname. Never delete either form while an old or current writer may still be active. Before upgrading, back up or commit `.knowledge/` with the other repository sources. Rolling back to a binary that understands only v1 also requires restoring the pre-migration single-file map and moving the v2 shards, archive, and previous file aside; an older binary must not edit a v2 contract. Normal shard cleanup retains the preceding root's refs and gives older shards at least a 60-second grace period. Uninstalling the binary or service never deletes repository-owned `.knowledge/` content; explicit repository-file cleanup relies on version control or a user backup for recovery.

...
Knowledge Map v2 is a repository-owned, versioned contract rather than platform runtime state. The first `map init` or controlled mutation performed by a new binary migrates the single-file `.knowledge/knowledge-map.yaml` v1 contract under the repository writer lock into a v2 root manifest with content-addressed `.knowledge/topics/` and `.knowledge/history/`; `.knowledge/knowledge-map.yaml.previous` retains the preceding valid root as the recovery boundary. Current writers put a protocol marker in the ignored `.knowledge/knowledge-map.yaml.lock` inode and use that inode only as an OS advisory lock target, so crashes release ownership automatically. Reopening that persistent inode uses platform no-follow semantics and accepts only a regular file; symbolic links and Windows reparse points are rejected instead of being followed outside the owned contract directory. An unmarked lock is treated as a possibly live create-new lock from an older binary and is never stolen. Before the first upgrade writer starts, stop managed and ad-hoc old writers; only after exclusive access is established may an operator remove an unmarked legacy lock left by a crashed old process. Before rolling back, stop every current writer and remove the marked advisory-lock inode so the old create-new protocol can acquire its pathname. Never delete either form while an old or current writer may still be active. Before upgrading, back up or commit `.knowledge/` with the other repository sources. Rolling back to a binary that understands only v1 also requires restoring the pre-migration single-file map and moving the v2 shards, archive, and previous file aside; an older binary must not edit a v2 contract. Normal shard cleanup retains the preceding root's refs and gives older shards at least a 60-second grace period. Uninstalling the binary or service never deletes repository-owned `.knowledge/` content; explicit repository-file cleanup relies on version control or a user backup for recovery.
...
Repository-level retention adds `code_repository_retention_jobs`, its `cutoff_publication_generation` watermark, the singleton `code_repository_retention_scans` scheduling cursor, and `code_repository_retention_catalog` revision idempotently during SQLite schema initialization; existing parent jobs and scans receive compatible watermark defaults. The default eligible indexed-repository limit is 10 and can be changed with `RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES` in `1..=i64::MAX`. Candidate discovery advances at most 64 catalog rows per transaction, resumes through the cursor after restart, and restarts when a catalog mutation changes candidate order or eligibility. Upgrade needs no manual data migration: existing indexed repositories are evaluated by later successful publications or maintenance passes, and any selected repository is drained through the existing phased scope GC. Repository rows and aliases remain compatible and are not deleted. Rollback must not run concurrently with a newer binary; an older binary may ignore the additive parent-job, scan, and catalog tables, but it cannot safely coordinate repository retention or reverse scopes already retired by child GC jobs.
The retention-trigger compatibility migration runs before the generic schema-marker fast path on every file-backed database open. A bounded `sqlite_master` existence check detects any of the 11 legacy `code_repository_retention_activity_*` triggers that use `INSERT OR IGNORE`; only a match opens one transaction that drops and recreates the complete trigger set. Failure rolls that trigger transaction back and surfaces `storage_unavailable`, no indexed facts are rewritten, the generic schema marker remains unchanged, and derived retrieval state is not invalidated or rebuilt. After stopping every writer, operators can diagnose the upgrade by confirming that no matching trigger SQL in `sqlite_master` contains `INSERT OR IGNORE`; the existing `relay_storage_schema_state` version should remain unchanged. Binary-only rollback may retain the upgraded trigger definitions because their SQL is compatible and fact data is unchanged. Exact database-state rollback restores the pre-upgrade runtime-database backup, and old and new binaries must never write concurrently.
...
- SQLite graph-store opens run the marker-independent retention-trigger compatibility check, atomically replace legacy dirty-activity triggers without invalidating derived retrieval or rewriting indexed facts, expose trigger diagnostics, and preserve a pre-upgrade database backup as the exact rollback boundary.

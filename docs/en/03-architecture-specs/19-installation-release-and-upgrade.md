...
Repository-level retention adds `code_repository_retention_jobs`, its `cutoff_publication_generation` watermark, and the singleton `code_repository_retention_scans` scheduling cursor idempotently during SQLite schema initialization; existing parent jobs receive the compatible watermark default `0`. The default eligible indexed-repository limit is 10 and can be changed with `RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES`. Candidate discovery advances at most 64 catalog rows per transaction and resumes through the cursor after restart. Upgrade needs no manual data migration: existing indexed repositories are evaluated by later successful publications or maintenance passes, and any selected repository is drained through the existing phased scope GC. Repository rows and aliases remain compatible and are not deleted. Rollback must not run concurrently with a newer binary; an older binary may ignore the additive parent-job and scan tables, but it cannot safely coordinate repository retention or reverse scopes already retired by child GC jobs.

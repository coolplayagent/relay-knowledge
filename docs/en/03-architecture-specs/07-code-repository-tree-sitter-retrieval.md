# Code Repository Tree-sitter Retrieval Specification

[English](../../en/03-architecture-specs/07-code-repository-tree-sitter-retrieval.md) | [中文](../../zh/03-architecture-specs/07-code-repository-tree-sitter-retrieval.md)

This is the English documentation page for `03-architecture-specs/07-code-repository-tree-sitter-retrieval.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
| 已有规格 | 本规格继承的约束 |
| --- | --- |
| `04-source-scope-and-multimodal-ingestion.md` | Git snapshot、changeset、scope 内索引版本和 rebase 隔离 |
| `06-open-agent-runtime-and-hybrid-retrieval-architecture.md` | BM25、semantic、vector、graph expansion 和 RRF 融合 |
| `05-storage-layer-design.md` | GraphStore 是事实真源，IndexStore 是派生读模型 |
| `13-background-service-and-self-healing.md` | 后台服务、静默更新、自愈、dead-letter 和资源预算 |
| `01-engineering-hard-constraints.md` | async-first、无阻塞 hot path、QoS、文件长度、测试覆盖和文档完整性 |

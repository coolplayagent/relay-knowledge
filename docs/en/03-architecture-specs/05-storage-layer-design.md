...
[English](../../en/03-architecture-specs/05-storage-layer-design.md) | [中文](../../zh/03-architecture-specs/05-storage-layer-design.md)

This is the English documentation page for `03-architecture-specs/05-storage-layer-design.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
新 scope 完整保留未变文件，旧 scope 仍可显式查询。启动时 SQLite adapter 会执行兼容
schema migration；过期的派生检索表和刷新队列可以局部重建，graph facts、evidence 和
mutation log 不得被静默删除。

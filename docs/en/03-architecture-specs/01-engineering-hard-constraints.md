...
[English](../../en/03-architecture-specs/01-engineering-hard-constraints.md) | [中文](../../zh/03-architecture-specs/01-engineering-hard-constraints.md)

This is the English documentation page for `03-architecture-specs/01-engineering-hard-constraints.md`. It follows the same structure, examples, commands, and implementation contracts as the Chinese edition so readers can switch languages without changing document location.
...
11. **代码解析禁止无界递归**: AST 遍历、import/symbol/call 抽取和路径图扩展不能使用随输入深度增长的函数递归或无界中间集合。
12. **UT 覆盖率必须大于 90%**: 单元测试行覆盖率必须保持在 90% 以上，不能依赖集成测试掩盖缺失的单元覆盖。
13. **分层测试与 Playwright Chromium 浏览器集成测试必须可用**: 整个项目必须分 UT 和集成测试两层验证；集成测试层必须像 `relay-teams` 一样安装 Playwright Chromium 并运行浏览器集成测试。
...
- CLI 自描述输出必须足以构建参数语法树，并作为解析、帮助、诊断和测试的共同来源。
- 解析错误必须说明已匹配命令路径、期望参数或子命令、最接近的合法用法和可执行修正建议；JSON 调用者必须能从结构化 diagnostic 中读取这些字段。
...
- 让解析器、help JSON、README 示例或错误建议使用互相独立的参数来源。
...
### 8.2 代码解析禁止无界递归和无界中间集合

代码解析必须按低内存、高吞吐和可恢复失败设计。tree-sitter AST 遍历、手写节点抽取、import/symbol/call 解析、路径候选扩展和依赖图扩散不能使用随源码嵌套深度、目录深度或依赖深度增长的函数递归，避免恶意或异常深层源码触发线程栈溢出。

要求:

- 使用显式 `Vec`、`VecDeque`、索引游标或 bounded work queue 表达 DFS/BFS；工作栈容量必须受单文件大小、节点数、队列预算或取消点约束。
- 解析 hot path 先构建小型索引或借用视图，再按候选集合查找；不能为每条 import、reference 或 call 重复全仓库扫描。
- 候选扩展必须有语言级上限，例如 TypeScript 扩展名候选、Java package path、C/C++ include path 和 Python module path 只能生成有限小集合。
- 深层嵌套语法、超多 import 和异常候选必须有单元测试或集成 fixture 覆盖，验证不会栈溢出、不会无界分配，并能按 degraded/ambiguous/unresolved 状态降级。
...
### 8.4 分层测试与 Playwright Chromium 浏览器集成测试门禁

...
`relay-knowledge` 当前已经具备一个可继续演进的知识图谱底座: 统一 API、异步 application service、SQLite 图状态、图版本、结构化事实、索引新鲜度元数据、scoped index cursor、bounded refresh queue、task lease/reconciler 诊断、FTS5 BM25 read model、RRF context pack、Tree-sitter 代码仓库索引、MCP Streamable HTTP、本地 ACP session adapter、CLI/Web 入口、`env`/`paths`/`net` 基础边界和 QoS 配置。它还不是完整 GraphRAG 系统，当前差距集中在 semantic/vector 后端、proposal/conflict lifecycle、安装后的 service manager/silent update operator、持久审计、时间图谱和多模态 evidence。
...
| 图事实模型 | evidence/entity、typed relation、claim/event、confidence、source span、status + graph version | valid time、proposal/conflict lifecycle 和多模态 provenance 缺失 | P0 |
...
- 为 semantic/vector 接入补充 model、dimension、source hash 和 backend-specific cursor 元数据。
...
1. 为 semantic/vector 增加真实 read model、model metadata、scope post-filter 和 backend-specific cursor。
2. 为 proposal/conflict lifecycle、valid time 和 temporal query 补齐 domain/API/storage 语义。

...
`relay-knowledge` 当前已经具备一个可继续演进的知识图谱底座: 统一 API、异步 application service、SQLite 图状态、图版本、索引新鲜度元数据、scoped index cursor、bounded refresh queue、task lease/reconciler 诊断、FTS5 BM25 read model、RRF context pack、Tree-sitter 代码仓库索引、MCP Streamable HTTP、CLI/Web 入口、`env`/`paths`/`net` 基础边界和 QoS 配置。它还不是完整 GraphRAG 系统，当前差距集中在 semantic/vector 后端、复杂事实建模、安装后的 service manager/silent update operator、ACP adapter、时间图谱和多模态 evidence。
...
- `retrieve_context` 已经使用 SQLite FTS5 BM25、graph evidence fallback、code graph documents 和 RRF context pack；semantic retrieval、vector ANN、path retrieval、community summary 和受预算约束的 graph expansion 还没有真实 read model。
- `index_status` 记录了 BM25、semantic、vector 等索引家族的聚合新鲜度；scoped cursor 按 kind/scope/text modality 记录 graph version，`refresh_indexes` 会调度持久化 task、获取 lease、replay mutation log 并更新 cursor。BM25 文档随 evidence/code graph 写入更新；semantic/vector 仍缺真实 backend read model 和 model/dimension 元数据。
...
| 方向 | 当前状态 | 主要差距 | 建议优先级 |
| --- | --- | --- | --- |
| 统一 API | 已有 ingest/query/context pack/status/health/code repo/agent identity API | scoped index metadata、audit metadata 和更细的 context artifact 仍需扩展 | P0 |
| 图事实模型 | evidence/entity + graph version | relation、claim/event、confidence、source span、valid time、proposal state 缺失 | P0 |
| 混合检索 | 有 BM25、graph evidence、code graph documents、RRF 和 context pack | semantic/vector backend、path retrieval、community summary 和 graph expansion 仍缺失 | P0 |
...
- 重新验证 ingest 边界传入的 span、confidence 和 version range，并要求 structured facts 引用 supporting evidence ids。
- 继续增强 `HybridRetrievalResponse` context pack，保留更多实体、关系、路径、source span 和 code graph artifact。
- 从检索候选中排除 `rejected`/`superseded` evidence，避免被废弃事实进入 grounding context。
- 提升 BM25 字段质量，覆盖 entity aliases、source path、doc comment、code symbol 和 code chunk。
- 保持 RRF 融合，新增 semantic/vector adapter trait、backend unavailable metadata 和 scope post-filter。
...
- affected scope 必须覆盖 evidence 移动前后的 scope，以及结构化事实引用的 evidence scope。
...
- 增加 bounded index refresh queue、active lease/attempt guard、retry backoff、lease-expiry dead-letter 和 stale diagnostics。
- 实现 startup/diagnostics reconciler: graph version 领先 index cursor 时补发 refresh，并在超大 scoped backlog 下返回 degraded diagnostics 而不是让 health/status 不可用；显式 refresh/wait-until-fresh 在 queue cap 阻止必要入队时返回错误。
...
- 完善 MCP server adapter，补齐 code graph query、code impact 和 audit log。
...
平台 service install/upgrade/uninstall、持久 task lease/dead-letter、watchdog 集成、
...
1. 扩展图事实模型到 relation/claim/evidence span，同时保持现有 ingest CLI 兼容。
2. 将 index refresh metadata 升级为 scoped cursor，并实现最小 reconciler。
3. 为 semantic/vector 增加 adapter trait、metadata、scope post-filter 和 backend unavailable 降级。
4. 完善代码图 context pack 和 MCP code graph/impact 工具。
5. 建立 GraphRAG evaluation harness，覆盖 stale index、ambiguous entity、多跳、时间和 code impact。

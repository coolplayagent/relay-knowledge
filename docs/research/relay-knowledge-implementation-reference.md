...
- `retrieve_context` 已经使用 SQLite FTS5 BM25、graph evidence fallback、code graph documents、local semantic token read model、local hashed-vector ANN read model、schema path、temporal event、community summary 和 RRF context pack；外部 embedding backend 与真实 graph expansion worker 尚未接入。
- `index_status` 记录了 BM25、semantic、vector 等索引家族的聚合新鲜度；scoped cursor 按 kind/scope/text modality 记录 graph version，`refresh_indexes` 会调度持久化 task、获取 lease、replay mutation log 并更新 cursor。BM25 文档随 evidence/code graph 写入更新；semantic/vector read model 随 evidence 写入记录 model、dimension、source hash、scope 和 graph version metadata。

...
`relay-knowledge` 当前已经具备一个可继续演进的知识图谱底座: 统一 API、异步 application service、SQLite 图状态、图版本、索引新鲜度元数据、FTS5 BM25 read model、RRF context pack、Tree-sitter 代码仓库索引、MCP Streamable HTTP、CLI/Web 入口、`env`/`paths`/`net` 基础边界和 QoS 配置。它还不是完整 GraphRAG 系统，当前差距集中在 semantic/vector 后端、复杂事实建模、scoped index refresh、后台任务恢复、ACP adapter、时间图谱和多模态 evidence。
...
- `interfaces::agent::mcp` 已经实现 MCP Streamable HTTP session、protocol header 校验、tool calls、access policy、QoS admission、cancellation registry 和 index refresh 权限控制。
...
- `index_status` 记录了 BM25、semantic、vector 等索引家族的新鲜度；BM25 文档随 evidence/code graph 写入更新，但 `refresh_indexes` 对 semantic/vector 仍主要是 cursor metadata，尚未调度真实 backend refresh。
...
- MCP Streamable HTTP 已经可用；ACP adapter、审计日志和更完整的 code graph MCP 工具仍待实现。
...
| Agent 接入 | 已有 MCP Streamable HTTP、access policy 和 QoS | ACP adapter、audit log、更完整 code graph tools 仍缺失 | P1 |

...
- 后台服务状态已暴露为 API，foreground `service run` 启动时会执行最小 startup index reconciler；service manager 安装、持久任务表、leases、dead-letter 和 silent update 仍主要停留在规格。
- MCP Streamable HTTP 和本地 ACP session adapter 已经可用；持久 audit sink、MCP resources/prompts 和旧 HTTP+SSE 兼容端点仍待实现。
...
| 后台服务 | 有 status API、foreground service run 和最小 startup index reconciler | 缺 service install、持久 task lease、dead-letter、watchdog、silent update | P1 |

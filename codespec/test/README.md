...
- `knowledge_architecture_route_assembles_ordered_v4_shard_sources` requires targeted v4 shard loading to preserve the reviewed seven-source architecture order.
- `knowledge_history_defaults_to_earliest_retained_recent_entry` requires an omitted start version to select the earliest retained v4 history entry and return one bounded page without a gap or duplicate.

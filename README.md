...
Repository-level retention uses the same phased GC and defaults to at most 10 indexed repositories outside user-managed repository sets, configured by `RELAY_KNOWLEDGE_CODE_INDEX_MAX_INDEXED_REPOSITORIES` in the range `1..=i64::MAX`. Candidate discovery scans at most 64 catalog rows per scheduling transaction and persists a restart-safe cursor when more work remains. Candidate-order or eligibility changes invalidate and restart that cursor, and `repo index-worker` keeps `maintenance_active=true` while a scan is unfinished. It schedules the oldest eligible successful publication through a durable parent job, preserves registration and aliases, allows concurrent/new index work, and protects unfinished task targets/bases plus post-cutoff publications. Automatic-workspace sets are counted; user-managed repository-set members are exempt, including when membership is added after scheduling. `repo status` exposes the parent job; incomplete publication history keeps it pending, and partitioned storage completes it only after untruncated control and shard cleanup converge.

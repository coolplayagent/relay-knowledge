...
Code-index task leases are attempt-scoped: expired running leases are recovered to retry or dead-letter before claim/status paths report them, stale workers cannot complete or fail a reclaimed task, and active workers renew the lease after each committed checkpoint batch. Checkpoints expose `updated_at_ms` in JSON status so operators can distinguish slow progress from a stuck task.

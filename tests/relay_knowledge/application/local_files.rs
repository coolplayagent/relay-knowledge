...
async fn content_query_returns_provenance_and_keeps_path_query_independent() {
    ...
    assert_eq!(content.freshness.state, FileIndexFreshnessState::Fresh);
    assert_eq!(content.freshness.stale_reason, None);
    assert!(content.freshness.content_read_model_cursors.is_empty());

    let fresh_content = service
        .query_file_content(
            FileContentQueryRequest {
                query: "depends database".to_owned(),
                source_scope: Some("local-files".to_owned()),
            ...
            RequestContext::with_ids(
                InterfaceKind::Cli,
                "req-content-query-fresh",
                "trace-content-query-fresh",
            ),
        )
        .await
        .expect("fresh content query should run after indexing");
    assert_eq!(fresh_content.results.len(), 1);
    assert_eq!(
        fresh_content.freshness.state,

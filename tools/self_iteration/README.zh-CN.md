...
默认 profile 是 `fast`。它只跑产品与 harness 的 `fmt --check`，检查 release workflow 仍强制 Linux GNU glibc 2.31 baseline，再跑产品 debug build、harness `cargo check` 和 targeted `skill_metadata_policy_cases`、`code_index_recovery_cases`、`code_index_sqlite_lock_cases` gate，然后用 `target/debug/relay-knowledge` 执行评估；默认不跑产品 release build、全量 clippy、全量 test、本地文件 fixture 或 research judge。CLI contract case 会在 debug 产品构建后、仓库评估前运行，验证 agent 可见 help 暴露 `repo index-worker`，并验证 idle worker attempt 返回可解析 JSON 以及 `started`/`item`/`completed` streaming JSON event，而不是空 stdout。`code_index_recovery_cases` 覆盖 code-index 过期 task lease 恢复、旧 worker 完成拒绝、attempt budget dead-letter 和 checkpoint batch 续租，不需要索引 exhaustive 大仓。SQLite lock gate 会用独立 file-backed store 打开同一个数据库，验证重复 full-index start 复用 running task，并且不同 fingerprint 的 task 能 claim 独立 lease、不会等待另一个 running task。`fast` 还会运行注册 guardrail，验证 `repo register --language` 被拒绝，避免 C/C++ 混合仓库在注册期被错误收窄。`fast` 评估 `index_performance_many_files`、`c_syntax_fixture`、`cpp_syntax_fixture`、`cross_language_syntax_fixture`、`typescript_syntax_fixture`、`nonstandard_layout_fixture`、`software_global_fixture`、`project_alias_fixture`、`relay_teams`、`leveldb_cpp`、`temporal_samples_go` 和 `temporal_sdk_go`，每个普通仓库默认取前 8 条 query case，并始终保留显式 `guardrail=true` 底线 case；新增的 index performance 生成式 fixture 会创建 1024 个小 Rust 文件并走真实 `repo register` + `repo index`，让默认 fast 在没有外部大仓 checkout 时也能暴露 cold register-to-index 吞吐回归；project alias fixture 会先无 `--alias` 注册生成仓库，再给同一个 root 增加 session 风格显式 alias，并验证两个 alias 都复用同一 indexed scope；TypeScript、C 和 C++ syntax fixture 会把 external import source fallback 底线 case 纳入默认 fast 循环，并要求外部依赖源码缺失保持 unresolved edge metadata 而不是 `degraded_reason`；C/C++ fixture 还覆盖 macro-generated handler 和 export-macro-decorated class，避免可恢复 parser error 迫使查询退到 source fallback；nonstandard layout fixture 会把 Python、TypeScript、Go、Java、C++ 和 Swift 中不在顶层 `src/` 下的源码纳入 fast 底线，并用 Cargo、npm、Go、Python、Maven BOM、Gradle 和 Conan manifest/lockfile case 守护 `repo query --kind sbom` 依赖清单能力；software global fixture 会在生成式仓库上运行 `repo software`，守护 dependencies、sdks、files、topics、relationships、build、iac、design 和 all 投影 kind，并要求这些事实只来自已索引证据，不扫描包缓存、云 API、SDK 目录或未索引外部源码；C fixture 在 fast case window 前段包含显式 source/text-fallback case，因此 fast 运行不需要额外索引大仓也能覆盖精确源码文本兜底，并且不依赖外部 `rg` 二进制；repo-set 默认保留 `temporal_go_workspace` 的 2 条跨仓门槛 case；semantic/vector 默认运行 1 条 guardrail query。它复用 `.git/relay-knowledge-self-iteration/cache-v2/fast-evaluation-home/` 作为评估 home，减少重复注册和索引成本。评分历史按 profile 和 category focus 隔离，`fast --categories semantic_vector` 只和同类 fast 历史比较，不会把 full/exhaustive 的 judge 分数或默认 fast workload 当作回归基线；采纳时还会检查同 profile 下跨 category 的最佳已提交 run，避免新 category 首次运行因为同类 baseline 为空而接纳低于 profile 水位的候选。可用 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPOS=index_performance_many_files,c_syntax_fixture,cpp_syntax_fixture,cross_language_syntax_fixture,typescript_syntax_fixture,nonstandard_layout_fixture,software_global_fixture,project_alias_fixture,relay_teams,leveldb_cpp,temporal_samples_go,temporal_sdk_go`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_CASE_LIMIT=12`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SETS=temporal_go_workspace` 和 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SET_CASE_LIMIT=2` 调整默认子集。需要完整旧门禁和完整 workload 时显式传 `--profile full`；长周期大仓扩展仍使用 `--profile exhaustive`。`full` 和 `exhaustive` 额外运行 `index_performance_wide_mixed_files` 生成式性能仓库，它创建 2048 个 Rust 目标文件与跨 shard bridge 查询，在不增加默认 fast 成本的前提下用更宽的 cold index 与查询 p50/p95/max 预算提高性能门槛。

`fast` 的 targeted gate 还包括 `skill_metadata_policy_cases`，用于拒绝把 Windows `.exe` 或 `assets/windows-*` 命令放进 bash/POSIX code fence 的 CLI skill 描述，确保 agent-facing 指令保持 shell-specific。
...
- 仓库注册后索引性能 targets：`cases/repository_index_performance_targets.json` 收紧 `index_budget_ms`，并新增 `register_index_budget_ms` 组合预算。默认 fast 包含生成式 `index_performance_many_files` 仓库，即使外部大仓缺失也能持续测到 cold throughput 回归。`full`/`exhaustive` 还包含 2048 文件的 `index_performance_wide_mixed_files` Rust workspace 和跨 shard bridge 查询。评估器会同时记录 `*_index_ms` 与 `*_register_index_ms`，让自迭代优先优化 `repo register` 后 cold index 的批处理、解析吞吐、SQLite 写入、finalize、查询 p50/p95/max 和增量复用路径。
- 软件全域投影 targets：`cases/repository_software_global_targets.json` 会在生成式 `scope=all` 仓库完成普通注册和索引后运行 `repo software`。这些 case 覆盖 `dependencies`、`sdks`、`files`、`topics`、`relationships`、`build`、`iac`、`design` 和 `all` 投影 kind，验证 unresolved 外部 SDK metadata、文档/配置关系、构建 target、IaC resource 和 design element 都来自已索引证据。
- CLI contract cases：`cases.json` 中的 `cli_contract_cases` 会直接运行产品 CLI 命令，不需要索引大仓。默认 fast guardrail 覆盖 `repo index-worker` 的 machine-readable help、idle JSON 输出和 streaming JSON 输出，确保没有长驻 service 时非交互 agent 仍能显式推进 durable code-index task。

...
| `software_global_fixture` | 确保 `repo software` 投影事实来自已索引证据，不扫描包缓存、云 API、SDK 目录或未索引外部源码；其 legacy inline Knowledge map 还必须保留必需的时间戳/history 元数据，使兼容性索引能进入 topic 投影阶段。 |

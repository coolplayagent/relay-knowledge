...
默认 profile 是 `fast`。它只跑产品与 harness 的 `fmt --check`，检查 release workflow 仍强制 Linux GNU glibc 2.31 baseline，再跑产品 debug build、harness `cargo check` 和 targeted `code_index_recovery_cases` gate，然后用 `target/debug/relay-knowledge` 执行评估；默认不跑产品 release build、全量 clippy、全量 test、本地文件 fixture 或 research judge。`code_index_recovery_cases` 覆盖 code-index 过期 task lease 恢复、旧 worker 完成拒绝、attempt budget dead-letter 和 checkpoint batch 续租，不需要索引 exhaustive 大仓。`fast` 评估 `c_syntax_fixture`、`cpp_syntax_fixture`、`cross_language_syntax_fixture`、`typescript_syntax_fixture`、`nonstandard_layout_fixture`、`relay_teams`、`leveldb_cpp`、`temporal_samples_go` 和 `temporal_sdk_go`，每个普通仓库默认取前 8 条 query case，并始终保留显式 `guardrail=true` 底线 case；TypeScript、C 和 C++ syntax fixture 会把 external import 底线 case 纳入默认 fast 循环，并要求外部依赖源码缺失保持 unresolved edge metadata 而不是 `degraded_reason`；C/C++ fixture 还覆盖 macro-generated handler 和 export-macro-decorated class，避免可恢复 parser error 迫使查询退到 grep；nonstandard layout fixture 会把 Python、TypeScript、Go、Java、C++ 和 Swift 中不在顶层 `src/` 下的源码纳入 fast 底线，并用 Cargo、npm、Go、Python、Maven BOM、Gradle 和 Conan manifest/lockfile case 守护 `repo query --kind sbom` 依赖清单能力；C fixture 在 fast case window 前段包含显式 grep/text-fallback case，因此 fast 运行不需要额外索引大仓也能覆盖精确源码文本兜底，同时不会把本机缺少 `rg` 变成 hard quality-gate 失败；repo-set 默认保留 `temporal_go_workspace` 的 2 条跨仓门槛 case；semantic/vector 默认运行 1 条 guardrail query。它复用 `.git/relay-knowledge-self-iteration/cache-v2/fast-evaluation-home/` 作为评估 home，减少重复注册和索引成本。评分历史按 profile 和 category focus 隔离，`fast --categories semantic_vector` 只和同类 fast 历史比较，不会把 full/exhaustive 的 judge 分数或默认 fast workload 当作回归基线；采纳时还会检查同 profile 下跨 category 的最佳已提交 run，避免新 category 首次运行因为同类 baseline 为空而接纳低于 profile 水位的候选。可用 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPOS=c_syntax_fixture,cpp_syntax_fixture,cross_language_syntax_fixture,typescript_syntax_fixture,nonstandard_layout_fixture,relay_teams,leveldb_cpp,temporal_samples_go,temporal_sdk_go`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_CASE_LIMIT=12`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SETS=temporal_go_workspace` 和 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SET_CASE_LIMIT=2` 调整默认子集。需要完整旧门禁和完整 workload 时显式传 `--profile full`；长周期大仓扩展仍使用 `--profile exhaustive`。
...
- 多语言代码仓库检索 targets：语言 case 已按
  ...
  reference 与 hybrid 精确文本兜底，确保 grep/text-fallback 能在默认 profile 中持续被验证。

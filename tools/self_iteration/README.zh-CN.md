...
| CLI contract case | 验证 agent 可见 help 暴露 `repo index-worker`，验证 idle worker 与 streaming worker 输出可解析 JSON，并保护强类型 CodeSpec/Knowledge map 的 help、校验、目录过滤、保留路由、有序 v3 shard 路由和 archive→recent 历史分页。 |
...
| `repository_map_graph_fixture` | 索引一个含 8 个内容寻址 topic shard 和 1 个未引用孤儿 shard 的有效 v3 Knowledge Map；要求 root/shard identity join 投影全部授权 topic 与 `documents` edge，禁止孤儿成为 map 证据，并在同一个 combined software view 中检查 dependency、build、IaC 与 design 维度。 |
...
| Repository Map 图谱 targets | `repository_map_targets.json` 把 8 个只读 CLI contract 与一个生成式 v3 仓库组合起来；3 个真实索引 case 覆盖内容寻址 root 授权、8 个 map topic/relationship 维度、孤儿隔离，以及查询热路径不读 live source 的 combined software projection。 |
...
| CLI contract cases | 直接运行产品 CLI，不需要大仓；默认 fast 覆盖 `repo index-worker` help、idle/streaming JSON，以及强类型 CodeSpec/Knowledge map 的 help、校验、目录过滤、保留/有序多源路由和 archive/recent 历史分页。 |

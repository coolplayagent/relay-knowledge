...
默认 profile 是 `fast`。它只跑产品与 harness 的 `fmt --check`，再跑产品 debug build 和 harness `cargo check`，用 `target/debug/relay-knowledge` 执行评估；默认不跑产品 release build、全量 clippy、全量 test、本地文件 fixture 或 research judge。`fast` 评估 `relay_teams`、`leveldb_cpp`、`temporal_samples_go` 和 `temporal_sdk_go`，每个普通仓库默认取前 8 条 query case，并始终保留显式 `guardrail=true` 底线 case；repo-set 默认保留 `temporal_go_workspace` 的 2 条跨仓门槛 case；semantic/vector 默认运行 1 条 guardrail query。它复用 `.git/relay-knowledge-self-iteration/cache-v2/fast-evaluation-home/` 作为评估 home，减少重复注册和索引成本。评分历史按 profile 和 category focus 隔离，`fast --categories semantic_vector` 只和同类 fast 历史比较，不会把 full/exhaustive 的 judge 分数或默认 fast workload 当作回归基线；采纳时还会检查同 profile 下跨 category 的最佳已提交 run，避免新 category 首次运行因为同类 baseline 为空而接纳低于 profile 水位的候选。可用 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPOS=relay_teams,leveldb_cpp,temporal_samples_go,temporal_sdk_go`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_CASE_LIMIT=12`、`RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SETS=temporal_go_workspace` 和 `RELAY_KNOWLEDGE_SELF_ITERATION_FAST_REPO_SET_CASE_LIMIT=2` 调整默认子集。需要完整旧门禁和完整 workload 时显式传 `--profile full`；长周期大仓扩展仍使用 `--profile exhaustive`。

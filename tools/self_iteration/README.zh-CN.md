...
Fast `software_relationship_storage_cases` 门禁执行 `cargo test --lib software_relationship_storage -- --nocapture`，覆盖 4,096 map topic、零持久边写入/新增页、配置边精确去重、稳定 ID/证据、limit 前 scope 过滤与有界计数/查询耗时，保护 schema 8 去除冗余兼容边存储且保留类型化 ontology statement 的合同。

...
- Treat cases.json as the target workload for self-improvement, not only as a snapshot of already-working behavior. Use research_judge_suite competitive_feature_targets and implementation_guardrails as the near-term research capability roadmap; if newly added JavaScript, Java, C, C++, local-file, research, or background-indexing targets fail, improve the underlying parser, graph extraction, candidate retrieval, ranking, UI/API workflow, or service workflow rather than deleting or weakening the case.

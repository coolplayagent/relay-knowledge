...
| `--profile fast` | You want the default local loop or pre-PR check | Runs formatting, debug build, harness check, key product gates including hierarchical BM25 invariants, the default repository subset, repo-set guards, and a semantic/vector guardrail. |
...
`full` and `exhaustive` also run `index_performance_wide_mixed_files`, which generates 2048 Rust target files and cross-shard bridge queries. Both generated performance repositories use an isolated runtime home, require cold completion evidence (`task.state=succeeded` or a non-zero parsed-file summary), and create a second Git commit with modified, added, and deleted files. The default-fast `index_performance_many_files` has 1024 files under its configured `src` filter plus one discovered `external_deps/rust_sdk` source file, then runs `repo index` again to protect full initialization's automatic historical-commit reuse with effective discovered filters; the wide case continues to run `repo update` against the persisted base. Reports record `*_cold_index_ms`, `*_cold_register_index_ms`, `*_initialization_incremental_index_ms` or `*_incremental_index_ms`, and query p50/p95/max metrics. The completion guards reject cached no-op cold runs and incremental runs that read or parse more files than the bounded delta permits.

`.github/workflows/benchmark-checks.yml` first exercises a real gitlink whose child expands beyond the historical-reuse path budget, then runs the 1025-file performance fixture on pull requests and verifies the completed cold task/checkpoint, the three-path incremental delta, the two-file blob/parse budget, completion commands, and all three latency budgets directly from the JSON report.

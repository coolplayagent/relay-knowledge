...
The fast `software_relationship_storage_cases` gate runs `cargo test --lib software_relationship_storage -- --nocapture`: 4,096 map topics, zero persistent edge writes/pages, exact configuration-edge deduplication, stable IDs/provenance, scoped filtering before limits, and bounded count/query latency. It protects the schema-8 removal of redundant compatibility-edge storage while retaining typed ontology statements.

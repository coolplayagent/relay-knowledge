...
| `configuration_scan_work_suite` | Runs after library preparation in the fast profile with a 120-second timeout and a 30-second diagnostic budget. A long function-only source line with absent API patterns must perform zero quote-predicate calls; this deterministic work assertion catches repeated character decoding without machine-specific latency thresholds. Full library tests additionally compare Unicode/quote/escape behavior and cached CR/LF/CRLF line positions against the original semantics. |
...
The fast/performance `code_index_persistence_performance_suite_scope_preview_stops_after_overflow_batch` case checks that read-only scope preview returns the first 50 degraded paths and parser reasons, marks truncation only after observing the 51st file, and does not request a subsequent parser batch. A cancellation guard fails if parsing continues past the overflowing batch, while exact selected-file statistics remain required. Separate boundary tests cover 0/49/50/51 entries, late overflow after exactly 50 diagnostics, and independent exclusion truncation. This limits preview work only; durable indexing still completes all stages.

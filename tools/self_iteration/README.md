...
| Software global ontology targets | `repository_software_global_targets.json` runs every compatibility and typed `repo software` kind, checks ontology version 1.0.0, projection schema 6, full statement-provenance completeness, and critical forbidden misclassifications. These cases run in the fast fixture and are also selected by `--categories performance`, so projection throughput/query budgets and semantic classification regressions remain coupled without repository-specific product code. |

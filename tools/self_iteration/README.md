...
| CLI contract cases | Verify agent-visible help exposes `repo index-worker`, idle worker plus streaming worker output parseable JSON, and typed CodeSpec/Knowledge map help, validation, directory filtering, reserved routes, ordered v3 shard routing, and archive-to-recent history paging. |
...
| `repository_map_graph_fixture` | Indexes a valid v3 Knowledge Map with eight content-addressed topic shards and one unreferenced orphan. It requires root/shard identity joins to project every authorized topic and `documents` edge, forbids the orphan from becoming map evidence, and checks the combined software view alongside dependency, build, IaC, and design dimensions. |
...
| Software global ontology targets | `repository_software_global_targets.json` runs every compatibility and typed `repo software` kind, checks ontology version 1.0.0, projection schema 7, full statement-provenance completeness, machine-readable OpenAPI provenance, and critical forbidden misclassifications. These cases run in the fast fixture and are also selected by `--categories performance`, so projection throughput/query budgets and semantic classification regressions remain coupled without repository-specific product code. |
| Repository Map graph targets | `repository_map_targets.json` combines eight read-only CLI contracts with a generated v3 repository. Three real index-backed cases cover content-addressed root authorization, all eight map topic/relationship dimensions, orphan isolation, and the combined software projection without reading live source at query time. |
...
| CLI contract cases | Run product CLI commands without indexing a large repository; default fast covers `repo index-worker` help/idle/streaming JSON plus typed CodeSpec/Knowledge map help, validation, directory filtering, reserved and ordered multi-source routing, and archive/recent history paging. |

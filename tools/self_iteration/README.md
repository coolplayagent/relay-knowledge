...
The 12,000-file `code_index_persistence_performance_suite` publication case also
resumes the durable software phases and requires 12,000 reported SDK relationships
with zero stored compatibility rows before accepting the completed checkpoint.
...
| Default repositories | `index_performance_many_files`, `index_performance_c_fragment`, `c_syntax_fixture`, `cpp_syntax_fixture`, `cross_language_syntax_fixture`, `typescript_syntax_fixture`, `nonstandard_layout_fixture`, `software_global_fixture`, `repository_map_graph_fixture`, `project_alias_fixture`, `relay_teams`, `leveldb_cpp`, `temporal_samples_go`, and `temporal_sdk_go`. |
...
| `software_global_fixture` | Ensures `repo software` facts come from indexed evidence and protects ontology classification: generic README headings stay documentation, machine-readable OpenAPI input becomes an `api_schema` API entity, Dockerfiles are build definitions, CI jobs are not IaC resources, and Terraform, Kubernetes, Compose, and systemd retain deployment/resource types. It also checks typed systems/APIs/resources/tests/deployments/releases, statement provenance, conflicts, ontology/schema versions, and completeness without scanning package caches, cloud APIs, SDK directories, or unindexed external source. |
| `repository_map_graph_fixture` | Indexes a valid v4 Knowledge Map with eight content-addressed topic shards and one unreferenced orphan. It requires root/shard identity joins to project every authorized topic and `documents` edge, forbids the orphan from becoming map evidence, and checks the combined software view alongside dependency, build, IaC, and design dimensions. |
...
Software lifecycle projection filters ordinary source chunks in SQLite before Rust materialization by using a semantic superset of the supported manifest, CI/IaC, and Markdown paths. It preflights fixed ceilings of 32,768 candidate documents, 262,144 chunks, and 256 MiB, streams one path-ordered document at a time through the build/IaC/design collectors, and reports candidate document, chunk, and byte counts. Component, dependency-usage, SDK, build, IaC, design, entity, statement, and diagnostic writes reuse bounded storage paths. For a fenced single-SQLite full or incremental index, software projection v2 advances through reset, dependencies, SDK usages, lifecycle, files, topics, relationships, ontology, and publish; the code scope remains stale while intermediate phases release the writer for lease renewal. After fence revalidation, software status, code-scope/repository freshness, checkpoint completion, and the publication receipt become visible together. A partitioned store does not claim cross-database atomicity: code and software facts complete in the target shard while the catalog route remains task-owned and `staged`; one fenced control-database transaction then validates that owner, activates the repository and scope routes, mirrors fresh status, and records the receipt. Public checkpoint state stays pre-publication until that active control route exists. Both pre-control and post-control crashes converge idempotently without re-parsing an already durable target. Task `succeeded` is a subsequent fenced completion transaction that requires the receipt and matching fresh target, and the external worker response still waits for that terminal task state.
...
`.github/workflows/benchmark-checks.yml` prebuilds the release product outside the timed pull-request workload so cold hosted-runner compiler variance is not misreported as index latency. It then runs the 1024-file performance fixture, asserts that the JSON report selected `target/release/relay-knowledge` and passed its incremental build gate, and verifies the completed cold task/checkpoint, the three-path incremental delta, the two-file blob/parse budget, completion commands, and all three latency budgets directly from that report.
...
| Software global ontology targets | `repository_software_global_targets.json` runs every compatibility and typed `repo software` kind, checks ontology version 1.0.0, projection schema 8, full statement-provenance completeness, machine-readable OpenAPI provenance, and critical forbidden misclassifications. These cases run in the fast fixture and are also selected by `--categories performance`, so projection throughput/query budgets and semantic classification regressions remain coupled without repository-specific product code. |
| Repository Map graph targets | `repository_map_targets.json` combines eight read-only CLI contracts with a generated v4 repository. Four real index-backed cases cover content-addressed root authorization, all eight map topic/relationship dimensions, orphan isolation, cross-dimension typed statement provenance, and the combined software projection without reading live source at query time. |
...
| CLI contract cases | Run product CLI commands without indexing a large repository; default fast covers `repo index-worker` help/idle/streaming JSON plus typed CodeSpec/Knowledge map help, validation, directory filtering, reserved and ordered multi-source routing, and recent-only history paging. |
...
The `canonical_call_query_work_budget` quality gate runs as an isolated stage in fast/full/exhaustive profiles after the existing library-test build. It executes the real product query entry point against 32,768 matching call edges and reports callers/callees SQLite VM steps from the same execution-budget facility used by the query. Each direction must emit one `SELF_ITERATION_METRIC` JSON line with its `canonical_call_callers_vm_steps` or `canonical_call_callees_vm_steps` name, a positive integer `value`, and `budget: 150000`. Both become key, lower-is-better metrics. Missing, malformed, duplicate, zero, or mismatched-budget observations fail the gate, including a Cargo filter that executes zero tests. Exceeding 150,000 steps fails regardless of wall-clock timing; the former full-sort query exceeds one million steps on this fixture. No SQL copy or fixture-specific product path is used. The existing canonical-call CLI workload still checks end-to-end results and its broader latency budget.
...
The `feature_flags_wide_v1` performance fixture also includes 1,100 query-only keys, 1,100 metadata-only keys, and one matching key. Four `feature_flags_metadata_intersection_*` guardrails combine text with domain, source format, or hot-reload predicates before candidate admission. They must find `zzz_metadata_needle` at rank 1 under the existing 2-second query budget; replacing group filtering with either individual seed window fails these cases. The fixture remains split into small files and retains the original exact-key check over more than 10,000 unrelated usages.

The separate `feature_flags_binding_groups` fast/performance guardrail adds 2,201 disjoint text/metadata candidates and a real Java constant read whose text evidence and properties metadata occupy different usages. Its rank-one, fresh-result query and 2,000 ms repository p95 budget exercise the complete alias-group SQL plan, including reverse binding traversal. This catches repeated correlated-closure work that a fixture containing only independent keys cannot expose; the original 36-file, five-case workload remains intact.

Two additional isolated cold runs of the unchanged 36-file fixture, `properties_parallel_cold_stability_1` and `_2`, require fresh results without a degradation reason. They protect properties scanner stability during parallel indexing and keep the same 2,000 ms query p95 budget.

The canonical call VM gate also includes 2,048 non-callable field snapshots sharing the selected function identity. Both exact directions must retain the same results and the existing 150,000-step ceiling; counting those fields against callable admission fails this fast-profile regression.

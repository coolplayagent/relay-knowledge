...
`.github/workflows/benchmark-checks.yml` runs the 1024-file performance fixture on pull requests that change the root Rust crate/tests, the self-iteration harness, or the performance workflow itself. Every relevant run verifies the completed cold task/checkpoint, the three-path incremental delta, the two-file blob/parse budget, completion commands, and the presence of all three latency observations directly from the JSON report. Root Rust and performance-harness candidates additionally enforce the configured absolute latency budgets; workflow-only maintenance still executes and exposes those latency values but does not fail solely because an unchanged product baseline runs slower on a particular hosted runner. Pushes to `main` and manual dispatches enforce latency unconditionally. The self-iteration candidate-adoption exit status is informational in this CI path because a clean Actions checkout may contain no uncommitted candidate diff; a missing report, failed deterministic gate/case, missing fixture evidence, or an enforced latency overrun still fails the strict `jq` validation. Documentation-only and unrelated tooling pull requests skip this gate.

...
| Repository Map graph targets | `repository_map_targets.json` combines eight read-only CLI contracts with a generated v4 repository. Three real index-backed cases cover content-addressed root authorization, all eight map topic/relationship dimensions, orphan isolation, and the combined software projection without reading live source at query time. |

//! CLI grammar diagnostics and command-aware suggestion ownership.

use super::{CliDiagnostic, CliError, OutputFormat, spec};
...
struct ParsedInvocation<'a> {
    ...
    command: Option<&'a spec::CliCommandSpec>,
    usage: Option<String>,
    expected: Vec<String>,
}

fn matched_path(commands: &[spec::CliCommandSpec], tokens: &[String]) -> Vec<String> {
...
fn command_for_path<'a>(
    commands: &'a [spec::CliCommandSpec],
    path: &[String],
) -> Option<&'a spec::CliCommandSpec> {
...
fn expected_terms(
    ...
    command: Option<&spec::CliCommandSpec>,
    matched_path: &[String],
) -> Vec<String> {
...
fn command_path_starts_with(command: &spec::CliCommandSpec, prefix: &[&str]) -> bool {
...
fn flag_style_command_suggestion(commands: &[spec::CliCommandSpec], token: &str) -> Option<String> {
...
fn local_option_suggestion(command: Option<&spec::CliCommandSpec>, token: &str) -> Option<String> {
...
fn enum_value_suggestion(command: Option<&spec::CliCommandSpec>, value: &str) -> Option<String> {
    let command = command?;
    command
        .options
        .iter()

...
#[test]
fn dependency_tour_attaches_file_evidence_to_file_only_modules() {
    let request = request(CodebaseViewKind::DependencyTour, 10);
    let snapshot = CodebaseViewSnapshot {
        files: vec![file("src/billing/service.rs", "rust")],
        ..CodebaseViewSnapshot::default()
    };
    ...
    let section = derived
        ...
        .find(|section| section.id == "section:dependency_tour")
        .unwrap();
    let module_node = derived
        .nodes
        .iter()
        .find(|node| node.id == "module:billing")
        .unwrap();

    assert!(!module_node.evidence_ids.is_empty());
    assert_eq!(section.evidence_ids, module_node.evidence_ids);
    assert!(derived.evidence.iter().any(|evidence| {
        module_node.evidence_ids.contains(&evidence.id)
            && evidence.evidence_kind == "file"
            && evidence.path == "src/billing/service.rs"
    }));
}
...
fn dependency_tour_includes_manifest_dependency_rows() {
    let request = request(CodebaseViewKind::DependencyTour, 10);
    let snapshot = CodebaseViewSnapshot {
        files: vec![file("Cargo.toml", "toml")],
    ...
    let root_node = derived
        .nodes
        .iter()
        .find(|node| node.id == "module:root")
        .unwrap();
    assert!(root_node.evidence_ids.iter().any(|evidence_id| {
        derived
            .evidence
            .iter()
            .any(|evidence| evidence.id == *evidence_id && evidence.path == "Cargo.toml")
    }));
    assert!(
        !derived
            .nodes
            .iter()
            .any(|node| node.id == "module:Cargo.toml")
    );
}

...
pub(super) fn route_domain(url: &str) -> Option<String> {
    url.trim_matches('/')
        ...
        .find(|segment| {
            ...
            !segment.is_empty()
                && !is_route_placeholder_segment(segment)
...
pub(super) fn path_domain(path: &str) -> Option<String> {
    ...
    let start = parts
        ...
        .position(|segment| {
            let lower = segment.to_ascii_lowercase();
            !is_source_root_segment(&lower) && !is_workspace_container_segment(&lower)
        })
        .unwrap_or(0);
    let start = path_domain_scan_start(&parts, start);
    parts
        .iter()
        .enumerate()
        .skip(start)
...
pub(super) fn module_key(path: &str) -> String {
    ...
    match parts.as_slice() {
        [] => "root".to_owned(),
        [single] if is_entrypoint_or_metadata_file(&single.to_ascii_lowercase()) => {
            "root".to_owned()
        }
        ...
        [first, package, domain, ..]
            if is_source_root_segment(first) && is_src_layout_package_segment(package, domain) =>
        {
        ...
        [first, second]
            if matches!(*first, "src" | "app" | "lib")
                && is_entrypoint_or_metadata_file(&second.to_ascii_lowercase()) =>
        {
            "root".to_owned()
        }
...
pub(super) fn affected_candidate_matches_changed_path(
    ...
    let changed_module = module_key(changed_path);
    let candidate_module = module_key(candidate_path);
    if changed_module == "root" && candidate_module == "root" {
        return same_changed_file_parent(changed_path, candidate_path);
    }
    changed_module == candidate_module || same_changed_file_parent(changed_path, candidate_path)
}

fn path_domain_scan_start(parts: &[&str], start: usize) -> usize {
    if start > 0
        && is_source_root_segment(parts[start - 1])
        && parts
            .get(start + 1)
            .is_some_and(|next_segment| is_src_layout_package_segment(parts[start], next_segment))
    {
        return start + 1;
    }
    if parts
...
fn is_src_layout_package_segment(segment: &str, next_segment: &str) -> bool {
    let lower = segment.to_ascii_lowercase();
    !is_module_boundary(&lower)
        && !is_source_root_segment(&lower)
        && !is_entrypoint_or_metadata_file(&lower)
        && !next_segment.contains('.')
        && is_domain_segment(next_segment)
}
...
fn is_route_placeholder_segment(segment: &str) -> bool {
    segment.starts_with(':')
        || segment.starts_with('{')

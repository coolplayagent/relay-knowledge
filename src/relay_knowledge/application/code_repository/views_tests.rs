...
#[test]
fn evidence_pruning_keeps_returned_edge_evidence_before_large_sections() {
    let request = request(CodebaseViewKind::ArchitectureLayers, 2, Vec::new());
    let mut files = (0..20)
        .map(|index| file(&format!("src/domain/module_{index}.rs"), "rust"))
        .collect::<Vec<_>>();
    files.push(file("src/interfaces/http.rs", "rust"));
    let snapshot = CodebaseViewSnapshot {
        files,
        imports: vec![import(
            "src/domain/module_0.rs",
            "crate::interfaces::http",
            Some("src/interfaces/http.rs"),
        )],
        ..CodebaseViewSnapshot::default()
    };
    ...
    let edge = derived
        .edges
        .iter()
        .find(|edge| edge.edge_kind == "imports")
        .unwrap();
    let evidence_ids = derived
        .evidence
        .iter()
...
#[test]
fn process_flow_matches_handler_file_calls_by_symbol_id() {
    let request = request(CodebaseViewKind::ProcessFlow, 10, Vec::new());
    let mut handler_call = call(
        "src/controllers/users.rs",
        Some("renamed_list_users"),
        "load_users",
        Some("src/domain/users.rs"),
    );
    handler_call.call.caller_symbol_snapshot_id = Some("symbol:list_users".to_owned());
    let snapshot = CodebaseViewSnapshot {
        routes: vec![route(
            "route-1",
            "src/routes.rs",
    ...
    assert!(derived.edges.iter().any(|edge| edge.edge_kind == "calls"));
    assert!(derived.nodes.iter().any(|node| {
        node.node_kind == "handler" && node.path.as_deref() == Some("src/controllers/users.rs")
    }));
    assert!(derived.nodes.iter().any(|node| {
        node.node_kind == "call_target" && node.path.as_deref() == Some("src/domain/users.rs")
    }));
}

#[test]
fn process_flow_matches_qualified_handler_leaves() {
    let request = request(CodebaseViewKind::ProcessFlow, 10, Vec::new());
    let mut handler_call = call(
        "src/api/users.rs",
        Some("api::users::listUsers"),
        "load_users",
        Some("src/domain/users.rs"),
    );
    handler_call.call.caller_symbol_snapshot_id = None;
    let snapshot = CodebaseViewSnapshot {
        routes: vec![route(
            "route-1",
            "src/api/users.rs",
...
#[test]
fn process_flow_summarizes_route_when_handler_node_budget_is_exhausted() {
    ...
    assert_eq!(derived.sections.len(), 2);
    assert_eq!(derived.nodes.len(), 3);
    assert_eq!(
        derived.sections[1].node_ids,
        vec!["route:route-2".to_owned()]
    );
    assert!(derived.budget.nodes_truncated);
    assert!(derived.budget.sections_truncated);
    assert!(
...
#[test]
fn process_flow_limit_one_returns_route_summary() {
    let request = request(CodebaseViewKind::ProcessFlow, 1, Vec::new());
    let snapshot = CodebaseViewSnapshot {
        routes: vec![route(
            "route-1",
            "src/api/users.rs",
    ...
    assert_eq!(derived.nodes[0].id, "route:route-1");
    assert_eq!(derived.sections[0].id, "section:route:route-1");
    assert_eq!(derived.sections[0].node_ids, vec!["route:route-1"]);
    assert!(derived.budget.nodes_truncated);
    assert!(
        derived.sections[0]
            .diagnostics

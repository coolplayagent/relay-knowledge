//! Maps storage-port traits to the focused SQLite persistence owners.
...
use super::super::{
    canvas,
    code::lifecycle::report,
    code_graph, file_index,
    graph::{commit_batch, current_graph_version, inspect_graph},
    indexing,
    mutation_log::read_mutations_after,
    ...
    fn health_snapshot(&self, now_ms: u64) -> StorageFuture<'_, HealthStorageSnapshot> {
        ...
        self.try_run_read(move |connection| {
            Ok(HealthStorageSnapshot {
                ...
                repository_code_totals: report::repository_totals(connection)?,

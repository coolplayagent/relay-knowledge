//! Cross-owner file and content retirement contracts.
...
fn retiring_roots_clears_content_diagnostics() {
    ...
    assert_eq!(initial.stale_content_cursor_count, 0);
...
fn content_truncated_scan_still_retires_unobserved_file_paths() {
    ...
    let keep_content_hits = search_content(
        &connection,
        FileContentSearchRequest {
            query: "keep".to_owned(),
            source_scope: Some("local-files".to_owned()),
    ...
    .expect("content search should run");
    assert_eq!(keep_content_hits.len(), 1);
    assert_eq!(keep_content_hits[0].path, "/workspace/docs/keep.md");
}
...
fn full_content_scan_retires_cursors_for_missing_files() {
    ...
    let initial = cursors(&connection).expect("cursors should load");
    assert!(
        initial
            .iter()
    ...
    let refreshed_cursors = cursors(&connection).expect("cursors should reload");
    assert!(
        refreshed_cursors
            .iter()

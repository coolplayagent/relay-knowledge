...
fn content_search_returns_user_source_chunks_and_candidate_facts() {
    ...
    let dependency = hits[0]
        .fact_candidates
        .iter()
        .find(|candidate| candidate.predicate == "depends_on")
        .expect("dependency candidate should be extracted");
    assert_eq!(dependency.span.start_line, 2);
    assert_eq!(dependency.span.end_line, 2);
    assert_eq!(dependency.span.start_byte, 10);
    assert_eq!(dependency.span.end_byte, 37);

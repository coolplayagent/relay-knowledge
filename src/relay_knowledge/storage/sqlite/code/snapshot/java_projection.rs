...
pub(super) const COPY_BYTES: &str = "CASE WHEN source.language_id='java' THEN
    ...
    +COALESCE((SELECT length(CAST(package AS BLOB))+length(CAST(source_set_kind AS BLOB))+length(CAST(module_root AS BLOB)) FROM code_repository_java_namespaces namespace
       WHERE namespace.source_scope=source.source_scope AND namespace.path=source.path),7)
    +COALESCE((SELECT sum(length(CAST(source_scope AS BLOB))+length(CAST(path AS BLOB))
       +length(CAST(package AS BLOB))+length(CAST(type_name AS BLOB))+length(CAST(source_set_kind AS BLOB))+length(CAST(module_root AS BLOB))+128)

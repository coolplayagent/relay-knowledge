...
async fn code_index_task_queue_status_reports_master_worker_backlog() {
    ...
    let retrying = store
        .run(|connection| queue_task(connection, seed("fp-retry", "scope-retry", 10)))
        .await
        .expect("retrying task should queue");
    let _queued = store
        .run(|connection| queue_task(connection, seed("fp-queued", "scope-queued", 11)))
        .await

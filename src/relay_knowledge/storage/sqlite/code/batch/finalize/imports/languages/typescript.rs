...
use super::super::super::symbols::{self, SymbolKey};
...
pub(in super::super) fn needs_symbol_index(import_path: &str, statement: &str) -> bool {
    TypeScriptImportRequest::parse(import_path, statement)
        .is_some_and(|request| !request.imported_names.is_empty())
}

pub(in super::super::super) struct NamedImportBinding {
    pub(in super::super::super) imported_name: String,
    pub(in super::super::super) local_name: String,
}

pub(in super::super::super) fn named_import_bindings(statement: &str) -> Vec<NamedImportBinding> {
...
pub(super) fn resolve_import(
    ...
    if request.imported_names.is_empty() {
        return module_paths::resolve_first_file(&request.module_paths, true, indexed_module_paths);
    }
...
impl TypeScriptImportRequest {
    ...
    fn parse_import_body(import_path: &str, body: &str) -> Option<Self> {
        if !body.contains(" from ") {
            let specifier = quoted(body)?;
            return Some(Self::for_specifier(import_path, specifier, Vec::new()));
        }

        let (imports, module) = body.rsplit_once(" from ")?;
        let specifier = quoted(module)?;
...
fn relative_module_candidates(import_path: &str, specifier: &str) -> Vec<String> {
    let base_path = if specifier.starts_with("./") || specifier.starts_with("../") {
        let Some(base_path) =
            module_paths::normalize_join(module_paths::parent(import_path), specifier)
        else {
            return Vec::new();
        };
        base_path

...
use super::super::super::{
    ...
    search::{delete_search_documents_for_kind, delete_search_documents_for_paths_and_kind},
};
...
/// Path-scoped variant of [`rebuild_reference_search_documents`]: only
/// deletes and rebuilds search documents for `affected_paths`.
pub(super) fn rebuild_reference_search_documents_for_paths(
    transaction: &Transaction<'_>,
    source_scope: &str,
    affected_paths: &[&str],
) -> Result<(), StorageError> {
    delete_search_documents_for_paths_and_kind(
        transaction,
        source_scope,
        "reference",
        affected_paths.iter().copied(),
    )?;
        ...
        let mut select = transaction.prepare(&format!(
            "
                ...
                LEFT JOIN code_repository_files file
                  ON file.source_scope = reference.source_scope
                 AND file.path = reference.path
                WHERE reference.source_scope = ? AND reference.path IN ({placeholders})
                "
        ))?;
        let mut rows = select.query(params_from_iter(values))?;
        let mut inserter = SearchDocumentInserter::new(transaction)?;
        while let Some(row) = rows.next()? {
            let record_id = row.get::<_, String>(0)?;
            let path = row.get::<_, String>(1)?;
...
/// Path-scoped variant of [`rebuild_import_search_documents`].
pub(super) fn rebuild_import_search_documents_for_paths(
    transaction: &Transaction<'_>,
    source_scope: &str,
    affected_paths: &[&str],
) -> Result<(), StorageError> {
        ...
        let mut select = transaction.prepare(&format!(
            "
                ...
                LEFT JOIN code_repository_files file
                  ON file.source_scope = import.source_scope
                 AND file.path = import.path
                WHERE import.source_scope = ? AND import.path IN ({placeholders})
                "
        ))?;
        let mut rows = select.query(params_from_iter(values))?;
        let mut inserter = SearchDocumentInserter::new(transaction)?;
...
/// Path-scoped variant of [`rebuild_call_search_documents`].
pub(super) fn rebuild_call_search_documents_for_paths(
    transaction: &Transaction<'_>,
    source_scope: &str,
    affected_paths: &[&str],
) -> Result<(), StorageError> {
        ...
        let mut select = transaction.prepare(&format!(
            "
                ...
                LEFT JOIN code_repository_symbols callee
                  ON callee.source_scope = call.source_scope
                 AND callee.symbol_snapshot_id = call.callee_symbol_snapshot_id
                WHERE call.source_scope = ? AND call.path IN ({placeholders})
                "
        ))?;
        let mut rows = select.query(params_from_iter(values))?;
        let mut inserter = SearchDocumentInserter::new(transaction)?;

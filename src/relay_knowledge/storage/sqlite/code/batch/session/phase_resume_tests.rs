...
async fn every_legacy_coarse_checkpoint_repairs_and_restores_across_reopen() {
    ...
    store
        .run(|connection| {
            ...
            connection.execute_batch(
                "DROP INDEX code_repository_calls_caller_lookup;
                DROP INDEX code_repository_calls_caller_identity_lookup;
                DROP INDEX code_repository_calls_callee_identity_lookup;",
            )?;
            connection.execute(

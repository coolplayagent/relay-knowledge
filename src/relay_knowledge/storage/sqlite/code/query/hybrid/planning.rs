...
use super::super::{
    api_identities,
    conversion_terms::conversion_action_term,
    relevance::{query_is_single_symbol_identity, query_terms},
};
...
pub(in super::super) fn hybrid_query_prefers_chunk_first(request: &CodeRetrievalRequest) -> bool {
    hybrid_chunk_first_plan(request).is_some()
}

pub(in super::super) fn strict_hybrid_chunk_candidate_limit(
...
pub(in super::super) fn hybrid_query_requires_chunk_first_before_symbols(
    request: &CodeRetrievalRequest,
) -> bool {
    matches!(
...
pub(in super::super) fn query_language_scoped_workflow_surface_scopes(
    request: &CodeRetrievalRequest,
) -> Vec<&'static str> {
...
pub(in super::super) fn workflow_language_scope_language_ids(
    scope: &str,
) -> &'static [&'static str] {
    match scope {
        "csharp" => &["cs", "csharp"],
...
fn hybrid_chunk_first_plan(request: &CodeRetrievalRequest) -> Option<HybridChunkFirstPlan> {
    ...
    if !api_identities::hybrid_api_symbol_identities(&request.query, request).is_empty() {
...
pub(in super::super) fn hybrid_sequence_terms(query: &str) -> Vec<String> {
    let raw_terms = query_terms(query);
    hybrid_sequence_terms_from_raw(raw_terms.iter().map(String::as_str))
}

pub(in super::super) fn hybrid_query_has_declaration_expansion_intent(query: &str) -> bool {
...
pub(in super::super) fn hybrid_query_has_conversion_expansion_intent(query: &str) -> bool {
    let raw_terms = query_terms(query);
    let terms = hybrid_sequence_terms_from_raw(raw_terms.iter().map(String::as_str));
...
pub(in super::super) fn hybrid_query_has_inline_expansion_intent(query: &str) -> bool {
    hybrid_sequence_terms(query).iter().any(|term| {
        matches!(
            term.as_str(),
...
pub(in super::super) fn workflow_language_scope_matches(language_id: &str, scope: &str) -> bool {

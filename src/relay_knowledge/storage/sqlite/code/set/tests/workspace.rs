//! Cross-owner workspace package overlay regression.
...
async fn repository_set_overlay_refresh_resolves_pnpm_workspace_package_imports() {
    ...
    let (summary, edges) = store
        .run(|connection| {
            ...
            let set = set::create_set(connection, set_seed("workspace", 20))?;
            set::add_member(
                connection,
            ...
            let summary = set::refresh_overlay(connection, "workspace", 30)?;
            let edges = set::cross_edges_for_set(connection, &set.set_id)?;

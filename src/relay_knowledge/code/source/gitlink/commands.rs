...
pub(in crate::code) fn submodule_worktree_root_for_commit(
    root: &Path,
    path: &str,
    commit: &str,
) -> Option<PathBuf> {
...
#[derive(Debug)]
pub(in crate::code) struct GitTreeLookup {
    pub(in crate::code) object: String,
    pub(in crate::code) kind: String,
}

pub(in crate::code) fn git_tree_entry(
    root: &Path,
    commit: &str,
    path: &str,
...
pub(in crate::code) fn git_tree_entry_from_git_dir(
    git_dir: &Path,
    commit: &str,
    path: &str,
) -> Result<Option<GitTreeLookup>, CodeIndexError> {
...
pub(in crate::code) fn safe_relative_path(path: &str) -> bool {
    !path.is_empty()
        && !path.starts_with('/')
        && !path.contains('\\')
        && !path.contains('\0')

...
use super::{
    ...
    changes::{
        ...
        submodule_git_dir, submodule_worktree_root, tracked_entries, tracked_entries_from_git_dir,
    },
    git::git_bytes,
};
...
pub(super) fn submodule_bytes(
    ...
    match target.location {
        ...
        GitlinkTargetLocation::GitDir(git_dir) => git_dir_bytes(
            &git_dir,
            &["show", &format!("{}:{}", target.commit, target.path)],
        ),
    }
}
    ...
    match target.location {
        ...
        GitlinkTargetLocation::GitDir(git_dir) => git_dir_bytes(
            ...
            &[
                "cat-file",
                "-s",
                &format!("{}:{}", target.commit, target.path),
            ],
        )
        .map(|bytes| String::from_utf8_lossy(&bytes).trim().parse::<usize>().ok()),
    }
}

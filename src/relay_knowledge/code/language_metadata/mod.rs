//! Owns language detection, parser metadata, and documentation comment policy.
...
pub(in crate::code) struct LanguageSpec {
    pub(in crate::code) id: &'static str,
    pub(in crate::code) language: fn() -> Language,
    pub(in crate::code) tags_query: &'static str,
}
...
pub(in crate::code) fn language_id(path: &str) -> Option<&'static str> {
...
pub(in crate::code) fn detect_language(path: &str) -> Option<LanguageSpec> {
    ...
    if let Some(language) = config_files::detect(path) {
...
pub(in crate::code) fn strip_supported_extension(path: &str) -> &str {
    ...
    if language_for_extension(extension)
        .or_else(|| config_files::detect(path))
        .is_none()
    {
        return path;
    }
...
pub(in crate::code) fn doc_comment_text<'a>(
    trimmed: &'a str,
    language_id: &str,
) -> Option<&'a str> {
    match language_id {
        ...
        _ => config_files::doc_comment_text(trimmed, language_id),
    }
}

pub(in crate::code) fn python_builtin_type_reference(name: &str) -> bool {
    matches!(
        name,
        "Annotated"
            | "Any"
...
pub(in crate::code) fn typescript_builtin_type_reference(name: &str) -> bool {
    matches!(
        name,
        "Array"
            | "AsyncGenerator"

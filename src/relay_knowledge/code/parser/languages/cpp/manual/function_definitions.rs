...
use super::super::super::super::nodes::{
    SyntaxRange, node_text, push_children_reverse, syntax_range,
};
use super::super::super::super::recovery::{
...
use super::lexical::{
    CppHeadToken, cpp_builtin_type_token, cpp_declaration_prefix_token, cpp_decorator_token,
    cpp_head_tokens, cpp_tokens_joined_by_qualifier, cpp_type_name_candidate,
};

pub(super) fn function_definition_symbol(
    content: &str,
    node: Node<'_>,
...
pub(in crate::code::parser) fn function_definition_is_destructor(
    ...
    let Some(parameter_start) = cpp_last_top_level_parenthesis_start(head) else {
        return false;
    };
...
fn cpp_last_top_level_parenthesis_start(head: &str) -> Option<usize> {
...
pub(super) fn gcc_decorated_function_name(content: &str, node: Node<'_>) -> Option<String> {
    let text = node_text(content, node);
    if !text.contains('{') {
        return None;
    }
...
#[cfg(test)]
#[path = "function_definitions_tests.rs"]
mod tests;

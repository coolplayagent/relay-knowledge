...
use super::{
    ...
    LEGACY_CODE_QUERY_INDEX_PLAN_V5, LEGACY_CODE_QUERY_INDEX_PLAN_V6, query_index_plan_unit_count,
};
...
impl CodeReferenceResolutionQueryIndexRepair {
    ...
    pub(crate) fn next_state(self, completed_unit: usize) -> Option<String> {
        ...
        .or_else(|| {
            code_reference_resolution_query_index_repair_state(
                completed_unit,
                self.reference_resolution,
            )
        })
...
pub(crate) fn code_reference_resolution_query_index_repair(
    ...
    ) || query_index_plan_unit_count(plan_version).is_none_or(|count| completed_unit >= count)
        || protocol_version != VERSION

...
默认 `fast` profile 还包含 targeted `code_index_recovery_cases` 和注册期 language guardrail，用来在不跑 exhaustive 大仓 workload 的情况下持续验证 code-index 过期租约恢复、旧 worker 完成拒绝、dead-letter、checkpoint 续租和混合语言注册安全性。

...
- 混合 GraphRAG 上下文包：包含 flat 或有界 single-FTS hierarchical BM25、本地语义签名、本地哈希向量检索、图证据回退、schema 路径、时间/社区上下文、新鲜度元数据、截断状态和排序解释。
...
- [第四卷第 12 章：分层 BM25 算法分析](docs/zh/04-research/12-hierarchical-bm25-analysis-2026.md)：论文证据、single-FTS route-intersection SQLite 工程适配、rank-safety 边界和基准要求。
...
满足门禁的 current graph version 可在单一全局 FTS 中按有界 SimHash group 路由 scoped BM25 query，SQL scope predicate 仍是硬授权边界。Schema-v4 通过带 lease、可续跑的 shadow rebuild 构建读模型，完成前暂停 semantic、vector 与 fuzzy read。确定性的 4,096-document synthetic production-write/query-path 门禁要求 Recall@10 >= 0.9 且 planned-MATCH result domain 更小；它既不测量 FTS posting/VM work，也不测量 query latency，更不声称自然语料性能结论。算法身份、rank-safety、预算、迁移与 benchmark 计划详见[分层 BM25 算法分析](docs/zh/04-research/12-hierarchical-bm25-analysis-2026.md)。

...
Code-index task lease 绑定 attempt：过期 running lease 会在 claim/status 路径前恢复为 retry 或 dead-letter，旧 worker 不能完成或失败已经被新 worker 接管的 task，活跃 worker 每次提交 checkpoint batch 后都会续租。JSON status 中的 checkpoint 会暴露 `updated_at_ms`，便于区分慢速推进和真正卡住。

...
Web 路由检测把 Express registration、参数/handler 解析和 mount materialization
统一收敛在 `detect/express/` 子域，并由每个 owner 直接挂载 UT。Python 静态路由
字符串与 JavaScript 注释/字符串/正则词法状态继续使用独立具名模块，不得用笼统
